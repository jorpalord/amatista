...
4. **Mismo caso peligroso (Caso 1) con mayúsculas/minúsculas reales mezcladas** (`c:\USERS\jorpa\appdata\Local\TEMP\Amatista-Hallazgo1-Test\APP` contra un `$INSTDIR` tipeado con casing distinto) → **detectado igual**, mismo rechazo — confirma que `${StrCase}` normaliza correctamente antes de comparar.
