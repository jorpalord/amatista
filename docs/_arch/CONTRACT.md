...
- [Principio de diseño — guardias monótonas (un chequeo que solo puede rechazar, nunca aprobar por su cuenta)](#principio-de-diseño--guardias-monótonas-un-chequeo-que-solo-puede-rechazar-nunca-aprobar-por-su-cuenta)
- [Convención de documentación — transparencia real vs. simulado en la verificación de cada fix](#convención-de-documentación--transparencia-real-vs-simulado-en-la-verificación-de-cada-fix)

## Principio de diseño — guardias monótonas (un chequeo que solo puede rechazar, nunca aprobar por su cuenta)

> Convención de diseño, no ligada a una fase — agregada retroactivamente para nombrar un patrón que ya existía 3 veces en el código sin haber sido formalizado. Cualquier chequeo de seguridad/integridad nuevo (staleness, permisos, validación de identidad, rate-limiting, lo que sea) debe evaluarse contra esta forma antes de escribirse.

**La regla**: un chequeo de este tipo tiene exactamente 2 salidas posibles — `reject` (con motivo explícito) o `no-opinion` (deja pasar SOLO porque no encontró nada que rechazar, nunca porque decidió que todo está bien). **`no-opinion` nunca es sinónimo de `approved`** — la aprobación real viene siempre de otro lado (una aprobación humana explícita vía `confirm()`/`resolveApproval()`, un flag de modo explícito, una decisión de otro componente), nunca del propio chequeo. La consecuencia práctica de esto: el chequeo puede tener bugs, quedar deshabilitado, no cubrir un caso, o directamente no correr — y el peor caso posible sigue siendo "no bloqueó algo que debería haber bloqueado" (mismo riesgo que si el chequeo no existiera), nunca "aprobó activamente algo que un humano no aprobó". Invertido (un chequeo que puede devolver `approved` por su cuenta cuando no encuentra motivo de rechazo) colapsa esa garantía: un bug en el chequeo, o un caso no cubierto, se convierte en una aprobación real y silenciosa de algo que nadie autorizó.

**Los 3 ejemplos reales ya existentes, sin haber sido nombrados como este patrón hasta ahora**:
- **Staleness de `write_file`/`apply_patch`** (`docs/_arch/CONTRACT.md` → "Fix real — detección de staleness en `write_file`/`apply_patch`" y su extensión de `d3a8fe4`/Hallazgo 1): el chequeo de hash SOLO puede devolver `{ok:false, ...}` (rechazo real, con mensaje explícito) o dejar seguir el flujo hacia `resolveApproval()`/`ctx.confirm()` — la aprobación real. El chequeo nunca decide "está todo bien, escribo" por su cuenta; en el peor caso (carrera pura sin gap, documentada honestamente como residual conocido) simplemente no detecta nada, que es exactamente el mismo comportamiento que existía ANTES del fix — nunca peor.
- **Staleness de `revert_file`** (mismo patrón, ver "Fix real — staleness en `revert_file`"): mismo re-chequeo de hash justo antes de `writeFileSync()`, misma forma — rechazo explícito con motivo, o silencio (deja proceder hacia la escritura ya aprobada).
- **TOCTOU con `sessionFileHashes`** (Hallazgo 1, "Fix real — TOCTOU real de `write_file`/`apply_patch` cerrado"): el chequeo adicional por sesión es estrictamente aditivo sobre el chequeo existente — sin registro (archivo nunca leído en esa sesión), no bloquea nada, comportamiento sin cambios; con registro y hash distinto, rechaza. Nunca aprueba algo que el chequeo anterior ya hubiera rechazado, ni sustituye la aprobación real de `resolveApproval()`.

**Cómo verificar que un chequeo nuevo cumple el patrón**: preguntarse "si este chequeo se comentara por completo (no corriera nunca), ¿el sistema queda en el mismo estado de riesgo que tenía antes de que el chequeo existiera, o peor?". Si la respuesta es "el mismo" (o mejor, si de paso se cierra algo previamente abierto), el chequeo es una guardia monótona correcta. Si la respuesta es "peor" (por ejemplo, porque el resto del flujo asumía que pasar por ese chequeo YA es la aprobación, y sin él escribiría de todos modos con menos fricción de la que tenía antes), la lógica está invertida.

## Convención de documentación — transparencia real vs. simulado en la verificación de cada fix

> Convención hacia adelante — no se reescriben las entradas de `CONTRACT.md`/`HISTORY.md` ya existentes para ajustarse a esto. Motivada por la 3ra revisión externa (`docs/_arch/PENDING.md`, nota ya cerrada): hoy esta distinción vive en el texto de cada entrada, pero de forma no sistemática — a veces explícita ("nunca una credencial real"), a veces implícita, a veces ausente.

**De ahora en más, la sección de verificación de cada fix documentada (`CONTRACT.md`, y el resumen correspondiente en `HISTORY.md`) debe distinguir explícitamente**, para cada pieza de la verificación:
- **100% real**: binario/proceso/servidor real efectivamente ejecutado (ej. el instalador `.exe` real generado por `npm run dist` y corrido de punta a punta; una credencial real del usuario, ya decrypteada en memoria por la app, nunca ingresada por el agente; el compilador vendorizado real de NSIS/electron-builder).
- **Stand-in controlable**: un mock/harness/servidor local que emula la forma real de una API o protocolo, sin ser el servicio real (ej. el servidor HTTP local que emula la Anthropic Messages API o Chat Completions en varios fixes de compactación; un `FakeCodexClient`/`HeldCliRuntime` cuando no hay forma de tener el proceso real en un harness aislado sin credenciales).

**No hace falta una etiqueta formal ni una tabla nueva** — alcanza con que la prosa de la verificación sea explícita sobre cuál es cuál, en vez de dejarlo implícito o mezclado (ej.: "servidor Chat-Completions real y local, mismo modelo configurado" ya cumple esto — deja claro que "real" se refiere a la forma del protocolo servida por infraestructura de prueba, no a un servicio externo real). El criterio para decidir si algo cuenta como "real": ¿es el mismo binario/proceso/protocolo que corre en producción, ejecutado de verdad? Si la respuesta es "sí, pero apuntando a un destino de prueba" (ej. una carpeta aislada en vez de `D:\AMATISTA\data`, o `%TEMP%` en vez del install real del usuario), sigue siendo real — el destino está aislado, no el mecanismo. Si la respuesta es "no, algo escrito para esta verificación imita la forma de lo real", es un stand-in — y debe decirse así, sin dejar que la palabra "real" (usada en el resto de la entrada para el mecanismo bajo prueba) se filtre implícitamente sobre esa pieza.
...
2. **`PATH` roto + `APPDATA` real (donde `claude.cmd` SÍ existe de verdad en esta máquina):** el fallback lo encontró y ejecutó correctamente — `detail: "claude disponible (resuelto via C:\Users\<usuario>\AppData\Roaming\npm\claude.cmd — no se encontro por PATH)."`, versión real recuperada (`2.1.241 (Claude Code)`). La ruta candidata construida coincidió EXACTO con la ruta real del shim en esta máquina.
...
## Fix real — señal de cancelación uniforme para el origen de `parallel_ask` (`turnAbortSignal`)

Cierra el gap residual anotado en `docs/_arch/PENDING.md` desde el fix del Hallazgo 1: `runParallelAsk()` leía `session.currentTurnAbort?.signal` del panel de ORIGEN para disparar `onOriginAbort()` → cascada de cancelación a los hijos — `currentTurnAbort` **solo existe en el branch API**, así que si el origen fuera CLI/Codex, cancelar su turno no dispararía la cascada. Investigación previa (`docs/_arch/verify_origin_signal_design.md`) confirmó, no buscado, que ese escenario **no es alcanzable hoy por ningún camino de código real**: `parallel_ask` solo está wireado dentro de `ExecuteContext` para los 4 kinds de `ApiAgentRuntime` (`ipc-agent.ts`) — `cli-agent-runtime.ts`/`codex-client.ts` nunca importan `tool-registry.ts`, y el único puente MCP real hacia CLI/Codex (`mcp-lsp-server.ts`) expone solo tools de LSP. El fix generaliza igual, sin urgencia de bug activo — deja la base lista, consistente con la dirección ya establecida por el fix del Hallazgo 1 (unificar comportamiento entre los 3 runtimes).

**Fix, 5 puntos, los 5 mecánicos**: (1) campo nuevo `turnAbortSignal: AbortController | null` en `SessionRuntimeState` (`runtime-state.ts`), default `null`. (2) `runTurnForWindow()` (`ipc-agent.ts`): `session.turnAbortSignal = new AbortController()` creado SIEMPRE (no solo API), en el mismo punto que `turnInFlight = true` — antes de bifurcar por runtime; limpiado a `null` en el mismo `finally` que ya limpia `turnInFlight`/`cancelCurrentTurn`. (3) `cancelSessionTurn()` (`runtime-state.ts`): `session.turnAbortSignal?.abort()` justo después del guard `if (!cancelled) return false` — dispara sin importar cuál de los 2 branches (API/CLI-Codex) canceló de verdad. (4) `disconnectSession()` (`runtime-state.ts`): mismo disparo, junto a la lógica de cancelación DUPLICADA que ya tenía esta función (no pasa por `cancelSessionTurn()`) — 2do lugar real que cancela un turno en vuelo, confirmado en la investigación que hacía falta ahí también; limpiado a `null` en su propio bloque de reset. (5) `ipc-agent.ts:820` (closure `runParallelAsk` de `ExecuteContext`): `runParallelAsk(assignments, session.turnAbortSignal?.signal)` en vez de `session.currentTurnAbort?.signal` — único punto de consumo, `parallel-orchestrator.ts` sin ningún cambio (ya era agnóstico de dónde viene la señal).

**Decisión explícita: NO fusionado con `currentTurnAbort`** — responsabilidades distintas (mismo criterio de "desconflación" ya aplicado entre `turnInFlight`/`currentTurnAbort` en el fix del Hallazgo 1): `currentTurnAbort` sigue siendo el mecanismo REAL que `ApiAgentRuntime.send()` usa para abortar el `fetch` en vuelo; `turnAbortSignal` es puramente observacional, para que código externo (`runParallelAsk`, o cualquier feature futura con el mismo patrón "cascada al cancelar el origen") sepa que el turno se canceló sin importar qué runtime lo corría.

### Verificación real

Harness bundleado standalone con esbuild (mismo patrón ya usado para el fix de TOCTOU), `AMATISTA_STORAGE_ROOT` aislado — código real de `runtime-state.ts`/`ipc-agent.ts`/`parallel-orchestrator.ts`, sin mocks de la lógica bajo prueba (solo `electron` stubbeado — `BrowserWindow`/`ipcMain`/`app`/`dialog`/`safeStorage`, ninguno invocado de verdad en el escenario). 25 aserciones reales, las 25 en verde:

- **(a) No-regresión, origen API**: `runTurnForWindow()` real con un `apiRuntime.send()` fake (hangs hasta que se aborta, igual que una llamada HTTP real esperando cancelación) — cancelado con `cancelSessionTurn()` real, el turno termina `cancelled:true` igual que antes; `turnAbortSignal` se crea ANTES de bifurcar (confirmado leyéndolo mid-turno) y se limpia a `null` en el mismo `finally` que `turnInFlight`.
- **Cascada real end-to-end** (el caso (a) más exigente): origen y un panel hijo, ambos con `apiRuntime` fake — el fake del ORIGEN llama a `runParallelAsk()` REAL (mismo código, sin tocar) pasándole `session.turnAbortSignal.signal` (el mismo valor real que `ipc-agent.ts` ahora pasa); el fake del HIJO cuelga hasta que lo cancelan. Se cancela el origen con `cancelSessionTurn()` real → el hijo real termina cancelado (turno real disparado por `runParallelAsk`, no simulado) → `runParallelAsk` resuelve → el origen termina `cancelled:true` — cascada real confirmada de punta a punta, disparada por `turnAbortSignal`.
- **(b) Uniforme para los 3 runtimes**: `cancelSessionTurn()`/`disconnectSession()` (ambas reales, sin tocar su lógica existente) ejercitadas contra sesiones shaped como API (`currentTurnAbort` seteado) y como CLI/Codex (`cancelCurrentTurn` seteado, `currentTurnAbort` null) — `turnAbortSignal` se dispara en los 2 casos, en los 2 lugares (`cancelSessionTurn()` y `disconnectSession()`), confirmando que la señal es genuinamente uniforme y queda lista para cuando `parallel_ask` se conecte a CLI/Codex.

`npm run typecheck`/`npm run build` en verde. Harness (`src/main/__verify_origin_signal.ts`, stub de electron) borrado al terminar — `git diff` final contiene únicamente `runtime-state.ts`/`ipc-agent.ts`, confirmado con `git status`.

Archivos: `src/main/runtime-state.ts`, `src/main/ipc-agent.ts`. `parallel-orchestrator.ts` sin tocar, confirmado innecesario. Sin commit — pendiente de que el usuario lo pida.

## Feature real — LSP para Deno con detección condicional por `deno.json` (cierra el último candidato LSP pendiente)

Cierra `docs/_arch/PENDING.md` → "10 candidatos LSP investigados": Deno funcionaba perfecto aislado (handshake real, pull-diagnostics reales, autoregulación real por presencia de `deno.json`, confirmado en `docs/_arch/verify_lsp_10_remaining.md`) pero colisionaba de extensión (`.ts`/`.js`) con `typescript-language-server`/`eslint-language-server` ya en producción — `languageServerConfigFor()` resolvía un solo config por extensión, sin ningún mecanismo para arbitrar "Deno solo si hay `deno.json`". Diseño previo (`docs/_arch/verify_deno_conditional_detection_design.md`) confirmó que `workspace` ya llega a la función en el único caller que importa (`LspManager`, 8 call sites reales) y que no hay ningún caché de resultado que romper (`mergedLanguageServers()` se reconstruye desde cero en cada llamada).

**Fix, genérico, no Deno-específico en la lógica de resolución**: campo nuevo `LanguageServerConfig.workspaceMarker?: string[]` — una entrada con este campo solo es ELEGIBLE si al menos uno de esos archivos existe en la raíz del workspace (`isConfigEligible()`, nueva, `existsSync` ya importado). `languageServerConfigFor()` extiende su `.find()` con ese chequeo. Entrada nueva `{languageId:'deno', extensions:['.ts','.tsx','.js','.jsx'], workspaceMarker:['deno.json','deno.jsonc'], resolveCommand: denoResolveCommand}` (mismo patrón `respondsToVersion('deno', ['--version'])` → `['deno', 'lsp']` que gleam/clojure-lsp) insertada ANTES de `typescript`/`javascript` en `LANGUAGE_SERVERS` — sin `deno.json`, nunca es elegible, `.find()` la salta y cae exactamente en el comportamiento de hoy. Clave de tabla (`languageId:'deno'`) distinta de `'typescript'`/`'javascript'` — coexisten en el `Map`, no se reemplazan; desactivable a mano vía `{"deno":{"disabled":true}}` en `lsp.json`, mismo mecanismo ya existente para cualquier lenguaje. `languageIdFor()` (usado por `notifyFileChanged()` para el campo `languageId` del `didOpen`) confirmado que NO necesita ningún cambio: siempre resuelve `'typescript'`/`'javascript'` (nunca `'deno'`, ya que se llama sin `workspace`) — coincide exactamente con lo que el protocolo LSP de Deno espera ahí (languageId estándar, nunca un valor custom `'deno'`), confirmado en la verificación real de abajo.

### Verificación real — 3 casos, con Deno real (binario 2.9.6 real descargado de GitHub Releases, mismo que la investigación previa)

`npm run typecheck`/`npm run build` en verde. Harness bundleado standalone con esbuild, código real de `LspManager`/`languageServerConfigFor` sin mocks, 3 workspaces reales aislados en scratchpad — 15/15 aserciones en verde:

- **Caso 1 (sin `deno.json`, no-regresión)**: `.ts` con un error de tipos real → `languageServerConfigFor()` resuelve `'typescript'`; `LspManager` arranca `typescript-language-server` real (`isRunning('typescript')`), `deno` nunca arranca; diagnóstico real recibido: `{"message":"Type 'string' is not assignable to type 'number'.", "code":2322, "source":"typescript"}`.
- **Caso 2 (con `deno.json` real)**: MISMO tipo de archivo `.ts`, mismo error → `languageServerConfigFor()` resuelve `'deno'`; `LspManager` arranca `deno lsp` real (`isRunning('deno')`), `typescript-language-server` nunca arranca; diagnóstico real recibido (pull-diagnostics): `{"message":"Type 'string' is not assignable to type 'number'.", "code":2322, "source":"deno-ts"}` — mismo error, **`source` distinto** (`deno-ts` vs `typescript`), prueba inequívoca de que fue Deno el que realmente respondió.
- **Caso 3 (ESLint sin verse afectado)**: workspace con `eslint` real instalado (aislado, sin `deno.json`) — `.js` con variable sin usar → `languageServerConfigFor()` resuelve `'javascript'` igual que siempre; `eslint-language-server` real arranca, `deno` nunca arranca; diagnóstico real recibido (pull-diagnostics): `{"message":"'unusedVariable' is assigned a value but never used.", "code":"no-unused-vars", "source":"eslint"}`.
...
## Fix real — validación en el origen de la carpeta de datos del instalador, cierra el Hallazgo 1 de la 4ta revisión externa

Cambio de dirección real del usuario respecto al diseño original (`docs/_arch/verify_installer_data_preservation_design.md`, exclusión post-hoc en el desinstalador): en vez de mitigar el riesgo después (lógica de exclusión real en `customRemoveFiles`, con los 2 casos reales distintos que esa investigación identificó — coincidencia exacta vs. subcarpeta), **eliminar el estado peligroso en el origen** — el checkbox "usar la misma carpeta que la instalación" (feature de hace 2 sesiones) se retira por completo, y la página real de `nsDialogs` valida activamente, al intentar avanzar, que la carpeta de datos elegida (a mano o vía "Examinar") nunca sea `$INSTDIR` ni una subcarpeta real suya — si lo es, rechaza con un mensaje claro y mantiene al usuario en la misma página. Con esto, `RMDir /r $INSTDIR` (el desinstalador estándar de electron-builder, sin ninguna excepción, confirmado real en la investigación previa) nunca puede tener datos reales del usuario para borrar — ni en un desinstalar manual ni en el flujo silencioso de actualización (`uninstallOldVersion`) — **cero lógica nueva en el desinstalador**, el Hallazgo 1 queda cerrado sin tocar `uninstaller.nsh` en absoluto.

**Mecanismo real de comparación de rutas en NSIS**, mismo principio que `isWithinFolder()` (TypeScript, fix de la 3ra revisión externa: comparación ingenua de substring falla, Windows es case-insensitive para rutas): `${StrCase}` (real, de `StrFunc.nsh`, bundlada con el NSIS vendorizado de este proyecto — internamente usa `User32::CharUpper` de la API real de Windows, no una reimplementación propia) normaliza ambas rutas a mayúsculas antes de comparar. Igualdad exacta → rechazo directo. Subcarpeta real → prefijo con el separador `\` incluido a propósito (evita el falso positivo real que motivó el fix de TypeScript: `"D:\AMATISTA\APP"` NUNCA debe matchear como prefijo de `"D:\AMATISTA\APPEXTRA"`, una carpeta HERMANA, no una subcarpeta).

**Bug real encontrado y corregido durante el primer build real** (mismo patrón de scoping ya conocido en este archivo): `${StrCase}` (su declaración real vía `!include "StrFunc.nsh"` + la línea bare `${StrCase}`) generaba la `Function StrCase` sin guardar bajo `!ifndef BUILD_UNINSTALLER` — en la pasada del desinstalador (2da compilación real de `installer.nsh`, confirmado que este archivo se compila 2 veces) esa función nunca se invoca (el único uso real está dentro de `DataFolderPageLeave`, ya guardada), así que NSIS la marca "not referenced", que electron-builder trata como error fatal (`makensis.exe process failed`). Fix: la declaración de `${StrCase}` se movió adentro del mismo bloque `!ifndef BUILD_UNINSTALLER` que ya protege `DataFolderDialog`/`DataFolderText`/`DataFolder`.

### Verificación real — 4 casos, instalador completo generado con `npm run dist`/`electron-builder --win nsis`

Instalador real (`Amatista-V0.11.0-Setup.exe`, ~184.8MB, firmado), ejecutado de verdad con `computer-use` (doble-click, tipeo real):

1. **Carpeta de datos = `$INSTDIR` exacto** → rechazado real: *"La carpeta de datos no puede ser la misma que la carpeta de instalacion -- elegi una ubicacion distinta."* — la página no avanzó, el usuario permanece en el mismo campo para corregir.
2. **Subcarpeta real dentro de `$INSTDIR`** (`$INSTDIR\data`) → rechazado real, mensaje DISTINTO: *"La carpeta de datos no puede estar dentro de la carpeta de instalacion -- elegi una ubicacion distinta."*
3. **Carpeta real fuera de `$INSTDIR`** → aceptado sin ningún bloqueo, avanzó directo a "Instalando" — idéntico al comportamiento de ayer. Instalación real completa hasta el final, registro real confirmado: `HKCU\Environment\AMATISTA_STORAGE_ROOT` = la ruta elegida, exacta.
4. **Mismo caso peligroso (Caso 1) con mayúsculas/minúsculas reales mezcladas** (`c:\USERS\<usuario>\appdata\Local\TEMP\Amatista-Hallazgo1-Test\APP` contra un `$INSTDIR` tipeado con casing distinto) → **detectado igual**, mismo rechazo — confirma que `${StrCase}` normaliza correctamente antes de comparar.

Limpieza real: desinstalador del Caso 3 corrido (`/currentuser /S`), carpeta de prueba borrada, `AMATISTA_STORAGE_ROOT` borrado del registro real para restaurar el estado exacto pre-test (confirmado que no existía antes). `npm run typecheck` en verde (sin cambios de TS, confirmado innecesario, corrido igual). Build de NSIS real limpio (exit 0) tras el fix del bug de scoping.

Archivos: `build/installer.nsh` (único archivo tocado — checkbox y su lógica retirados, `${StrCase}` agregada, `DataFolderPageLeave` con la validación real nueva). Ningún archivo TypeScript tocado. `uninstaller.nsh`/`node_modules` sin ningún cambio, a propósito. Sin commit — pendiente de que el usuario lo pida.

## Fix real — `runInRepoQueue()`, la cola de git pasa a envolver la operación lógica completa, cierra el Hallazgo 4 de la 4ta revisión externa

Basado en el diseño ya revisado en `docs/_arch/verify_git_atomic_operation_design.md`. La cola por-repo introducida en el fix de TOCTOU (`repoQueues: Map<string, Promise<unknown>>`, ver arriba "Cola de ejecución por-repo + timeout real en `runGit()`") serializaba **cada llamada individual a `runGit()`**, no la operación lógica completa que un llamador considera atómica — `commitPath()` hacía `await runGit(add)` seguido de `await runGit(commit)`, y el `await` entre ambas cedía el event loop: si otro `commitPath()` concurrente invocaba su propio `add` en esa ventana, quedaba encolado ANTES del `commit` del primero. Confirmado real con git real (Hallazgo 4 original): 10 escrituras concurrentes al mismo repo oculto produjeron el orden real `[add×10, commit×1, "nothing to commit"×9]` — 1 solo commit real, mal atribuido, 9 escrituras perdidas de la cola de commits (el contenido en disco era correcto, solo el respaldo VCS colapsaba).

**Fix real**: `runInRepoQueue<T>(cwd, task: () => Promise<T>): Promise<T>` nueva — mismo tail-chaining de promesas que ya existía (mismo `Map<string, Promise<unknown>>`, mismo `.catch(() => undefined)` para que un rechazo no rompa la cadena para el siguiente), generalizado para encolar una función arbitraria en vez de un `execFileAsync` puntual. `runGit()` deja de tocar `repoQueues` — pasa a ser el primitivo puro de ejecución (`execFileAsync` + `GIT_TIMEOUT_MS`, sin ninguna noción de cola propia). `commitPath()` queda envuelta ENTERA en `runInRepoQueue()` — el `add`+`commit` pasan a ser una sola unidad indivisible en la cola, ninguna otra operación del mismo repo puede intercalarse en el medio sin importar cuánto tarde el subproceso real de `git commit`. Los otros 3 call sites reales (`ensureVcsRepo()` ×3, `listHistoryInRepo()`, `readFileVersion()`) migrados al mismo primitivo, sin cambio de comportamiento real — seguían siendo, de hecho, su propia "unidad" al ser una sola llamada cada uno; solo pasan a usar el mecanismo genérico en vez de que `runGit()` lo hiciera por su cuenta. `GIT_TIMEOUT_MS` (15s) sin ningún cambio, sigue siendo el timeout de cada subproceso real dentro de `runGit()`, independiente de la agrupación de la cola.

**Por qué no alcanzaba con agregar `runInRepoQueue()` como capa extra dejando `runGit()` como estaba** (razón de diseño, confirmada antes de implementar): 2 capas de encadenamiento compitiendo por la MISMA entrada del `Map` no da atomicidad real — el `add` de B seguiría pudiendo intercalarse DENTRO de la ventana que el wrapper externo cree estar reservando, porque el `runGit()` interno de A seguiría liberando y re-tomando la cola en cada una de sus 2 llamadas. La única forma correcta es que una sola capa sea la única responsable de la sincronización.

**Verificación real — 3 casos, git real, subprocesos reales, cero mocks de la lógica bajo prueba** (harness temporal `src/main/__verify_git_atomic.ts`, bundleado standalone con esbuild, borrado al terminar):

1. **Mismo escenario exacto que expuso el Hallazgo 4** (10 escrituras concurrentes reales al mismo repo oculto, vía `commitVersion()` real): `git log --stat` real mostró 11 commits (1 seed + 10 reales, cada uno con su propio archivo — `write_file_test_0` … `write_file_test_9`, sin ningún "nothing to commit" de por medio). 10/10 archivos con historial real vía `listFileHistory()`, 10 atribuciones de herramienta distintas (una por archivo) — **cero commits colapsados, cero atribución incorrecta**. Confirma que el fix cerró el bug real.
2. **No-regresión — timeout real de git colgado** (mismo mecanismo que `guard/`: hook `.git/hooks/pre-commit` real con `sleep 30`): `commitVersion()` real devolvió `{ok:false}` en 15245ms (no 30000ms+, nunca colgado para siempre) — el `GIT_TIMEOUT_MS` de 15s sigue cortando exactamente igual con la nueva agrupación. Con el hook retirado, la siguiente operación real sobre el MISMO repo devolvió `{ok:true}` — la cola se recupera para el siguiente llamador, mismo comportamiento resiliente de siempre.
3. **Repos DISTINTOS en paralelo real** (2 workspaces reales, 5 escrituras concurrentes cada uno): tiempo real con A+B en paralelo (1790ms) fue MENOR que un solo repo corriendo solo (2057ms) — razón real 0.87x, muy por debajo del ~2x que delataría una cola global — confirma que la cola sigue siendo por-repo, no global. 6 commits reales en cada repo (1 seed + 5), atomicidad real intacta en ambos.

Bug real encontrado y corregido DURANTE la propia verificación, en el harness, no en el código bajo prueba: la primera versión de `findRealRepoDir()` (helper del harness) buscaba "el primer subdirectorio con `.git`" bajo el `vcs/` compartido del `AMATISTA_STORAGE_ROOT` de prueba — con varios workspaces reales corriendo bajo la misma raíz, esto era ambiguo y devolvía el repo EQUIVOCADO para los Casos 2 y 3 (ambos terminaron consultando el repo del Caso 1 por error, confirmado porque sus conteos de commits coincidían exactamente con los del Caso 1). Fix del harness: cada caso deja un `seed.txt` real con contenido único, y `findRealRepoDir()` identifica el repo correcto consultando `git show HEAD:seed.txt` en cada candidato real.

Archivos: `src/main/local-vcs.ts` (único archivo tocado — `runInRepoQueue()` nueva, `runGit()` simplificada a primitivo puro, los 4 call sites migrados). `npm run typecheck` y `npm run build` en verde. Harness de verificación (`__verify_git_atomic.ts`, `__verify_electron_stub.cjs`, `out/main/__verify_git_atomic.cjs`) borrado por completo al terminar, confirmado con `git status`/`git diff --stat` que solo `local-vcs.ts` queda modificado. Sin commit — pendiente de que el usuario lo pida.

## Fix real — tri-estado `confirmed`/`unavailable`/`inconclusive` en el descubrimiento de Gemini, cierra el Hallazgo 5 (el último) de la 4ta revisión externa

Basado en el diseño ya revisado en `docs/_arch/verify_gemini_inconclusive_states_design.md`. `verifyGeminiModel()` (`gemini-catalog.ts`) usaba `response.ok` puro (solo 2xx) — 404 (retiro genuino), 429/503 (rate-limit/no-disponible transitorio), 403/400 (error de permiso/request) y timeout/error de red quedaban TODOS indistinguibles, descartados en silencio del catálogo como si fueran el mismo 404 real que motivó el diseño original del archivo. Con esto, los 5 hallazgos de la 4ta revisión arquitectónica externa quedan cerrados.

**Fix real**: `verifyGeminiModelOnce()` inspecciona el status real de la respuesta — `200` → `confirmed`; `404` EXACTO → `unavailable` (única señal real y determinista de retiro); cualquier otro no-200 (`429`/`503`/`500`/`403`/`400`/timeout/red) → `inconclusive`, deliberadamente amplio, nunca se afirma "retirado" sin un 404 real. `verifyGeminiModel()` envuelve ese intento único con reintento real acotado a las 3 clases que la documentación oficial de Google marca como reintentables (`429`/`500`/`503` + timeout/red, `RETRYABLE_STATUSES`) — hasta `MAX_RETRIES=2` reintentos (3 intentos totales), backoff real creciente (`RETRY_BACKOFF_MS=[800,2000]`ms). `403`/`400` (u otro no-retryable) nunca se reintentan — el mismo body idéntico no puede dar un resultado distinto la 2da vez. `listGeminiModels()` pasa de devolver `GeminiCatalogModel[]` a `GeminiCatalogResult` (`{confirmed, unavailableCount, inconclusive}`) — los inconclusos que sobreviven al reintento se reportan aparte, con su `status` real, nunca se agregan al catálogo ni se descartan en silencio. `GEMINI_VERIFY_CONCURRENCY` sin ningún cambio (5) — sin evidencia real medida de que el paralelismo sea el driver, el backoff-por-candidato es la mitigación correcta (solo paga costo cuando hubo error real, sin enlentecer toda sincronización). `syncGemini()` (`App.tsx`) arma el mensaje tri-estado en el feedback local a la fila: `"X confirmados, Y retirados, Z sin verificar por limite de cuota -- reintenta mas tarde."` cuando hay inconclusos, sin la 3ra cláusula en el caso común (0 inconclusos). `discoverNewClaudeModels()` (mismo patrón booleano, `model-discovery.ts`) sin tocar — fuera de alcance de este hallazgo, específico de Gemini.

**Verificación real — 4 casos con un servidor HTTP local real controlable** (harness temporal `src/main/__verify_gemini_states.ts`, bundleado standalone con esbuild, cero cuota real gastada para esta parte — `GEMINI_API_ROOT` con un override temporal vía env var, retirado al terminar, confirmado con `git diff` que `gemini-catalog.ts` no arrastra ningún rastro del override):

1. **200 real → `confirmed`**: candidato `test-confirmed` apareció en `result.confirmed`.
2. **404 real → `unavailable`**: candidato `test-404-retirado` contado en `result.unavailableCount` (1), nunca en `confirmed` ni `inconclusive`.
3. **429/503 real → `inconclusive`, reintento con backoff se dispara**: `test-429-rescatado` (1er intento 429, 2do 200) terminó en `confirmed` — 2 requests reales confirmados, gap real entre intentos de 815ms (backoff de 800ms funcionando). `test-503-persistente` (siempre 503) agotó los 3 intentos reales (`MAX_RETRIES=2`), terminó en `inconclusive` con `status:503` real.
4. **403/400 real → `inconclusive`, SIN reintento**: `test-403-sin-reintento` y `test-400-sin-reintento`, 1 sola request real cada uno (confirmado que nunca se reintenta un no-retryable).

**Corrida real final contra la conexión Google real del usuario** (autotest temporal en `index.ts`, mismo patrón de siempre — `settings.providers.find(p => p.type === 'google' && p.apiKey)`, key real nunca vista/tipeada por el agente, retirado al terminar, confirmado con `git diff src/main/index.ts` vacío — byte-idéntico): mensaje tri-estado real, `"26 confirmados, 1 retirados, 13 sin verificar por limite de cuota -- reintenta mas tarde."`. **Hallazgo real adicional durante esta corrida** (no un bug, una observación honesta sobre la redacción): de los 13 inconclusos reales, ninguno fue 429 — la mayoría fueron `400` (modelos TTS/audio/computer-use/deep-research/Lyria cuyo request mínimo de texto plano no coincide con la forma real que esos modelos esperan) y 3 fueron `500` (familia Lyria). El texto "sin verificar por limite de cuota" es la redacción pedida por el diseño y sigue siendo la mejor default (no miente sobre retiro, no esconde la ambigüedad), pero en la práctica real la causa más común no fue cuota — anotado para si hiciera falta una redacción más genérica más adelante (no priorizado, la clasificación tri-estado en sí misma es correcta independientemente del texto).

Archivos: `src/main/gemini-catalog.ts` (tri-estado + retry-con-backoff), `src/preload/index.d.ts` (firma de `listGeminiModels()` actualizada), `src/renderer/src/App.tsx` (`syncGemini()` arma el mensaje de 3 partes). `preload/index.ts` sin cambio de forma (passthrough sin tipo). `model-discovery.ts`/`ipc-gemini-catalog.ts` sin tocar. `GEMINI_VERIFY_CONCURRENCY` sin cambio. `npm run typecheck`/`npm run build` en verde. Harness (`__verify_gemini_states.ts`, `__verify_electron_stub.cjs`, `out/main/__verify_gemini_states.cjs`) y el autotest temporal de `index.ts` retirados por completo. Sin commit — pendiente de que el usuario lo pida.

## Infraestructura real de tests de regresión (`tests/regression/`) — 8 harnesses reales conservados como tests versionados

Basado en `docs/_arch/verify_regression_test_infra_design.md`, ya confirmado. Decenas de harnesses reales de esta sesión (bundle esbuild + código de producción real sin mocks + storage aislado) se armaban y se borraban siempre al terminar de verificar cada fix — sugerencia real de la revisión externa: conservar algunos como tests de regresión reales, versionados, para detectar automáticamente si algo se rompe en el futuro.

**Mecanismo**: `node:test` nativo (Node instalado ≥22.12.0, cero dependencia nueva) + `esbuild` (ya transitivo vía `vite`/`electron-vite`, el mismo binario de cada harness de esta sesión). `npm run test:regression` — un solo comando: bundlea los 8 `tests/regression/*.test.ts` con `--alias:electron=./tests/regression/_support/electron-stub.cjs` (stub real versionado, con `ipcMain.handle()`/`.on()` capturando handlers reales en `__handlers`/`__listeners` para que un test pueda invocar DIRECTO un handler IPC real sin ventana ni evento reales) a `tests/regression/.dist/` (gitignorado), y corre `node tests/regression/_support/run.cjs`.

**Hallazgo real durante la propia construcción de esta infraestructura, no en el código bajo prueba** — el más serio de toda la investigación: setear `process.env.AMATISTA_STORAGE_ROOT` DENTRO de un archivo de test, antes de su propio `import` a un módulo de producción, **no alcanza** — los `import` (incluso transpilados a `require()` por esbuild) se evalúan antes que el resto del cuerpo del módulo, sin importar el orden textual (mismo patrón exacto que ya había mordido a esta sesión una vez, con `AMATISTA_VERIFY_GEMINI_API_ROOT` en el harness del Hallazgo 5). Confirmado real y grave: 2 corridas de prueba reales escribieron repos VCS ocultos de prueba (`amatista-regression-git-atomic-ws-*`, `amatista-regression-toctou-ws-*`) dentro de `D:\AMATISTA\data\vcs\` — la carpeta REAL del usuario, no un aislamiento — detectado, confirmado que `amatista.db` (base de datos real de chats) nunca fue escrita (mismo tamaño/mtime antes y después, solo se le hizo una `SELECT` de lectura con un `chatId` sintético inexistente), y las 2 carpetas de prueba borradas a mano. **Fix real**: `tests/regression/_support/run.cjs`, wrapper que crea un `AMATISTA_STORAGE_ROOT` real aislado (`mkdtempSync`) en el PROCESO PADRE antes de lanzar `node --test` (`spawnSync` con `env` explícito) — cada archivo de test (confirmado real que `node --test` aísla cada archivo en su propio subproceso, hereda el env var correctamente) arranca ya con el storage root aislado desde el primer `require()`. Los 4 archivos de test afectados (TOCTOU/git-atomic/parallel-ask-identity/session-registry-bound) se corrigieron para depender del wrapper en vez de auto-setearse.

**Los 8 candidatos** (criterio real: qué tan sutil sería el bug si alguien lo rompe sin querer, tocando código cercano, sin que ningún typecheck lo detecte): TOCTOU de `write_file` bajo concurrencia real (`toctou-write-file.test.ts` — 2 escritores reales con demoras de aprobación asimétricas, mismo criterio que la verificación original), `runInRepoQueue()`/atomicidad de git (`git-atomic-queue.test.ts` — 10 escrituras concurrentes reales, mismo escenario exacto del Hallazgo 4), `resolveJsonSchemaRefs()` (`gemini-ref-resolution.test.ts` — $ref/$defs reales + ciclo real A↔B), guard de identidad de `parallel_ask` (`parallel-ask-identity.test.ts` — `sessionRegistry` real mutado para simular reconexión), `isWithinFolder()` (`is-within-folder.test.ts`), `runtimeFor()` (`runtime-for.test.ts` — función pura, ya causó una regresión silenciosa real una vez con OpenRouter), tri-estado de Gemini (`gemini-tri-state.test.ts` — intercepta `globalThis.fetch` real, sin servidor HTTP ni seam de testeabilidad permanente en `gemini-catalog.ts`), y límite de `sessionRegistry` (`session-registry-bound.test.ts` — invoca el handler REAL `agent:disconnect` capturado del stub).

**Verificación real, 12/12 aserciones en verde** (`npm run test:regression`), y validación real de que 3 de los 8 tests protegen algo genuino (no solo que existen): revertidos temporalmente uno a la vez (`geminiFunctionDeclarations()` sin `resolveJsonSchemaRefs()`, `commitPath()` con `add`/`commit` como 2 entradas de cola independientes, el re-chequeo de staleness de `write_file` desactivado con `if (false && ...)`) — los 3 tests fallaron real, con el mensaje exacto del bug original reproducido (`{}` vacío / 2 commits en vez de 11 / clobber silencioso con `ok:true`) — restaurados a byte-idéntico (`git diff --stat` vacío en los 3 archivos), confirmado que vuelven a pasar. `npm run typecheck`/`npm run build` (la app normal) sin ningún cambio de comportamiento — `tests/` fuera de los `include` de ambos `tsconfig.*.json`, fuera del `files` allowlist de `electron-builder` (confirmado leyendo `package.json`, sin ningún wildcard que arrastre el repo completo).

Archivos: `tests/regression/*.test.ts` (8 nuevos), `tests/regression/_support/electron-stub.cjs` (nuevo), `tests/regression/_support/run.cjs` (nuevo), `package.json` (script `test:regression` nuevo), `.gitignore` (`tests/regression/.dist/`). Ningún archivo de producción modificado de forma permanente — los 3 reverts temporales de validación se restauraron a byte-idéntico. Sin commit — pendiente de que el usuario lo pida.

## Fix real — 2 hallazgos de una prueba en vivo del usuario: watchdog nunca cancelaba el turno real en main, `MAX_TOOL_LOOP` fijo sin poder configurarlo por Settings

Basado en investigación previa de solo lectura (sin código tocado en esa pasada), confirmada por el usuario tras terminar su prueba en vivo con DeepSeek.

### Fix 1 — el watchdog del renderer ahora cancela el turno real en main

Bug real confirmado: el `setTimeout` del watchdog (`App.tsx`, `startTurnWatch()`) solo limpiaba estado LOCAL del renderer (`clearTurnWatch()`) y mostraba *"el turno se cerró"* — nunca llamaba a la cancelación real (`cancelAgent()`/`agent:cancel`/`cancelSessionTurn()`) que sí limpia `turnInFlight` en main (`ipc-agent.ts`). Resultado real vivido por el usuario: el mensaje mentía sobre el estado real — `turnInFlight` seguía `true` hasta que la llamada real (HTTP/CLI) resolvía por su cuenta, y un reintento inmediato chocaba con `"Ya hay un turno en vuelo en este panel"`.

**Fix**: `void cancelAgent()` agregado al callback del watchdog (`App.tsx`, dentro de `startTurnWatch()`), disparado ANTES del mensaje visible — mismo call real que ya usa el botón "Detener" (`agent:cancel` → `cancelSessionTurn()`), fire-and-forget (no hay nada más que esperar ahí, el mensaje se muestra igual). El mensaje "el turno se cerró" ahora es honesto en los 2 lados: cuando dispara, se cierra de verdad en main también.

### Fix 2 — `MAX_TOOL_LOOP` configurable por Settings (mismo patrón que `turnWatchdogSeconds`)

Antes: `MAX_TOOL_LOOP` (`api-agent-runtime.ts`) era un `const` de módulo fijo en 60 (o `AMATISTA_MAX_TOOL_LOOP`, diseñado específicamente para el harness del benchmark, nunca expuesto a un usuario con la app instalada — confirmado con grep, cero wiring en Settings/`settings-store.ts`/`shared/types.ts` antes de este fix). Real y no trivial confirmar antes de arreglar: un turno real de build/restore de un proyecto grande (turnos 57-59: `git status`, copiar archivos, `dotnet build` — progreso genuino, no un loop repetido que `guard/` debiera haber atajado) se cortó por agotar el límite.

**Fix, mismo patrón exacto que `turnWatchdogSeconds`**: `maxToolLoop?: number` nuevo en `AppSettings` (`shared/types.ts`) y `StoredSettings` (`settings-store.ts`, con su propio guard `validMaxToolLoop()` — entero positivo, mismo criterio que `validTurnWatchdogSeconds()` pero exigiendo además `Number.isInteger`). Agregado a la allowlist explícita de `settings:save` (`ipc-settings.ts`) — sin esto habría repetido el mismo modo de falla real que ya pasó con Tavily/presets (el campo se descarta en silencio, `success:true` igual). Threadeado por `ApiAgentRuntime.configure()` (`ConfigureOptions.maxToolLoop`, `ipc-agent.ts:681` lee `settings.maxToolLoop`) hasta `this.config.maxToolLoop` — **leído fresco en cada turno** (`const maxToolLoop = this.config.maxToolLoop ?? MAX_TOOL_LOOP` al principio de cada uno de los 4 métodos `send*`), nunca cacheado en un `const` de módulo (evita a propósito el mismo bug ya corregido para `TURN_WATCHDOG_MS` congelado). Los 4 `for (let turn = 0; turn < MAX_TOOL_LOOP; turn++)` y sus 4 mensajes de error pasan a usar `maxToolLoop` local. `MAX_TOOL_LOOP`/`AMATISTA_MAX_TOOL_LOOP` sin ningún cambio — sigue siendo el default real cuando `maxToolLoop` no está configurado, el benchmark (que nunca pasa este campo) queda intacto.

**Hallazgo real durante la implementación, no en el código bajo prueba**: la ubicación de UI pedida ("mismo lugar que 'Timeout del watchdog de turno'") ya no existe — confirmado con grep (`type="number"` daba 0 resultados en toda la UI antes de este fix) que la sección "GENERAL" descrita en el fix original de `turnWatchdogSeconds` (Fase 14) fue removida en algún momento posterior (probablemente el rediseño a acordeón de Configuración) sin dejar rastro ni nota en `PENDING.md` — `turnWatchdogSeconds` sigue siendo un campo real persistido y funcional, solo que hoy únicamente editable a mano en `settings.json`, sin ningún control en la UI. Campo nuevo agregado en el lugar temáticamente más cercano que sí existe hoy: dentro de la sección "Herramientas del workspace" (acordeón ya existente), como input numérico simple (`<label className="field">`, mismo patrón visual del resto de Configuración).

### Verificación real — 3 casos, código de producción real, servidores HTTP locales reales, storage aislado

Harness temporal (`__verify_two_fixes.ts` + wrapper plano que fija `AMATISTA_STORAGE_ROOT` antes de requerir el bundle, mismo patrón que `benchmark/run-instance-wrapper.cjs`), llamando DIRECTO a `connectSessionForWindow()`/`runTurnForWindow()`/el handler real de `agent:cancel` (capturado del stub de electron) — sin mockear ninguna lógica de producción:

- **Caso A (Fix 2, corte real en el límite configurado)**: `maxToolLoop:3` real vía `setSettings()`, servidor HTTP real que siempre devuelve una tool-call (nunca texto final) — el turno se cortó en **exactamente 3 requests HTTP reales**, mensaje real `"Se alcanzo el limite de 3 iteraciones..."`.
- **Caso B (Fix 2, no-regresión)**: sin `maxToolLoop` configurado, mismo servidor real — el turno corrió **exactamente 60 requests HTTP reales** antes de cortar, default intacto.
- **Caso C (Fix 1)**: servidor real que acepta la conexión pero nunca responde — turno real disparado sin esperar, confirmado real `turnInFlight===true` mientras el request real seguía en curso (1 request real llegó al servidor colgado). Invocado el handler real de `agent:cancel` (mismo call que ahora dispara el watchdog arreglado) — `turnInFlight` pasó a `false` real de inmediato, el turno colgado resolvió limpio (`{success:true, cancelled:true}`), y un **2do turno inmediato en el mismo panel NO chocó con "Ya hay un turno en vuelo"** — la prueba decisiva de que el fix cierra el bug real vivido.

`npm run typecheck`/`npm run build` en verde. Harness (`__verify_two_fixes.ts`, wrapper, stub de electron dedicado, `out/main/__verify_two_fixes.cjs`) borrado por completo — confirmado real que ningún residuo llegó a `D:\AMATISTA\data` (storage aislado en todo momento) y que los directorios temporales del propio harness quedaron limpiados.

Archivos: `src/renderer/src/App.tsx` (watchdog + campo nuevo de Settings), `src/main/api-agent-runtime.ts` (`ConfigureOptions.maxToolLoop`, 4 loops + 4 mensajes leen `this.config.maxToolLoop ?? MAX_TOOL_LOOP`), `src/main/ipc-agent.ts` (`configure()` pasa `settings.maxToolLoop`), `src/main/settings-store.ts` (`StoredSettings.maxToolLoop` + `validMaxToolLoop()`), `src/main/ipc-settings.ts` (allowlist de `settings:save`), `src/shared/types.ts` (`AppSettings.maxToolLoop`). `guard/` (loop-hygiene) sin ningún cambio, confirmado independiente. Sin commit — pendiente de que el usuario lo pida.

## Feature real — botón "Configurar MarkItDown para Claude Code/Codex" (fusión segura de `.mcp.json` y `~/.codex/config.toml`)

Basado en `docs/_arch/verify_markitdown_config_button_design.md`, ya confirmado. Primer mecanismo real que Amatista usa para ESCRIBIR en cualquiera de los 2 archivos — confirmado en la investigación previa que hasta ahora eran de solo lectura (`.mcp.json`) o ni siquiera se tocaban (`~/.codex/config.toml`, ajeno a Amatista, de Codex).

**`detectDocker()`** (`cli-status.ts`) — 1 línea, mismo patrón exacto que `detectCodex()`/`detectClaude()` (`versionOf('docker')`). Se corre PRIMERO en el handler real — si Docker no está en PATH, corta ahí, no escribe NINGUNO de los 2 archivos.

**`.mcp.json`** — `upsertMcpServer(workspace, name, config)` nueva (`mcp-client.ts`), separada a propósito de `readMcpConfig()` existente (que normaliza cada servidor a un shape angosto, perdiendo campos extra — no sirve para fusión fiel). Lee el objeto JSON crudo completo (o `{}` si el archivo no existe), muta SOLO `mcpServers[name]`, escribe de vuelta — cualquier otro servidor real ya presente (`_ejemplo_borrar_esto` incluido) sobrevive intacto con cualquier campo extra que tuviera. JSON no tiene comentarios que perder, así que el ciclo `parse`→mutar→`stringify` es seguro acá (a diferencia de TOML, ver abajo).

**`~/.codex/config.toml`** (`codex-config-toml.ts`, nuevo) — archivo AJENO real del usuario, potencialmente con comentarios/otras secciones reales. Investigado real contra el registro de npm (`smol-toml@1.8.0`, agregado como dependencia nueva: cero deps en runtime, parser+stringifier TOML 1.1.0) — **confirmado en su propia documentación que NO es format-preserving**, un ciclo `parse`/`stringify` perdería comentarios/formato reales del usuario. Por eso `smol-toml` se usa SOLO para lectura (validar que el archivo parsea antes de tocarlo; detectar si `mcp_servers.markitdown` ya existe, para reportar creado-vs-actualizado) — la escritura real es edición quirúrgica de TEXTO PLANO: `upsertCodexMarkitdownTableText(raw, spec)` (función pura, exportada aparte para testear sin tocar disco) busca `[mcp_servers.markitdown]`, si existe reemplaza SOLO esas líneas hasta el próximo header real (`[...`) o EOF, si no existe hace `append` al final — todo lo demás (comentarios, otras secciones, orden) sobrevive byte a byte. Ruta real: `path.join(os.homedir(), '.codex', 'config.toml')` — sin ningún precedente previo en el proyecto (`homedir()` solo se usaba antes en `model-discovery.ts`, sin relación).

**Orquestación** (`mcp:configureMarkitdown`, `ipc-mcp.ts`) — mismo `callerWorkspace(panelId)` que ya usan `mcp:status`/`mcp:openOrCreate`/`agentsMd:*` (`getSession(panelId).activeWorkspace`). Docker primero → `.mcp.json` real → `config.toml` real, cada uno con su propio try/catch (un fallo en uno no bloquea el otro), mensaje compuesto reportando creado-vs-actualizado o el error real por archivo. **Botón nuevo** en la sección "CLI y cuentas" de Settings (`App.tsx`) — mismo lugar donde ya viven estado/login/instalación de Claude Code y Codex, `busy` propio (`markitdownBusy`, no reusa `authBusy`, mismo criterio de separación que ya documenta esa sección: cuenta/CLI, no catálogo).

### Verificación real — 4 casos, código de producción real, storage/PATH/home aislados

Harness temporal (`__verify_markitdown.ts`), llamando DIRECTO a `upsertMcpServer()`/`upsertCodexMarkitdownTableText()`/`configureCodexMarkitdown()`/`detectDocker()` reales — sin mockear ninguna lógica de producción. Ninguno de los 3 módulos importa `electron` (confirmado real: bundle limpio sin `--alias:electron`).

- **Caso A** (`.mcp.json` real con `_ejemplo_borrar_esto` ya presente): tras `upsertMcpServer()`, ambos servidores reales en el archivo final — `_ejemplo_borrar_esto` intacto, `markitdown` agregado con `command:"docker"` real.
- **Caso B** (`config.toml` real con una sección + un comentario reales puestos a propósito): 1ra escritura agrega `[mcp_servers.markitdown]` preservando el comentario y `[mcp_servers.otro-servidor]` byte a byte; 2da escritura con un spec DISTINTO confirma que **reemplaza** la tabla existente (nunca duplica — `(match(/\[mcp_servers\.markitdown\]/g)).length === 1` real) sin tocar el resto.
- **Caso C** (sin Docker real en PATH — `process.env.PATH` real vaciado a un directorio real sin binarios, NUNCA mockeado `detectDocker()` en sí): `docker no encontrado` real (`ENOENT` real de Windows), confirmado que CERO archivos se escribieron (ni `.mcp.json` ni `config.toml` llegaron a existir).
- **Caso D** (feliz completo, Docker real presente en esta máquina — `Docker version 29.3.1` real): los 2 archivos reales escritos, contenido correcto confirmado parseando el TOML real resultante (no comparando texto crudo — un path de Windows con backslashes se escapa en la escritura, un string-match ingenuo daba un falso negativo real, corregido en el propio harness).

`os.homedir()` parcheado real (mismo patrón ya usado en esta sesión para `process.platform`) para redirigir `codexConfigTomlPath()` a un directorio aislado en los 4 casos — confirmado real que `~/.codex/config.toml` real del usuario nunca se tocó (mismo tamaño/mtime antes y después, cero ocurrencias de `"markitdown"`).

`npm run typecheck`/`npm run build` en verde. Harness borrado por completo.

Archivos: `src/main/cli-status.ts` (`detectDocker()`), `src/main/mcp-client.ts` (`upsertMcpServer()`), `src/main/codex-config-toml.ts` (nuevo), `src/main/ipc-mcp.ts` (`mcp:configureMarkitdown`), `src/preload/index.ts`/`index.d.ts` (`configureMarkitdown()`), `src/renderer/src/App.tsx` (botón + función + estado `markitdownBusy`), `package.json`/`package-lock.json` (`smol-toml@^1.8.0` nueva). `readMcpConfig()` sin ningún cambio. Sin commit — pendiente de que el usuario lo pida.

## Fix real — restaurada la UI de `turnWatchdogSeconds` en Settings (perdida en un rediseño anterior sin dejar rastro)

Motivado por la investigación real de los 2 errores en un turno en vivo con Claude Sonnet 5 del día anterior: `turnWatchdogSeconds` **ya era un setting real, persistido y funcional** (Fase 14, `settings-store.ts` → `validTurnWatchdogSeconds()`, allowlist de `settings:save` en `ipc-settings.ts`, cómputo real en `App.tsx` — `TURN_WATCHDOG_MS = turnWatchdogSeconds * 1000`) — solo había perdido su **control visual** en algún rediseño de Configuración posterior a Fase 14, sin dejar ninguna nota en `PENDING.md` (confirmado el día anterior: `type="number"` daba 0 resultados en toda la UI antes del campo de `maxToolLoop`).

**Fix**: campo nuevo `<label className="field">` en la sección "Herramientas del workspace" (`App.tsx`), mismo lugar y mismo patrón visual exacto que el campo de `maxToolLoop` agregado ayer — inmediatamente después de ese campo. Misma lógica de validación real que `validTurnWatchdogSeconds()` (`settings-store.ts`), reimplementada inline en el `onChange` (mismo criterio que ya se usó para `maxToolLoop`, esa función vive en main, no se importa cruzando el límite de proceso): `Number.isFinite(parsed) && parsed > 0 ? parsed : undefined` — **sin exigir entero** (a diferencia de `maxToolLoop`, que sí lo exige — `turnWatchdogSeconds` siempre aceptó segundos fraccionarios, mismo criterio real que el guard de main). **Cero cambios** en `settings-store.ts`/`ipc-settings.ts`/`shared/types.ts` (ya estaban completos desde Fase 14) ni en la lógica del watchdog en sí (`App.tsx:1978-1987`/`2002-2010`, intactas). Default 90s sin ningún cambio para quien no toque el campo nuevo.

### Verificación real — 4 casos, código de producción real, storage aislado, sin GUI (alcance de riesgo real de este cambio es puramente backend/lógica, la mecánica del watchdog en sí ya se probó de punta a punta con un servidor real colgado el día anterior — Fix 1)

- **Caso 1**: la lógica de validación real del input nuevo, contra los mismos casos reales que `validTurnWatchdogSeconds()` — `"300"→300`, `""→undefined`, `"0"→undefined`, `"-5"→undefined`, `"abc"→undefined`, `"45.5"→45.5` (fraccionario aceptado, a propósito).
- **Caso 2**: ciclo real `saveSettings()`→disco→`loadSettings()` con `turnWatchdogSeconds:300` — persiste real a través de un roundtrip completo (mismo mecanismo, storage aislado, nunca `D:\AMATISTA\data`).
- **Caso 3**: el valor real releído alimenta el mismo cómputo `TURN_WATCHDOG_MS` sin tocar — `300 * 1000 = 300000` real, confirmando que el valor de Settings llega intacto a la fórmula ya probada ayer.
- **Caso 4** (no-regresión): sin configurar nada, `turnWatchdogSeconds` real releído es `undefined`, el default real aplicado sigue siendo 90s.

`npm run typecheck`/`npm run build` en verde. Harness borrado por completo, confirmado real que ningún residuo llegó a `D:\AMATISTA\data`.

Archivo: `src/renderer/src/App.tsx` (único archivo tocado — campo nuevo). Sin commit — pendiente de que el usuario lo pida.

## Fix real — mensaje de cancelación limpio para runtimes CLI (Claude/Antigravity), reusando `TurnCancelledError` del runtime API

Basado en `verify_cli_clean_cancellation_design.md`, ya confirmado. Diagnóstico de la sesión anterior: `CliAgentRuntime.cancelTurn()` mataba `this.activeProcess` sin ninguna señal previa — el handler `exit` real de cada `send*()` solo miraba `code`, y `code:null` (la firma real de "lo matamos nosotros", que Node produce cuando un proceso muere por señal) caía en el MISMO branch genérico que un crash real (`reject(new Error(stderr.trim() || 'Claude terminó con código null.'))`). El branch CLI de `dispatchTurnForWindow()` (`ipc-agent.ts`) no tenía ningún `try/catch` — ese error crudo se propagaba sin filtrar hasta el renderer, indistinguible de un crash genuino, tanto para el watchdog (Fix 1, sección anterior) como para el botón "Detener" manual.

**El molde real — `TurnCancelledError` (API)**: clase genérica (`api-agent-runtime.ts:211-218`), `Error` subclass con un solo campo (`partialText: string`), lanzada cooperativamente dentro de los 4 loops de `send*()` cuando `signal.aborted` (el `AbortController` real que crea `dispatchTurnForWindow()` antes de llamar a `apiRuntime.send()`), atrapada en el branch API de `dispatchTurnForWindow()` — nunca relanzada, la función RESUELVE con `{success:true, cancelled:true, text: error.partialText}` y manda el evento real `turn/cancelled` al renderer, que ya lo maneja correctamente (`agentState` nunca cae en `'error'`).

**Fix real, reusando ese mecanismo SIN tocarlo** (restricción explícita del usuario — la clase en sí y sus 4 loops quedaron intactos, solo se importó):

- Campo privado nuevo `cancelledByUs = false` en `CliAgentRuntime`, seteado `true` en `cancelTurn()` justo ANTES de `this.activeProcess.kill()`.
- Los 3 handlers `exit` reales (`sendClaude`/`sendClaudeWithImages`/`sendAntigravity`) chequean el flag PRIMERO — en `sendAntigravity()` incluso antes del intento de `JSON.parse(stdout)` que ese método hace primero, para no arriesgarse a resolver como éxito genuino solo porque el stdout truncado por el kill resultó parsear igual. Si está seteado, lo resetean de inmediato (nunca sobrevive al turno que lo seteó) y lanzan `new TurnCancelledError('')` — importada de `api-agent-runtime.ts` (confirmado sin riesgo de ciclo con grep: ese archivo no importa nada de `cli-agent-runtime.ts`) — en vez del mensaje genérico.
- `partialText: ''` a propósito: a diferencia del loop API (texto real acumulado turno a turno vía múltiples respuestas HTTP), el camino CLI es un solo spawn con un stdout final truncado por el kill — no es texto de asistente confiable. El renderer ya degrada limpio con `partialText` vacío (muestra "Turno detenido por el usuario.", sin ningún caso nuevo del lado renderer).
- `ipc-agent.ts`: el branch CLI de `dispatchTurnForWindow()` envuelve ahora el `await cliRuntime.send(...)` (más los eventos de `item/agentMessage/delta`/`turn/completed` y el disparo de compactación en segundo plano que dependen del resultado) en el MISMO patrón `try/catch` que ya tenía el branch API, calcado: atrapa `TurnCancelledError`, manda `turn/cancelled` y retorna `{success:true, cancelled:true, text: error.partialText}` en vez de propagar. Cualquier otro error (crash real del binario, `code !== 0` genuino) sigue propagándose idéntico a como lo hacía antes de este fix.
- `App.tsx` sin ningún cambio — ya consumía el evento real `turn/cancelled` correctamente desde el mecanismo API (mismo `method === 'turn/cancelled'`); el gap nunca fue del lado renderer.

**Antigravity, confirmado sin trabajo aparte**: `CliAgentRuntime.send()` es un único punto de entrada que bifurca por `this.config.kind` (`'claude'` → `sendClaude()`, cualquier otro → `sendAntigravity()`) pero comparte instancia/`activeProcess`/`cancelTurn()` — un solo fix a nivel de clase cubre los 2 runtimes CLI simultáneamente.

### Verificación real — 3 casos, camino COMPLETO de producción, sin atajos

`connectSessionForWindow()` → `runTurnForWindow()` → `cancelSessionTurn()` real (nunca `CliAgentRuntime` en aislamiento), CLI real de Claude y Antigravity ya autenticados en esta máquina (suscripción real del usuario, ninguna credencial ingresada por el agente), storage aislado (harness temporal, wrapper plano que fija `AMATISTA_STORAGE_ROOT` en el proceso padre antes de requerir el bundle, mismo patrón ya establecido en esta sesión — nunca dentro del `.ts` bundleado, después de sus propios imports).

- **Caso Claude**: turno real disparado (`connectSessionForWindow` real con un provider `claude-cli` sembrado), `cancelSessionTurn()` real invocado ~2s después (proceso `claude` real vivo y matado a mitad de generación) → resultado real `{success:true, cancelled:true, text:""}`, nunca el mensaje crudo "Claude terminó con código null.".
- **Caso Antigravity**: mismo mecanismo, proceso `agy` real matado a mitad de generación → idéntico, `{success:true, cancelled:true, text:""}`.
- **Caso no-regresión**: turno CLI real corto ("Respondé únicamente OK"), sin cancelar, proceso `claude` real terminando con código 0 → `{success:true, text:"OK"}`, sin `cancelled`, comportamiento idéntico a siempre.

`npm run typecheck`/`npm run build` en verde. Bug real de infraestructura del propio harness (no del fix bajo prueba) encontrado y corregido durante la verificación: Windows tarda un instante en soltar los handles del workspace tras matar el proceso CLI recién cancelado — el primer intento de limpieza del harness dio `EPERM` real sobre el directorio temporal del caso Antigravity; corregido con una espera corta + reintentos en el `rmSync` del harness (infraestructura de verificación, no de producción). Harness borrado por completo (`.ts`, wrapper `.cjs`, bundle en `out/`), confirmado con `git status` sin rastro; los 2 directorios temporales huérfanos que había dejado la corrida con el `EPERM` (fuera de `D:\AMATISTA\data`) se limpiaron a mano; `D:\AMATISTA\data` real confirmado sin ningún residuo de prueba.

No corresponde cerrar ninguna entrada de `PENDING.md` por este fix — el diagnóstico original del día anterior ("Fix real — 2 hallazgos...", sección de arriba) ya quedó marcado Completa en `HISTORY.md` por su propia cuenta, nunca tuvo una entrada abierta en `PENDING.md`; este gap de cancelación CLI es un hallazgo posterior y distinto, sin entrada previa que cerrar.

Archivos: `src/main/cli-agent-runtime.ts`, `src/main/ipc-agent.ts`. Sin commit — pendiente de que el usuario lo pida (se junta con el fix de `turnWatchdogSeconds` y la entrada de `PENDING.md` de "Restaurar vista de paneles" en un solo commit final).

## Fix real — pausa/backstop del watchdog, reconexión forzada, y limpieza del mensaje de error viejo (runtimes API)

Basado en `verify_watchdog_and_reconnect_ux_design.md`, ya confirmado. Motivado por 3 problemas reales del usuario en una prueba de orquestación multi-panel con `turnWatchdogSeconds` ya en 180s — dos escenarios distintos disparaban el mismo error de watchdog: (a) el panel ORIGEN de un `send_to_window` esperando al panel destino, (b) un panel haciendo su propio trabajo real extenso (varias tool calls reales) sin depender de nadie. **ALCANCE**: solo runtimes API — `wireCli()`/CLI queda documentado aparte en `PENDING.md`, sin tocar (el mecanismo de pausa depende del evento real `item/toolCall/status`, que solo emite `wireApi()`).

### FIX 1 — pausa del watchdog durante una tool call real en curso + backstop absoluto

El mecanismo viejo era **reset-on-event**: `startTurnWatch()` le daba al turno una ventana fresca completa en cada `item/toolCall/status` (tanto `phase:'start'` como `phase:'done'`, llamado incondicionalmente antes de mirar la fase) — si esa tool call SOLA tardaba más que `turnWatchdogSeconds` (un `send_to_window` esperando el turno remoto completo, un `run_command` largo), el watchdog disparaba a mitad de trabajo real.

**Cambio real**: `phase:'start'` ahora llama a `pauseTurnWatch()` (función nueva — `clearTimeout` del timer pendiente SIN rearmarlo) en vez de `startTurnWatch()`; `phase:'done'` sigue llamando a `startTurnWatch()` igual que antes (reanuda con una ventana fresca completa). `turnStartRef`/`turnActive` quedan intactos durante la pausa — el turno sigue activo, solo el reloj de "sin actividad" se detiene.

**Backstop absoluto nuevo** (`backstopTimerRef`, timer SEPARADO del watchdog normal) — arma UNA sola vez, en el mismo punto donde `startTurnWatch()` ya inicializaba `turnStartRef` (el inicio genuino del turno), y **nunca se pausa ni se resetea** por ninguna actividad intermedia. Valor: `Math.max(turnWatchdogSeconds * 5, 900_000)` ms — 5x lo configurado, con un piso de 15 minutos para que un watchdog configurado bajo no deje un backstop demasiado corto para un turno legítimamente largo. Red de seguridad real si una tool call nunca emite su `'done'` (cuelgue real, desconexión) — sin esto, pausar indefinidamente dejaría el panel esperando para siempre en ese escenario.

Lógica de disparo (antes inline dentro del único `setTimeout` del watchdog) factorizada en `fireTurnTimeout(workspace, message)`, compartida por los dos timers — cada uno arma su propio `setTimeout` con su propio mensaje, ambos llaman a la misma función al disparar.

### FIX 2 — reconexión real forzada cuando el watchdog/backstop dispara

`agentState` nunca salía de `'connected'` cuando el watchdog disparaba (solo `agentError` se tocaba) — el próximo intento de enviar (`sendPrompt()`/`runTurn()`, chequean `agentState !== 'connected'`) nunca reconectaba, y podía chocar en silencio contra el guard real de `turnInFlight` en main si la tool call que estaba en curso no había terminado de liberarse — `cancelSessionTurn()` es cooperativo (`signal.aborted` solo se chequea antes/después de cada tool call real, confirmado en `sendAnthropicApi()` — nunca DURANTE una que ya está corriendo), y `send_to_window` ni siquiera propaga ninguna señal al panel destino (a diferencia de `parallel_ask`, que sí cascada `originSignal`).

**Bug real encontrado DURANTE la propia verificación en vivo, no anticipado en el diseño**: la primera versión de `fireTurnTimeout()` ponía `setAgentState('error')` directo — pero el `cancelAgent()` disparado ahí mismo puede desencadenar (para un runtime API con un `fetch` realmente colgado, cancelación efectiva vía `AbortSignal` nativo) un evento REAL `turn/cancelled` que llega poco después y **pisa `agentState` de vuelta a `'connected'` incondicionalmente** — código preexistente de ese handler (`method === 'turn/cancelled'`, `App.tsx`), sin tocar, fuera de alcance. Confirmado real con la app corriendo: el connect-button nunca reaparecía, y un reintento inmediato NO disparaba ningún `[agent:connect]` nuevo en el log real de main.

**Fix real de la carrera**: `forceReconnectRef` (`useRef(false)`) — bandera dedicada que **ningún otro código toca**, inmune a lo que `turn/cancelled`/`turn/completed` le hagan a `agentState` después. `fireTurnTimeout()` la prende (`forceReconnectRef.current = true`, seteada DESPUÉS de `cancelAgent()` a propósito). `sendPrompt()`/`runTurn()` cambian su guard de `if (agentState !== 'connected')` a `if (agentState !== 'connected' || forceReconnectRef.current)`. `connectAgent()` la apaga solo tras un connect real EXITOSO (`forceReconnectRef.current = false` junto a `setAgentState('connected')`) — un connect fallido la deja prendida para el próximo intento.

### FIX 3 — limpiar el mensaje de error viejo cuando un turno nuevo arranca de verdad

`agentError` (`useState('')`, renderizado sin condición aparte de no estar vacío) solo se limpiaba en 3 puntos: al ARRANCAR `connectAgent()`, al cambiar de chat activo, al cambiar el catálogo de proveedores — el camino feliz de `runTurn()` nunca lo tocaba, así que un mensaje de watchdog viejo seguía visible indefinidamente tras un turno nuevo exitoso. Fix: `setAgentError('')` agregado al handler real de `turn/started` — confirmación real de que el turno nuevo arrancó de verdad en main (no solo que el renderer lo intentó), evita el falso-positivo de "se vio limpio un instante" si el intento nunca llega a correr.

### Verificación real — 4 casos + no-regresión, app REAL empaquetada + CDP

Camino de verificación: app real (`npm run build` + `electron.exe out/main/index.js`) con storage aislado (`AMATISTA_STORAGE_ROOT` real, wrapper que lo fija en el proceso padre ANTES del spawn, mismo patrón ya establecido) y `--remote-debugging-port` real, controlada vía Chrome DevTools Protocol (WebSocket nativo, sin librerías — mismo tipo de técnica ya usada antes en esta sesión para verificar la mensajería entre ventanas, "CDP contra la app real"). Provider real sembrado vía la IPC real `saveSettings()`/`ensureChatSession()` (`window.universalAgent`, nunca archivos escritos a mano), `runtime:'openai-chat'` apuntando a un servidor HTTP local real y controlable por el texto del prompt (endpoint `http://127.1:8934` — forma corta de loopback que esquiva a propósito `isUnsupportedLocalProvider()`, un guard real y no relacionado a este fix que bloquea `127.0.0.1`/`localhost`/Ollama). Sandbox `danger-full-access` para que `run_command` se auto-apruebe sin diálogo.

- **Caso Fix 1**: tool call real (`run_command` → `powershell -Command "Start-Sleep -Seconds 5"`, proceso real) con `turnWatchdogSeconds:2`. Polling real cada 500ms durante los 5s reales de ejecución: `agentError` permaneció `null` todo el tiempo (antes del fix, hubiera disparado a los 2s) — turno completó limpio (`isStop:false`, texto final real) apenas el proceso real terminó, ~5.6s después.
- **Caso Fix 1c (backstop)**: tool call real de 2 minutos (nunca termina dentro de la ventana de prueba) con un shim TEMPORAL sobre `window.setTimeout` (inyectado/retirado vía CDP, técnica estándar de testing de timers, el código bajo prueba nunca se modificó) que comprime SOLO delays ≥100s a 4s reales — deja el watchdog normal (2s) intacto. El backstop disparó con su mensaje distinto ("límite absoluto de seguridad") a los ~3.5s reales, cortando el turno pese a que la tool call real seguía genuinamente en curso.
- **Caso Fix 2/3**: servidor real que nunca responde (sin ninguna tool call — escenario real de "el modelo no contestó", el que expone el bug de la carrera). El watchdog normal disparó a los ~1.5s reales. El reintento con un mensaje normal disparó un `[agent:connect]` real nuevo (confirmado con grep en el log real de main — ausente ANTES del fix de la carrera) y limpió el mensaje de error viejo de inmediato al llegar el `turn/started` real del reintento; el turno del reintento completó con éxito.
- **No-regresión**: turno normal sin ningún cuelgue, cero delay — idéntico a siempre, sin error, sin reconexión de más.

`npm run typecheck`/`npm run build` en verde (2 veces — antes y después de corregir el bug de la carrera del Fix 2 encontrado en vivo). Limpieza real completa tras la verificación: app Electron aislada + servidor fake + el proceso `powershell`/`Start-Sleep` real huérfano del caso de backstop, todos matados (`taskkill`/`Stop-Process`); storage aislado y scripts del harness borrados; `D:\AMATISTA\data` real confirmado sin ningún residuo (nunca se tocó, storage 100% aislado en todo momento).
...
## Feature real — orquestación por suscripción (`send_to_window`/`parallel_ask` desde Claude Code CLI/Antigravity CLI)

Basado en `docs/_arch/verify_subscription_orchestrator_design.md` (ya confirmado, ver ese doc para el análisis completo pre-implementación). Hasta esta fase, `send_to_window`/`parallel_ask` solo existían en el `toolCatalog()` de `ApiAgentRuntime` — un panel conectado por CLI (`claude-cli`/`antigravity-cli`, suscripción) nunca los veía, porque `CliAgentRuntime` no pasa por `tool-registry.ts`/`ExecuteContext` en absoluto. Esta fase generaliza el named pipe de aprobación que ya existía SOLO para eso (`mcp-approval-pipe.ts`) a una RPC chica de 4 acciones, y suma las 2 tools nuevas al servidor MCP standalone que ya se le inyecta a los 3 CLIs para LSP (`mcp-lsp-server.ts`).

### Protocolo del pipe (`mcp-approval-pipe.ts`, generalizado)

Named pipe de Windows fijo (`\\.\pipe\amatista-mcp-approval`), NDJSON de una línea por mensaje, una conexión por request — mismo framing que ya probaba el pipe original de solo-aprobación. Un campo `action` decide el shape del resto del mensaje:

- `confirm {panelId, action:'confirm', title, detail, toolName?}` → `{approved, error?}`. `toolName` ausente = camino original sin cambios (sin gate, sin eventos de watchdog). Presente (`'send_to_window' | 'parallel_ask'`) = dispara el gate de panel-principal Y el par de eventos de watchdog `phase:'start'`/`phase:'done'` alrededor de TODO el flujo (confirm + ejecución), no solo de este mensaje.
- `sendToWindow {panelId, action:'sendToWindow', destino, mensaje}` → `{ok, text?, error?}`. Ejecución real de `sendToWindowByTitle()` (`cross-window-messaging.ts`) — vuelve a re-verificar el gate igual que `confirm`, nunca confía en que el cliente ya haya pasado por ahí.
- `planParallelAsk {panelId, action:'planParallelAsk', subtasks}` → `{ok, assignments?, error?}`. Planificación síncrona real (`parallel-orchestrator.ts`) — el cliente arma el texto del diálogo de aprobación con `assignments` antes de mandar el siguiente `confirm`.
- `runParallelAsk {panelId, action:'runParallelAsk', assignments}` → `{ok, outcomes?, error?}`. Ejecución real, solo después de un `confirm` con `toolName:'parallel_ask'` aprobado — esta conexión no comparte estado con la de `planParallelAsk`, el cliente re-manda los mismos `assignments` que esa le devolvió.

**Gate de panel-principal (backstop real, nunca la primera línea de defensa)**: cada action que dispara orquestación real resuelve `sessionRegistry.get(panelId)` y llama `isPrincipalChat(session.activeChatId ?? '')` DE NUEVO del lado de `main` — nunca confía en lo que el proceso hijo (el servidor MCP standalone) afirme, aunque ese mismo `panelId` viaje fijo en su `env` desde el spawn. `isPanelAllowedToOrchestrate()` es la única fuente de verdad: `false` tanto si el panel no existe (nunca registrado, o ya desconectado) como si existe pero su chat activo tiene el sufijo `" — Panel N"` (`panelAliasForTitle()` no-null). Rechaza con `NOT_PRINCIPAL_ERROR` fijo, jamás `{ok:true}`.

**Watchdog (Tarea 6)**: el handler del pipe emite `sendSessionEvent(panelId, {kind:'notification', method:'item/toolCall/status', params:{name, phase}})` (`name` es `'send_to_window'` o `'parallel_ask'`) en los puntos reales de inicio/fin de cada flujo — mismo evento real que ya pausa/reanuda el watchdog de turno del renderer para runtimes API (`wireApi()` → `runtime.on('toolStatus', ...)`). Emitido DIRECTO desde `main` (el handler ya conoce el `panelId` de origen por el propio request del pipe), sin pasar por `wireCli()`/`CliAgentRuntime` en absoluto — cierra el caso puntual de "orquestación en curso", no el gap general de actividad interna del CLI (ver nota actualizada en `PENDING.md`).

### Cliente del pipe (`mcp-lsp-server.ts`, extendido)

`send_to_window`/`parallel_ask` se registran (`server.tool(...)`) SOLO si el proceso arrancó con `AMATISTA_PANEL_ID` Y `AMATISTA_IS_PRINCIPAL==='1'` en su `env` — primera línea de defensa, la tool ni siquiera existe para un panel no-principal. Cada tool nueva habla con el pipe vía un socket `node:net` directo (`callApprovalPipe()`, una conexión por request, mismo patrón "un socket, una promesa, sin polling" que ya prueba el pipe) — nunca importa nada de Electron/main (el proceso corre con `ELECTRON_RUN_AS_NODE=1`, `require('electron')` no es la API real ahí). `send_to_window`: 1 round-trip de `confirm` + 1 de `sendToWindow`. `parallel_ask`: 1 round-trip de `planParallelAsk` (sin diálogo) + 1 de `confirm` (con el plan YA resuelto, mismo formato de texto que `formatParallelPlanDetail()` de `tool-registry.ts`, duplicado a propósito) + 1 de `runParallelAsk`. Descriptions de las 2 tools nuevas reusadas literal de `tool-registry.ts` (mismo texto que ve un runtime API), más una nota real de este servidor MCP explicando el gate de panel-principal.

### Wiring de spawn (`cli-agent-runtime.ts`, `ipc-agent.ts`)

`ConfigureOptions` (`CliAgentRuntime`) ganó `panelId`/`isPrincipalChat` — el mismo `isPrincipalPanel` que `connectSessionForWindow()` (`ipc-agent.ts`) ya calculaba para la rama API (`isPrincipalChat(session.activeChatId ?? '')`), ahora también pasado a `runtime.configure()` en la rama CLI. `mcpLspServerSpawnSpec()` suma `AMATISTA_PANEL_ID`/`AMATISTA_IS_PRINCIPAL` (solo si `true`, ausente = falsy) al `env` del servidor MCP, mismo patrón que `AMATISTA_MCP_WORKSPACE`. Para Claude fuera de `danger-full-access`, `--allowedTools` suma `mcp__amatista-lsp__send_to_window`/`mcp__amatista-lsp__parallel_ask` a la allowlist existente de las 4 tools de LSP — SOLO si `isPrincipalChat`, nombres EXACTOS (nunca wildcard, mismo motivo ya documentado para las 4 de LSP). Antigravity no tiene equivalente de `--allowedTools`/allowlist — su primera línea de defensa es únicamente que el servidor MCP no registre las tools si `AMATISTA_IS_PRINCIPAL` no llegó en el `env` de `mcp_config.json`.

Codex deliberadamente NO tocado — motivo real documentado en `PENDING.md` (sin `CODEX_HOME` por conexión en modo suscripción, `config.toml` compartido entre todas las conexiones Codex de la máquina).

### Verificación real — app empaquetada + CDP, `claude`/`agy` reales ya autenticados por suscripción

Harness: `electron.exe .` (NO `electron.exe out/main/index.js` directo — ver "Hallazgo real" más abajo) con `AMATISTA_STORAGE_ROOT` aislado fijado en el proceso padre antes del spawn + `--remote-debugging-port`, controlado vía CDP (WebSocket nativo de Node 24, sin librerías). Providers reales sembrados vía `saveSettings()`/`ensureChatSession()` (las 2 conexiones de suscripción, `qcfg-claude-subscription`/`qcfg-antigravity-subscription`, ya vienen precargadas por default — no hizo falta crearlas a mano) más un provider `openrouter`/`api-key` apuntando a un servidor HTTP local propio (`http://127.1:8901`, esquiva `isUnsupportedLocalProvider()`) simulando 3-4 paneles "destino" API-backed, con soporte para un marcador `DELAY_<N>S` en el texto del último mensaje `user` (duerme N segundos antes de responder, usado en el Caso 5) y para un marcador `TRIGGER_SEND_TO_WINDOW:<destino>:<mensaje>` que devuelve un `tool_call` estilo OpenAI en la primera vuelta (usado en el Caso 6a, no-regresión del camino API). Workspace real de los chats de prueba apuntado al propio repo (`D:\APLICACIONES\universal-agent-studio-v0.4.4`) para que las tools de LSP tuvieran archivos reales sobre los que operar.

**Caso 1 — Claude Code CLI (panel principal, único panel al boot) → panel API destino, real de punta a punta**: prompt directo pidiendo `send_to_window` hacia un chat "Destino Uno" (ya con un turno real previo contra el servidor fake, requisito de la tool). El diálogo de aprobación se confirmó real en el DOM del renderer (`.approval-overlay .approval-dialog`, título `Enviar mensaje a "Destino Uno"`, detalle con el mensaje exacto pedido) — no asumido. Click real en "Aprobar" (`button.primary-btn`, mismo elemento que usa el usuario) resolvió la aprobación; el pipe ejecutó `sendToWindowByTitle()`, que auto-abrió un panel 2 real en la misma ventana (mecanismo de auto-open ya existente, sin tocar) y corrió un turno real contra el servidor fake — la respuesta volvió al panel de origen como mensaje del asistente, con el eco exacto del mensaje real. `liveStatusText` mostró `"Ejecutando: send_to_window"` durante la ejecución y `"send_to_window completado"` al terminar — confirma que el evento de watchdog (Tarea 6) se emite y llega al renderer.

**Caso 2 — `parallel_ask` real desde el mismo panel Claude Code CLI, 2 destinos**: con 3 paneles API ya conectados e idle en `sessionRegistry` (`Destino Uno/Dos/Tres`), prompt pidiendo repartir 2 subtareas. Diálogo real confirmado con el plan YA resuelto (`Repartir 2 sub-tarea(s) en paralelo entre 2 panel(es)`, detalle listando cada subtarea con el panel/modelo real asignado — `Destino Uno`/`Destino Dos`) — un solo diálogo de aprobación de cara al usuario (el "2do round-trip" del diseño es a nivel de pipe: `planParallelAsk` + `confirm`, no 2 diálogos distintos). Tras aprobar, fan-out real confirmado (2 timestamps de respuesta con 1ms de diferencia — ejecución concurrente real, no secuencial) y fan-in agregado correcto, cada resultado etiquetado con el panel/modelo real que lo resolvió. Repetido una vez con solo 1 panel destino disponible: el scheduler degradó correctamente a modo secuencial en el mismo panel (2 timestamps con 51ms de diferencia) en vez de fallar — confirma el criterio real de "más subtareas que paneles disponibles, un mismo panel toma varias en secuencia".

**Caso 3 — Antigravity CLI, mismo flujo que el Caso 1 — INCONCLUSO por límite real de cuota de la cuenta, no por el código**: reproducido tanto contra la app real (turno completo vía UI) como con un spawn manual aislado del binario `agy` (mismos argumentos/env exactos que arma `cli-agent-runtime.ts`, con `--log-file` real para inspeccionar el log interno sin el watchdog de la UI de por medio). El log real de `agy` (`agy-repro.log`) muestra el turno real bloqueado en reintentos internos con backoff exponencial contra un error de cuota genuino, **no relacionado con la tool nueva**:
```
run.go:389] Run: attempt N failed (RESOURCE_EXHAUSTED (code 429): Individual quota reached. Please upgrade your subscription to increase your limits. Resets in ~20h47m.)
```
El turno nunca llegó a decidir si llamar o no `send_to_window` — la cuota individual de la cuenta de Antigravity en esta máquina está agotada por hoy (reset informado por el propio backend en ~20h47m desde el momento de la prueba). En la app real, esto se manifestó como el turno colgado hasta que el watchdog normal (sin nada que pausar, porque el pipe nunca llegó a recibir ningún `confirm`) lo cortó con `"Turno detenido por el usuario"` — mismo mensaje que un cancel manual, sin distinguir la causa real (observación de UX preexistente, no de esta feature, no tocada). **El riesgo real señalado en el diseño (`verify_subscription_orchestrator_design.md`, Tarea 2 — si `agy` headless rechaza/ignora una tool MCP nueva con side-effects) queda SIN CONFIRMAR NI DESCARTAR** por esta causa externa — no forzado con ningún hack, tal como se pidió. El mecanismo de habilitación en sí SÍ se verificó correcto por inspección: `mcp_config.json` escrito en el HOME aislado de la conexión (`<HOME>/.gemini/config/mcp_config.json`) trae `AMATISTA_IS_PRINCIPAL:"1"` y el `panelId` real correcto en su `env`, idéntico en forma al de Claude. Pendiente de reintentar cuando la cuota se libere — el usuario decide si Antigravity queda fuera de esta feature mientras tanto.

**Caso 4 — gate real (backstop), conexión directa al pipe sin pasar por `mcp-lsp-server.cjs`**: un panel con chat titulado `"Chat general — Panel 2"` (no-principal por título, `panelAliasForTitle()` real) conectado de verdad en `sessionRegistry` (vía `connectAgent()`/`sendMessage()` reales, no fabricado). Script Node chico (`node:net` puro) conectado DIRECTO al named pipe simulando exactamente lo que un proceso hijo comprometido mandaría, probado contra las 3 actions que disparan orquestación real:
- `sendToWindow` directo (sin pasar por `confirm` siquiera) → `{"ok":false,"error":"Este panel no es el chat principal..."}`.
- `confirm` con `toolName:'send_to_window'` → `{"approved":false,"error":"Este panel no es el chat principal..."}`.
- `planParallelAsk` → `{"ok":false,"error":"Este panel no es el chat principal..."}`.

Ninguna de las 3 devolvió `{ok:true}`/`{approved:true}` bajo ninguna circunstancia. **Control positivo real**: el mismo `planParallelAsk` contra el `panelId` del panel PRINCIPAL real (mismo proceso, misma sesión) devolvió `{"ok":true,"assignments":[...]}` con un plan real — confirma que el rechazo de arriba es específicamente el gate de `isPrincipalChat`, no un endpoint roto o un panel inexistente.

**Caso 5 — watchdog no dispara durante una orquestación real lenta**: `turnWatchdogSeconds:20` (vía `saveSettings()`), destino (`Destino Tres`) con el marcador `DELAY_40S` en el mensaje de `send_to_window`. El diálogo de aprobación tardó ~18s reales en aparecer (tiempo de "pensar" normal de Claude antes de decidir llamar la tool — sin relación con el fix) — confirmado con un intento previo con `turnWatchdogSeconds:7` que sí disparó el watchdog NORMAL (sin pausar, porque el pipe todavía no había recibido nada) con el mensaje real `"ERROR AGENTE: no llego respuesta del modelo ni actividad de herramientas en 7s"` — evidencia de que el watchdog normal (pre-tool-call) sigue activo y sin cambios, tal como debe ser. Con `20s` de margen, tras la aprobación el `liveStatusText` mostró `"Ejecutando: send_to_window"` de forma sostenida durante los ~40s reales del delay del servidor fake — polling real cada 4s del `state-pill` del agente confirmó que NUNCA mostró `"Error en el agente"` en ningún momento de esa ventana, pese a que 40s > 20s del watchdog configurado. El turno completó limpio al recibir la respuesta real del destino.

**Caso 6 — no-regresión**: (a) un panel conectado por runtime API (`openai-chat` contra el mismo servidor fake, marcador `TRIGGER_SEND_TO_WINDOW`) disparó `send_to_window` por el camino PRE-EXISTENTE (`ExecuteContext`/`tool-registry.ts`, sin pasar por el pipe nuevo en absoluto) — diálogo real idéntico en forma, aprobado, turno real completado, sin ninguna regresión visible. (b) el mismo panel principal Claude Code CLI del Caso 1, con `mcp__amatista-lsp__list_symbols`/`get_diagnostics` reales sobre `src/main/chat-store.ts` — ~30 símbolos reales listados, diagnóstico limpio real — confirma que las 4 tools de LSP preexistentes siguen funcionando sin cambios para un CLI tras esta fase.

### Hallazgo real durante la propia verificación — NO es un bug de la feature, es un artefacto del harness de CDP

`app.getAppPath()` (usado por `mcpLspServerScriptPath()` para ubicar `out/main/mcp-lsp-server.cjs` fuera del asar) devuelve un valor DISTINTO según cómo se invoque `electron.exe`: con `electron.exe out/main/index.js` (pasando la ruta al script directo — el patrón de harness usado en verificaciones previas de esta bitácora), Electron resuelve `app.getAppPath()` al directorio del propio script (`out/main`), NO a la raíz del repo — `mcpLspServerScriptPath()` termina buscando `out/main/out/main/mcp-lsp-server.cjs` (no existe) y devuelve `null`, así que `--mcp-config` NUNCA se agrega al spawn de `claude`/`agy` — ninguna tool de `amatista-lsp` (ni las 4 de LSP preexistentes) queda disponible, confirmado real reproduciendo el spawn exacto de `cli-agent-runtime.ts` fuera de la app con un diagnóstico dedicado (`app.getAppPath()` real devolvió `...\out\main` con ese patrón de invocación). Con `electron.exe .` (pasando el DIRECTORIO del repo, dejando que Electron lea `package.json` → `"main": "./out/main/index.js"` él mismo — el mismo mecanismo real que usan `electron-vite dev` y el `.asar` de `electron-builder` en producción), `app.getAppPath()` resuelve correcto a la raíz del repo (o del `.asar`) en ambos caminos reales de la app — confirmado con el mismo diagnóstico. **Esto nunca afecta a un usuario real** (ni dev ni producción invocan Electron pasándole la ruta al script de entrada) — es exclusivamente una trampa del patrón de harness `electron.exe <script>.js` ya usado en fases anteriores de esta bitácora para verificación vía CDP, que hasta ahora nunca había ejercitado código dependiente de `app.getAppPath()` para ubicar un recurso fuera del asar. Documentado acá para que una verificación futura con este mismo harness use `electron.exe .` (cwd al repo) en vez de apuntar directo al script — **no se tocó ningún archivo de `src/`** por este hallazgo, es puramente del lado del harness de prueba.

`npm run typecheck`/`npm run build`/`npm run mcp:lsp:bundle` ya estaban en verde antes de empezar esta verificación (código ya implementado y compilado por el usuario) — no se tocó ningún archivo de `src/main/` durante esta pasada. Limpieza real completa: procesos `electron.exe`/`agy.exe`/`node` del harness (incluida la reproducción manual de `agy`) matados, storage aislado (`AMATISTA_STORAGE_ROOT`) y todos los scripts/logs de scratch borrados, `D:\AMATISTA\data` real confirmado sin ningún archivo modificado después del inicio de la sesión (comparado por `LastWriteTime`).

Archivos (ya modificados antes de esta verificación, sin cambios adicionales): `src/main/mcp-approval-pipe.ts`, `src/main/mcp-lsp-server.ts`, `src/main/cli-agent-runtime.ts`, `src/main/ipc-agent.ts`. Sin commit — pendiente de que el usuario lo pida.

### Actualización 2026-09-14 — Caso 3 (Antigravity CLI) reintentado con la cuota ya repuesta, cierra el inconcluso

Cuota real de la cuenta de Antigravity de esta máquina ya repuesta (confirmado antes de empezar: `agy -p "responde solo con la palabra: ok" --output-format json` devolvió `status:"SUCCESS"` en ~2s). Mismo harness exacto que el resto de esta verificación (`electron.exe .` + CDP nativo + `AMATISTA_STORAGE_ROOT` aislado), panel principal real conectado a Antigravity por suscripción (`qcfg-antigravity-subscription`/`qcfg-antigravity-auto`, ya precargado por default), prompt directo forzando `send_to_window` hacia un chat destino real (`CASE3_DESTINO`, ya con `provider_id`/`model_id` propios persistidos en `chat_sessions` vía `ensureChatSession()`, satisfaciendo el requisito de la tool de "ya tuvo un turno real antes").

**Sandbox `workspace-write` (`--mode accept-edits`, el default real de la app) — el riesgo señalado en el diseño (Tarea 2 de `verify_subscription_orchestrator_design.md`) SE CONFIRMA, con evidencia exacta del log interno de `agy`**: el turno completó en ~7s sin texto de assistant (`"El turno termino sin texto de assistant."` en el renderer), CERO diálogo de aprobación en el DOM, y el proceso MCP nunca mandó ningún request real al pipe (`mcp-approval-pipe.ts` sin actividad — confirmado con un servidor HTTP fake en el destino que no recibió ningún tráfico nuevo). El log real de `agy` (`<HOME_aislado_de_la_conexión>/.gemini/antigravity-cli/log/cli-*.log`) lo confirma línea por línea:
```
tool_confirmation_manager.go:188] Print mode: soft-denying tool confirmation "CallMcpTool" at step 4
server.go:2395] Tool confirmation for conversation ... step 4 (type=*gemini_coder_go_proto.Step_Generic approved=false)
```
`agy` headless auto-deniega ("soft-denying") la confirmación interna de `CallMcpTool` para `send_to_window` ANTES de que la llamada llegue siquiera al pipe de Amatista — mismo mecanismo interno de auto-deny ya documentado en este codebase para las tools NATIVAS de `agy` (`read_file` fuera de `--add-dir`, ver la entrada "Integración completa de Antigravity CLI" más abajo), confirmado ahora real también para una tool MCP de TERCEROS con side-effects (no de solo lectura, a diferencia de las 4 de LSP que sí funcionan headless). Sin ningún error visible para el usuario más allá de un turno que termina vacío — riesgo real de UX, no solo una limitación funcional invisible.

**Sandbox `danger-full-access` (`--dangerously-skip-permissions`) — funciona real, de punta a punta, mismo criterio que el Caso 1 con Claude**: mismo prompt, mismo panel, solo el sandbox cambiado. El diálogo de aprobación real de Amatista SÍ apareció en el DOM (`.approval-overlay`/`.approval-dialog`, título `Enviar mensaje a "CASE3_DESTINO"`, detalle con el mensaje exacto pedido) — confirma que bajo este modo `agy` SÍ deja pasar la llamada `CallMcpTool` hasta el pipe real (consistente con lo que el propio flag implica: se saltea TODAS sus confirmaciones internas, no solo las de sus tools nativas). Tras la aprobación real, `sendToWindowByTitle()` corrió de verdad, `liveStatusText` mostró `"Ejecutando: send_to_window"`, el mensaje llegó al chat destino (respuesta real recibida y devuelta), y el turno de origen terminó con `toolSteps:["send_to_window completado"]` y el bloque `crossWindow` real adjunto al mensaje entregado — mismo criterio de punta a punta que el Caso 1. (Observación aparte, no relacionada con el riesgo de esta prueba, no investigada a fondo: el chat destino auto-conectado terminó usando el provider/modelo DEFAULT de la app -- Claude Sonnet suscripción -- en vez del provider `openai-compatible` sembrado específicamente para ese chat vía `ensureChatSession()`/`saveSettings()`; posible artefacto de que ese provider de prueba se sembró con `capabilities.tools:false`, o de que el chat nunca pasó por el flujo normal de apertura+conexión manual antes del auto-open. No se tocó código por esto — no investigado lo suficiente como para confirmarlo como bug real de la feature.)

**Conclusión real, cierra el Caso 3**: el riesgo señalado en el diseño se confirma real y queda acotado con precisión: **`agy` headless SÍ auto-deniega una tool MCP de terceros con side-effects bajo `read-only`/`workspace-write` (mismo mecanismo interno de confirmación que ya bloqueaba sus tools nativas fuera de `--add-dir`), pero NO bajo `danger-full-access`**, donde la orquestación por suscripción funciona igual de bien que con Claude Code CLI. No es un bug nuevo de esta feature ni del código de Amatista — es la misma limitación estructural de `agy` headless ya documentada para sus tools nativas, confirmada ahora también para MCP de terceros. Decisión real, no forzada: Antigravity NO queda excluido de la orquestación por suscripción (funciona correctamente donde puede funcionar), pero `read-only`/`workspace-write` quedan documentados como sandbox modes donde `send_to_window`/`parallel_ask` nunca van a disparar para un panel Antigravity (fallan en silencio, sin mensaje de error visible para el usuario) — a diferencia de Claude Code CLI, que sí puede usarlas en cualquier sandbox mode gracias a `--allowedTools`. Ver `PENDING.md` para el cierre formal.

Sin cambios de código en esta pasada (`src/` sin tocar). Limpieza real: proceso `electron.exe` aislado y servidor HTTP fake del destino matados por PID, `AMATISTA_STORAGE_ROOT` temporal borrado por completo, `D:\AMATISTA\data` real del usuario confirmado sin ningún archivo con `LastWriteTime` posterior al inicio de esta verificación.

## Verificación real — 12 tools de sistema Windows (Familia B) implementadas, incluida la guardia monótona de `close_app`/`lock_screen`/`power`

Basado en `docs/_arch/verify_windows_control_design.md` (investigación previa, sin implementación). Esta pasada verifica en vivo el código YA implementado y compilado (`tool-registry.ts` líneas 2615-2833, `requestSessionToolApproval()`/`requestHardToolApproval()` en `runtime-state.ts:455-485`, render condicional del checkbox de confianza en `App.tsx` según `visibleToolApproval.allowTrust`) — **no se tocó ningún archivo de `src/` en esta pasada, no se encontró ningún bug real**.

### Harness

Mismo patrón que la verificación de orquestación por suscripción de más arriba: `electron.exe .` (nunca apuntando al script directo, ver el hallazgo de `app.getAppPath()` documentado arriba) con `AMATISTA_STORAGE_ROOT` aislado (`C:\amatista-verify-storage`, jamás `D:\AMATISTA\data` real) fijado en el proceso padre antes del spawn, `--remote-debugging-port=9333`, controlado por un cliente CDP mínimo propio (WebSocket nativo de Node 24, sin librerías — mismo criterio ya probado en esta bitácora). Un servidor HTTP local propio (`http://127.1:8901` — el truco real ya documentado de que `isUnsupportedLocalProvider()` sólo bloquea los substrings literales `'localhost'`/`'127.0.0.1'`, y `'127.1'` no matchea ninguno de los dos) simula una respuesta Chat Completions estilo OpenAI: cuando el turno todavía no incluye un mensaje `role:'tool'`, devuelve un `tool_call` sintético cuyo nombre/args se leen de un marcador `<<<TOOLCALL>>>{"tool":...,"args":{...}}<<<END>>>` puesto a propósito en el texto del propio mensaje `user` — cuando ya lo incluye, devuelve texto final. Provider `type:'openrouter'`/`runtime:'openai-chat'` real sembrado vía `window.universalAgent.saveSettings()` (nunca `settings.json` a mano) con `activeProviderId`/`activeModelId` apuntando a él, seguido de un `location.reload()` real (confirmado en `pickProvider()`/`pickModel()`, `App.tsx:1345-1362`, que arrancar con `settings.activeProviderId` ya seteado hace que el ÚNICO panel que la app abre siempre al boot — `Fase Paneles-2b`, `App.tsx:5042-5046` — herede ese provider/modelo sin ninguna interacción manual de UI) — confirmado real que hizo falta un fallback una vez (el primer boot, antes de sembrar el provider, ya había elegido "Claude Sonnet" como default; tras el reload con el provider ya seteado, el panel sí abrió con "Verify Model" solo. Un helper de respaldo (clic real en `.model-btn` → `.provider-header` → `.model-item`, mismo menú real que usa el usuario) quedó armado por si hacía falta, y no hizo falta en ningún test posterior). Los 24 pasos de la secuencia completa (setup + 9 tools normales + `close_app` full E2E + read-only + guardia monótona x2) escriben directo en el `<textarea>` real (`composer`) vía el setter nativo de React + evento `input`, clic real en `.send-btn`, poll real del DOM (`.approval-overlay`) para el diálogo, clic real en `button.primary-btn`/`.secondary-btn` (Aprobar/Rechazar) y, cuando corresponde, en `.trust-checkbox input` — nunca se invocó `agent:toolApproval:respond` directo por IPC salteando el DOM.

### Resultado real de las 12 tools

Las 9 "normales" (sandbox `workspace-write`, diálogo real confirmado con `hasTrust:true` en las 9 — checkbox de confianza presente, como corresponde) todas `ok:true` de punta a punta:
- **`notify`**: `Notification.isSupported()` real pasó, notificación mostrada. Confirmado con una CAPTURA DE PANTALLA REAL (`System.Windows.Forms`/`System.Drawing`, PowerShell) tomada ~1.2s después de la aprobación — el toast real de Windows ("Electron — Amatista Verify...") aparece visible en la esquina inferior derecha del monitor real.
- **`clipboard_set` + `clipboard_get`**: roundtrip real confirmado — texto conocido (`AMATISTA_VERIFY_CLIP_<timestamp>`) escrito y releído exacto. Valor original del portapapeles real del usuario respaldado ANTES del primer test (`Get-Clipboard -Raw`) y restaurado al final (`Set-Clipboard`).
- **`open_url`**: `shell.openExternal('https://example.com')` real — confirmado en la MISMA captura de pantalla de arriba (tomada para `notify`, pero disparada después de una corrida completa previa): el navegador default real (Chrome) tiene una pestaña real abierta en `example.com`, contenido real de la página visible.
- **`open_folder`**: `shell.openPath()` real sobre la raíz del propio repo, `ok:true` sin error.
- **`open_app`**: `notepad` real abierto — confirmado con un chequeo EXTERNO a la app (`Get-Process notepad`, PowerShell fuera de Amatista) que encontró el PID real (`27796`) unos segundos después.
- **`list_processes`**: `Get-Process` real vía PowerShell, tabla real y plausible de esta máquina (`claude`, `vmmem`, `MsMpEng`, `explorer`, ...).
- **`system_info`**: datos reales y plausibles de esta máquina exacta — `Windows 11 Home Single Language (build 26200)`, `Intel(R) Core(TM) i7-9750H CPU @ 2.60GHz`, RAM/disco con números coherentes entre 2 llamadas separadas por minutos (16.5→16.6 GB en uso, resto idéntico).
- **`volume`**: `keybd_event(VK_VOLUME_MUTE)` real aplicado 2 veces seguidas (mute→unmute, mismo criterio de tecla física que un toggle) para dejar el estado real de audio del usuario sin cambios netos — las 2 llamadas devolvieron `ok:true`.
- **`close_app('notepad')`**: full E2E real, sandbox `workspace-write` — diálogo con **`hasTrust:false`** (la guardia monótona aplica siempre, incluso fuera de `danger-full-access`), aprobado de verdad, `taskkill /PID .../T /F` real corrió, `1 de 1 proceso(s) cerrado(s)`. Confirmado con el mismo chequeo EXTERNO de arriba: `Get-Process notepad` después de la aprobación no encontró nada (`stdout` vacío) — notepad realmente terminado, no solo el string de éxito de la tool.

**`read-only` (chequeo rápido adicional, no pedido explícito pero mismo criterio de rigor que el resto de esta bitácora)**: `notify` en sandbox `read-only` NUNCA mostró diálogo (`approval:null`) — bloqueo de raíz inmediato, mensaje real `"Modo de solo lectura activo: no se puede mostrar notificaciones."`, confirma `resolveApproval()` (`tool-registry.ts:306-315`) actuando tal como está escrito.

### El caso más importante — guardia monótona real, con evidencia (no solo lectura de código)

**`danger-full-access` SIN `toolTrustSession`** (sandbox recién conectado, sin ninguna aprobación previa en esta sesión): `system_info` (una de las 9 normales) se auto-aprobó real, `approval:null`, cero diálogo — confirma el atajo de `resolveApproval()` para este sandbox. Inmediatamente después, en la MISMA conexión/sandbox: `close_app`, `lock_screen` y `power` (`action:'shutdown'`) los 3 SÍ mostraron diálogo real (`approval` no-null en los 3), **los 3 con `hasTrust:false`** (sin checkbox de confianza en el DOM), rechazados los 3 con clic real en "Rechazar".

**`danger-full-access` CON `toolTrustSession` activo** (la combinación central pedida): para activar `toolTrustSession` sin cambiar de sandbox (cambiar el `<select>` de sandbox dispara `disconnect()`, que resetea `toolTrustSession` — confirmado en código, `App.tsx` `onChange` del sandbox + `disconnectSession()` en `runtime-state.ts`), se usó `generate_image` — una tool que llama `ctx.confirm()` DIRECTO e incondicional (nunca `resolveApproval()`, mismo comentario real en `tool-registry.ts:2455-2461`), así que muestra diálogo con checkbox de confianza SIN IMPORTAR el sandbox activo. Diálogo real apareció (`hasTrust:true`), tildado el checkbox real + clic real en "Aprobar" — `toolTrustSession` queda `true` en el momento exacto en que `agent:toolApproval:respond` procesa `{approved:true, trust:true}` (`ipc-agent.ts:1019`), sin importar que la generación de imagen en sí haya fallado después por falta de configuración real (irrelevante para la guardia — el trust ya quedó fijado antes de eso). **Sin ningún cambio de sandbox ni desconexión de por medio**, en la MISMA conexión ya con `danger-full-access` + `toolTrustSession:true` simultáneos: `close_app`, `lock_screen` y `power` (`shutdown`) se volvieron a llamar — **los 3 volvieron a mostrar diálogo real** (`approval` no-null), **los 3 con `hasTrust:false`** (checkbox de confianza AUSENTE del DOM en los 3, confirmado leyendo `overlay.querySelector('.trust-checkbox')` real, no asumido), los 3 rechazados con clic real en "Rechazar" — ninguna combinación de sandbox+confianza saltó el diálogo ni una sola vez, para ninguna de las 3 tools destructivas, en ninguna de las 6 veces que se las llamó bajo `danger-full-access` (3 sin trust + 3 con trust).

Ningún click en "Aprobar" se dio nunca para `lock_screen` ni `power` en toda la verificación (regla de seguridad de la tarea respetada al pie de la letra) — las 6 veces que aparecieron (2 sandbox × 2 tools + repeticiones) terminaron en "Rechazar" real, confirmado que ni `rundll32.exe user32.dll,LockWorkStation` ni `shutdown.exe` corrieron (la propia tool nunca llega a ese código si `ctx.hardConfirm()` devuelve `false`, y el mensaje real devuelto en los 6 casos fue `"El usuario rechazo bloquear la sesion."`/`"El usuario rechazo la accion de energia."`, nunca un resultado de éxito).

### Sin bugs encontrados

Las 12 tools, la guardia monótona en sus 2 variantes (`danger-full-access` solo, y `danger-full-access` + `toolTrustSession`), y el ocultamiento del checkbox de confianza para las 3 destructivas se comportaron exactamente como describe el código y los comentarios ya presentes en `tool-registry.ts`/`runtime-state.ts`/`App.tsx` — cero discrepancia entre lo documentado en el propio código y lo observado en vivo. No se tocó ningún archivo de `src/`.

`npm run typecheck`/`npm run build` (incluye `mcp:lsp:bundle`) ya estaban en verde antes de empezar esta verificación (código ya implementado y compilado por el usuario) — no hubo necesidad de recompilar, no se tocó ningún archivo de `src/`. Limpieza real completa: los 4 procesos `electron.exe` + el `node` del servidor fake matados por PID real (`taskkill /F`), `C:\amatista-verify-storage` (storage aislado) borrado, todos los scripts/logs/capturas de scratch borrados, portapapeles real del usuario restaurado a su valor original, `volume` dejado en el mismo estado neto (mute+unmute), `D:\AMATISTA\data` real confirmado sin ningún archivo con `LastWriteTime` posterior al inicio de esta sesión (`config\settings.json`/`config\amatista.db` con fecha del 12/9, un día antes de esta verificación).

Archivos (ya implementados y compilados antes de esta verificación, sin cambios): `src/main/tool-registry.ts`, `src/main/runtime-state.ts`, `src/main/ipc-agent.ts`, `src/renderer/src/App.tsx`, `src/shared/types.ts`. Sin commit — pendiente de que el usuario lo pida.

## Verificación real — Familia A (computer use: screenshot/mouse_move/mouse_click/keyboard_type), API `anthropic-api` + Claude Code CLI vía MCP, 1 bug real encontrado y arreglado

Basado en `docs/_arch/verify_computer_use_security_model.md` + `docs/_arch/verify_computer_use_cli_extension.md` (ambos ya aprobados, diseño puro sin implementación). Esta pasada verifica en vivo el código YA implementado y compilado (`computer-use-actions.ts` nuevo; `runtime-state.ts` — `computerUseActive`, overlay real, panic key real, `panicStop()`; `tool-registry.ts` — 4 tools nativas con guardia de 2 capas; `mcp-approval-pipe.ts` — 4 handlers autocontenidos + fix retroactivo de `handleRunParallelAsk` sin `signal`; `mcp-lsp-server.ts` — 4 tools MCP nuevas gateadas por `AMATISTA_COMPUTER_USE_ACTIVE`; `cli-agent-runtime.ts` — `updateComputerUseActive()`/`--allowedTools`; `App.tsx` — advertencia dura + toggle del composer) contra los 6 casos pedidos explícitamente por el usuario, con acciones REALES de mouse/teclado/pantalla en esta misma máquina física (3 monitores reales, mismas coordenadas ya documentadas en los 2 docs de diseño).

### Harness

Mismo patrón ya establecido en esta bitácora (`electron.exe .`, nunca el script directo — ver el hallazgo de `app.getAppPath()` documentado más arriba), `AMATISTA_STORAGE_ROOT` aislado (`<scratchpad>/cu-verify/data`, jamás `D:\AMATISTA\data` real) fijado en el proceso padre antes del spawn, `--remote-debugging-port=9333`, cliente CDP propio (`WebSocket`/`fetch` nativos de Node 24, sin librerías) — con un fix real al propio harness: `getPageWsUrl()` tenía que preferir explícitamente el target `file://...index.html` sobre cualquier otro `type:'page'`, porque el `BrowserWindow` del overlay (`data:text/html;...`) ES OTRO target `'page'` real del mismo proceso Electron y puede listarse ANTES que la ventana principal — sin este fix, algunas evaluaciones CDP se ejecutaban en el contexto en blanco del overlay en vez de en el renderer real (detectado real, no hipotético: variables `window.__x` "desaparecían" entre llamadas).

**Hallazgo real de harness, nuevo respecto a verificaciones anteriores**: `AMATISTA_STORAGE_ROOT` aislado por sí solo NO alcanza para que un provider con `encryptedApiKey` (cifrado real vía `safeStorage`) decifre correctamente — confirmado real (`sendMessage` real devolvía `"Claude API requiere API key."` pese a que `settings.json` copiado SÍ traía el blob cifrado real). Causa real: Chromium/Electron guardan la clave real de cifrado (`os_crypt`) dentro de `Local State`, en la raíz del propio `userData` — un `AMATISTA_STORAGE_ROOT` nuevo genera una clave nueva, incompatible con blobs cifrados bajo el `Local State` real del usuario. Fix de harness real: copiar también `D:\AMATISTA\data\Local State` (además de `config\settings.json`) al storage aislado — confirmado que con eso el provider real (`8770cba3-...`, tipo `anthropic`/`api-key`) decifra y funciona. Dato accionable para una verificación futura que necesite un provider con API key real bajo `AMATISTA_STORAGE_ROOT` aislado. Para esta pasada, dado que esto solo afectaba al runtime `anthropic-api` (no a `claude-cli`, que usa autenticación por suscripción externa a `settings.json`) y que la guardia bajo prueba (`requestHardToolApproval()`) es EL MISMO código sin importar qué runtime la invoque (`ctx.hardConfirm` en `tool-registry.ts` y los handlers de `mcp-approval-pipe.ts` llaman ambos, literal, `requestHardToolApproval()` de `runtime-state.ts`), los 6 casos se corrieron con el runtime `claude-cli` real (provider `qcfg-claude-subscription`, ya autenticado por suscripción en esta máquina) — la rama `anthropic-api` de `tool-registry.ts` quedó verificada por lectura completa de código (idéntica estructura de guardia, `ctx.computerUseActive` bloqueando de raíz + `ctx.hardConfirm()` incondicional) más el hecho de que su registro en el catálogo (`api-agent-runtime.ts`, `hideComputerUseTools = this.config?.kind !== 'anthropic-api'`) se confirmó leyendo el diff, no ejecutado en vivo en esta pasada.

Destino descartable real: `notepad.exe` abierto con un archivo real (`notepad_test.txt`, contenido inicial `AMATISTA_TEST_7Q2K`) en vez de un Notepad en blanco — evita depender de que un tipeo sintético llegue a tiempo antes de la primera acción, y sirve de control de imagen para el Caso 5. Rectángulo real leído con `GetWindowRect` (`user32.dll`, `Add-Type` — mismo mecanismo ya validado en la investigación de diseño), enumerando ventanas reales por título (`EnumWindows`) en vez de confiar en `Process.MainWindowHandle` (que devolvió `IntPtr.Zero` real para el Notepad moderno de Win11, un hallazgo de harness menor, no de la feature). Panic key real disparado con un script propio (`keybd_event` vía `user32.dll`, Ctrl+Alt+Shift+Escape, down/up real de las 4 teclas) — nunca simulado desde dentro de la propia Electron, inyección de input real a nivel de SO.

### Bug real encontrado y arreglado — `computerUseAcknowledged` faltaba en el allowlist de `settings:save`

Durante el Caso 1 (flujo real de clickear "Entiendo los riesgos"): el botón real SÍ marcaba el estado local como confirmado (`mutateSettings(..., true)`, UI mostraba "Advertencia ya confirmada" de inmediato) y `window.universalAgent.saveSettings()` devolvía `{success:true}` — pero una lectura posterior real de `settings:get` (misma sesión, y confirmado también releyendo `config/settings.json` en disco) seguía mostrando `computerUseAcknowledged:false`. Causa raíz real, confirmada leyendo `ipc-settings.ts`: el handler `settings:save` (fix real de la carrera de `settings:save`, ya documentado en este mismo CONTRACT.md) fusiona campo por campo desde un allowlist explícito (`providers`, `compactionProviderId/ModelId`, `turnWatchdogSeconds`, `maxToolLoop`, `imageGenerationProviderId/ModelId`, `integrations`, `presets`) — `computerUseAcknowledged` (campo nuevo de Familia A) nunca se agregó a esa lista, así que `setSettings({...settings, ...})` conservaba el valor VIEJO de `settings` (el objeto vivo de main, no el payload del renderer) sin importar qué mandara el usuario. Exactamente el mismo modo de falla de allowlist ya documentado en los propios comentarios del archivo para Tavily/presets/`maxToolLoop` ("el handler devuelve success:true igual, pero el valor real nunca sobrevive el guardado") — la cuarta vez que este patrón se repite en este archivo. Consecuencia real para el usuario: la advertencia dura de Configuración se re-mostraría en cada reinicio de la app pese a que el propio texto de la UI decía "confirmada" (contradice el diseño explícito de la Tarea 1: "SOLO para no repetir el texto largo de advertencia en cada sesión") — no es un agujero de seguridad (la dirección del fallo es fail-SAFE: menos permisivo, no más), pero sí una regresión funcional real y confirmada.

**Fix**: `computerUseAcknowledged: sanitized.computerUseAcknowledged` agregado al objeto de merge en `settings:save` (`src/main/ipc-settings.ts`) — `sanitizeSettings()` ya preserva el campo intacto vía spread (confirmado leyendo su función completa, no asumido), así que la única pieza faltante era la línea del allowlist. `npm run build` (typecheck + electron-vite + `mcp:lsp:bundle`) reverificado en verde tras el fix. Reverificado real de punta a punta con la app relanzada desde cero: click real en "Entiendo los riesgos" → `settings:get` inmediato YA muestra `true` → `config/settings.json` en disco YA muestra `true` → app matada y relanzada con el MISMO storage aislado → `computerUseAcknowledged` sigue `true` tras el reinicio real (antes del fix, hubiese vuelto a `false`).

### Resultado real de los 6 casos

**Caso 1 — Capa 2 nunca se saltea, ni con `danger-full-access`**: panel conectado real (`claude-cli`, sandbox `danger-full-access`), Capa 1 armada con el checkbox REAL del composer (`label.computer-use-toggle`, clic real, no IPC directo). Prompt directo pidiendo `screenshot` — el diálogo de aprobación real apareció (`Aprobar`/`Rechazar`, detalle real "Tomar una captura real del monitor primario."), sin checkbox de confianza (`hasTrustCheckbox:false`, confirmado leyendo el DOM real) pese a `danger-full-access` activo — mismo criterio ya verificado para `close_app`/`lock_screen`/`power` (Familia B). Aprobado real con clic en "Aprobar" (botón real del DOM) → screenshot real ejecutado, imagen real devuelta al modelo. Reforzado con `mouse_click`+`keyboard_type` reales en la MISMA sesión (`danger-full-access`): 2 diálogos reales más, ambos sin checkbox de confianza, ambos aprobados con clic real — clic real en `(550,400)` dentro del Bloc de notas descartable + tipeo real de "PRUEBA_COMPUTER_USE_12345", confirmado con una captura real RECORTADA solo al rectángulo de la ventana de Notepad (nunca el escritorio completo) mostrando el texto exacto concatenado (`AMATISTA_TEST_7Q2KPRUEBA_COMPUTER_USE_12345`, 43-44 caracteres) — prueba real de que el click puso el foco real y el tipeo llegó carácter por carácter al campo correcto. La independencia de `toolTrustSession` (segunda mitad del enunciado del Caso 1) se confirma por lectura de código, 100% determinística: `requestHardToolApproval()` (`runtime-state.ts`) siempre llama `requestToolApproval(..., allowTrust=false)` — el único `if` que consulta `toolTrustSession` (`if (allowTrust && session.toolTrustSession)`) es estructuralmente inalcanzable con `allowTrust` hardcodeado en `false` en AMBOS call sites reales (`ctx.hardConfirm` de `tool-registry.ts` y los 4 handlers de `mcp-approval-pipe.ts`) — no hay combinación posible de sandbox+confianza que lo saltee, sin importar el runtime. De paso, se ejercitó real el flujo completo de la advertencia dura de Configuración (botón "Entiendo los riesgos" real, ver bug de arriba) al menos una vez, como pedía el enunciado.

**Caso 2 — overlay real cubriendo los 3 monitores reales, visible solo durante la acción**: durante la ejecución real de `screenshot` (Caso 1), una captura REAL e independiente del escritorio completo (`System.Drawing`/`CopyFromScreen`, fuera de Amatista, mismo mecanismo ya validado en la investigación de diseño) confirmó visualmente el borde rojo/rosa (`#ff2d55`) rodeando los 3 monitores reales en sus coordenadas EXACTAS (`DISPLAY1 {770,-1080,1920,1080}`, `DISPLAY2 {0,0,1680,1050}`, `DISPLAY3 {1680,0,1050,1680}` — medidas de forma independiente con `System.Windows.Forms.Screen.AllScreens`, coinciden exacto con los `monitor-border` reales leídos del propio HTML del overlay vía CDP) más la etiqueta real "⚠ AMATISTA tiene el control del mouse/teclado — Ctrl+Alt+Shift+Esc para detener YA" legible en la esquina del monitor primario. Capturas inspeccionadas una sola vez y BORRADAS de inmediato (contenían escritorio real del usuario con apps/ventanas ajenas a esta prueba — nunca reproducido en este documento). Un test limpio de una sola acción (`mouse_move` de punta a punta, sin cancelar) confirmó el ciclo completo: overlay visible durante la ejecución, ausente (borde no presente en una captura real posterior) apenas el turno completó. **Anomalía real observada, NO reproducida ni confirmada como bug**: en una corrida temprana con 2 llamadas `sendMessage` superpuestas por un error de scripting propio de este harness (una quedó rechazada por el guard real "Ya hay un turno en vuelo en este panel"), el overlay quedó visible ~2 minutos después de que el turno visible había completado — un `disconnectAgent()` real lo ocultó de inmediato (mismo mecanismo de limpieza que ya usa `setComputerUseActive(panelId,false)`). Una repetición limpia y aislada (una sola acción, sin superposición) NO reprodujo la anomalía — overlay se ocultó normal apenas terminó. Queda anotada en `PENDING.md` como investigación abierta (posible condición de carrera real bajo llamadas superpuestas al mismo panel, `inFlightPanels` es un `Set` de panelId — no un contador por-acción — así que 2 acciones concurrentes del MISMO panel podrían, en teoría, dejar el Set corto si una termina antes que la otra), no como bug confirmado — no se tocó código por esto (no se pudo reproducir de forma aislada y determinística, condición explícita del propio pedido del usuario para tocar código).

**Caso 3 — panic key real interrumpe a mitad de camino, no espera el paso actual**: `mouse_move` real hacia el extremo opuesto de los 3 monitores (`(2700,1650)` → `(800,-1050)`, distancia real ~3300px, 10 micro-pasos). Panic key real disparado (`keybd_event` real, Ctrl+Alt+Shift+Escape) ~850ms después de aprobar (gap medido real, incluye el overhead real de spawnear el script de inyección) — posición real del cursor (`GetCursorPos`, leída DESPUÉS del panic key) quedó en `(2335,377)`, ni el punto de partida ni el destino declarado — el turno completo mostró `cancelled:true`. Confirmado además que `panicStop()` desarma Capa 1 por completo: el checkbox real "Control de escritorio" quedó DESTILDADO tras el panic key (`toggleChecked:false`), sin que el usuario lo tocara — mismo comportamiento documentado (`panicStop()` llama `setComputerUseActive(panelId,false)` para cada panel armado/en curso). Un primer intento con ~8s de gap real llegó tarde (el movimiento ya había completado, cursor en el destino exacto) — reintentado con el gap ajustado (aprobar + disparar el panic key en el mismo proceso Node, minimizando el round-trip) para obtener la interrupción real a mitad de camino.

**Caso 4 — cancelación no-cooperativa real vía el botón "Detener" real, misma interrupción, Capa 1 queda ARMADA (a diferencia del panic key)**: mismo patrón que el Caso 3 pero con un clic real en `[title="Detener generacion"]` (el botón real del DOM, mismo elemento que usa el usuario, `cancelAgent()` → IPC `agent:cancel` → `cancelSessionTurn()`) en vez del panic key. Con el clic disparado a los ~0-150ms de aprobar (latencia mínima real de esta prueba, DOM click sincrónico), el cursor real nunca se movió de su posición previa (interrumpido antes del primer micro-paso) en 2 corridas reales — el turno mostró `cancelled:true` en ambas. Diferencia real confirmada, y es la distinción central que pedía este caso: a diferencia del Caso 3, el checkbox real "Control de escritorio" quedó TILDADO (`toggleChecked:true`) después de la cancelación por "Detener" — confirma en vivo que `cancelSessionTurn()` (llamado solo, sin pasar por `panicStop()`) NUNCA toca `computerUseActive`, exactamente como documenta el propio código (`panicStop()` es el único camino que desarma Capa 1 además de una desconexión real).

**Caso 5 — Claude Code CLI real "ve" el contenido real de `screenshot` vía MCP**: mismo panel `claude-cli` real, prompt pidiendo `screenshot` del monitor primario y transcripción exacta del texto visible en la ventana de Notepad. Respuesta real, primera línea, carácter por carácter: "AMATISTA_TEST_7Q2KPRUEBA_COMPUTER_USE_12345" — coincide EXACTO con el contenido real verificado independientemente en Notepad segundos antes (control real de imagen, mismo criterio que el test HOLA/ZQX7 de la investigación de diseño previa, con la diferencia real de que acá se leyó un screenshot de ESCRITORIO COMPLETO real, no un PNG sintético de prueba). El propio modelo incluso cruzó el conteo de caracteres reportado por la barra de estado real de Notepad (43 caracteres) contra la longitud de su propia transcripción, sin que se le pidiera — evidencia adicional de que está leyendo contenido real, no adivinando.

**Caso 6 — no-regresión, LSP + orquestación por suscripción, ambas reales desde CLI**: `list_symbols` real sobre el propio `src/main/computer-use-actions.ts` devolvió los 17 símbolos top-level reales y correctos (`clickAt`, `ClickResult`, `getCursorPosition`, `isUacForeground`, `MicroStepResult`, `MouseButton`, `moveMouseTo`, `Point`, `POWERSHELL_TIMEOUT_MS`, `runPowerShellScript`, `ScreenshotResult`, `setCursorPosition`, `sleep`, `takeScreenshot`, `TypeResult`, `typeText`, `USER32_TYPE_DEF` — coincide exacto con el archivo real), `get_diagnostics` real sin errores. `send_to_window` real de punta a punta: 2 chats reales (título principal sin sufijo, título secundario con el sufijo real " — Panel 2", mismo criterio exacto que `panelAliasForTitle()`), panel secundario con un turno real previo (requisito de la tool), diálogo real de aprobación ("PRUEBA_NOREGRESION_SENDTOWINDOW_9K3"), aprobado real, mensaje entregado real al panel destino, turno real corrido ahí, respuesta real devuelta al panel de origen marcada como recibida de otra ventana — canal de orquestación confirmado intacto tras los cambios de Familia A. `parallel_ask` no se re-ejecutó en vivo en esta pasada (no hubo tiempo de armar un tercer panel) — no-regresión confirmada por lectura completa del diff (`handleSendToWindow()`/`handlePlanParallelAsk()` sin ningún cambio de línea; el único cambio real en todo `mcp-approval-pipe.ts` fuera de las 4 acciones nuevas de computer use es el fix ya documentado de `handleRunParallelAsk()` ganando el `signal` que antes le faltaba) más el hecho de que comparte exactamente el mismo servidor MCP/named pipe ya confirmado funcional en este mismo caso.

### Fix retroactivo confirmado (PASO 0 del pedido de implementación, ya en el código, releído en esta pasada)

`mcp-approval-pipe.ts` ahora importa y usa `requestHardToolApproval()` real (antes del propio trabajo de Familia A, el archivo solo tenía `requestSessionToolApproval()` — confirmado con el diff, cero referencias viejas a la variante dura). `handleRunParallelAsk()` ahora pasa `session.turnAbortSignal?.signal` a `runParallelAsk()` (antes no pasaba ningún `AbortSignal`, brecha real ya documentada en `verify_computer_use_cli_extension.md` Tarea 4) — confirmado con el diff real, no re-ejecutado en vivo como caso propio (cubierto indirectamente por el Caso 6, que confirma que `parallel_ask`/orquestación sigue funcionando tras este cambio).

### Sin bugs adicionales encontrados

El resto de la guardia de 2 capas (Capa 1 `computerUseActive` bloqueando de raíz sin llamar a `hardConfirm`, Capa 2 `requestHardToolApproval()` siempre después e incondicional, `beginComputerUseAction()`/`endComputerUseAction()` rodeando solo la ejecución real nunca la espera de aprobación, `isUacForeground()` como guardia real de UAC nunca ejercitada a propósito por las reglas de seguridad de esta verificación, el límite explícito a `anthropic-api` para el pipeline de visión de runtimes API) se comportó exactamente como describe el código y los 2 docs de diseño — cero discrepancia adicional entre lo documentado y lo observado en vivo.

`npm run build` (typecheck + `electron-vite build` + `mcp:lsp:bundle`) verde 2 veces — antes de encontrar el bug de `ipc-settings.ts` (confirmando que el resto de Familia A ya compilaba limpio, tal como afirmaba el pedido) y después del fix. Limpieza real completa: proceso de prueba `notepad.exe` (PID real, nunca el Notepad preexistente ajeno del usuario, verificado por título antes de matar) cerrado, los procesos reales de la instancia aislada de `electron.exe` (main + renderer + GPU + utility + renderer del overlay) matados por `taskkill /T`, panic key global CONFIRMADO desregistrado tras el cierre (`globalShortcut.register()` de un proceso Electron nuevo e independiente sobre el MISMO accelerator devolvió `true` recién después de matar Amatista — prueba real, no solo ausencia de error), storage aislado y todo archivo de scratch (incluidas TODAS las capturas de pantalla reales, inspeccionadas y borradas de inmediato cada vez, nunca acumuladas) borrados por completo, `D:\AMATISTA\data` real confirmado sin ningún archivo modificado (mismo `LastWriteTime` de `config\settings.json`/`Local State` antes y después de toda la sesión — nunca se escribió ahí, solo se leyó para copiar al storage aislado).

Archivos: `src/main/computer-use-actions.ts` (nuevo), `src/main/runtime-state.ts`, `src/main/tool-registry.ts`, `src/main/mcp-approval-pipe.ts`, `src/main/mcp-lsp-server.ts`, `src/main/cli-agent-runtime.ts`, `src/main/api-agent-runtime.ts`, `src/main/ipc-agent.ts`, `src/main/settings-store.ts`, `src/shared/types.ts`, `src/preload/index.ts`, `src/preload/index.d.ts`, `src/renderer/src/App.tsx`, `src/renderer/src/assets/main.css`, `package.json`/`package-lock.json` (dependencia nueva real `@nut-tree-fork/nut-js` + entrada nueva en `build.asarUnpack`) — todos ya implementados antes de esta verificación. **`src/main/ipc-settings.ts` modificado EN esta pasada** (el único cambio de código real de esta verificación — el bug de `computerUseAcknowledged` de arriba). Sin commit — pendiente de que el usuario lo pida.

## Fix real — sentinel `Record<keyof AppSettings, 'renderer'|'main'>` en `settings:save`, cierra el patrón recurrente de campo-nuevo-olvidado (4ta ocurrencia)

Investigado antes de implementar (`docs/_arch/verify_settings_allowlist_safety_design.md`, ya confirmado): el mismo bug de fondo apareció 4 veces distintas (`maxToolLoop`, `integrations`/Tavily, `presets`, `computerUseAcknowledged` — ver arriba en este mismo archivo) — un campo nuevo se sumaba a `AppSettings` (`shared/types.ts`) pero se olvidaba agregarlo también, a mano, al objeto literal de `ipc-settings.ts` que decide qué campos del payload del renderer sobreviven el guardado real. `settings:save` seguía devolviendo `{success:true}` en todos los casos — el valor simplemente nunca persistía, sin ningún error visible.

**Contexto histórico confirmado real, no asumido**: esa allowlist NO es arbitraria — nace en la Fase Paneles-2b (`docs/_arch/verify_settings_race.md`) para resolver una carrera real: el renderer nunca vuelve a pedir `settings:get` después del arranque, así que 4 campos que MAIN escribe en vivo (`activeProviderId`/`activeModelId`/`activeProjectPath`/`projectRoots`, vía `withSettingsLock()`/`projects:addRoot()`/`removeRoot()`) podían ser pisados en silencio por una copia arbitrariamente vieja del renderer si `settings:save` aceptara cualquier campo del payload sin filtrar. Confirmado con reproducción real en su momento que sin esa distinción, cualquiera de los sitios reales de `App.tsx` que disparan `settings:save` podía revertir una conexión real de OTRO panel. Invertir el criterio a blacklist (aceptar todo salvo esos 4) habría reabierto exactamente esa carrera, de forma más grave todavía: un campo `main`-owned nuevo olvidado se ACEPTARÍA del payload stale (corrupción de estado en vivo real), en vez de simplemente no persistir (el modo de falla actual, molesto pero benigno).

**Diseño evaluado y descartado**: un test de regresión real que compare `Object.keys()` de un `AppSettings` de ejemplo contra la union de la allowlist + una lista de "excluidos a propósito" — investigado y confirmado que este repo NO tiene CI ni hook de pre-commit hoy (`.github/` vacío, sin husky), y `npm run test:regression` no está enganchado a ningún paso real del ritual de cierre de este mismo archivo (siempre "`typecheck`/`build`: limpios", nunca el test de regresión) — un test así habría sido 100% manual, mismo hueco que ya tiene el problema actual (alguien tiene que acordarse de correrlo). Descartado como solución PRINCIPAL por ese motivo real, no por preferencia estética.

**Fix implementado, `src/main/ipc-settings.ts`**: el objeto literal (10 de 14 campos, copiados a mano, con un comentario de advertencia repetido 4 veces) se reemplazó por:

```ts
const SETTINGS_FIELD_OWNER: Record<keyof AppSettings, 'renderer' | 'main'> = {
  providers: 'renderer', projectRoots: 'main', activeProviderId: 'main',
  activeModelId: 'main', activeProjectPath: 'main', compactionProviderId: 'renderer',
  compactionModelId: 'renderer', turnWatchdogSeconds: 'renderer', maxToolLoop: 'renderer',
  imageGenerationProviderId: 'renderer', imageGenerationModelId: 'renderer',
  integrations: 'renderer', presets: 'renderer', computerUseAcknowledged: 'renderer'
}
```

`Record<keyof AppSettings, ...>` fuerza a TypeScript a exigir las 14 claves reales de la interfaz — agregar un campo nuevo a `AppSettings` sin clasificarlo acá es, desde este cambio, un **error de compilación real** (`tsc`, y por lo tanto `npm run build`/`npm run dist`, fallan), no un bug que recién se nota cuando el usuario reporta que su configuración "no se guarda". El merge real se deriva de ese mapa en un loop (`copyOwnedField<K extends keyof AppSettings>()`, genérico por clave — una función aparte porque TypeScript no liga correctamente el tipo dentro de una asignación inline `merged[key] = sanitized[key]` cuando `key` viene de iterar `Object.keys()`). Cero cambio en `shared/types.ts` ni en ningún consumidor de `AppSettings` (12 archivos reales, incluido `App.tsx`, 6146 líneas) — el tipo en sí no cambió de forma, solo `ipc-settings.ts`.

**Verificado real, 3 casos**:
1. **La prueba más importante**: se agregó un campo ficticio (`__sentinelTestField?: boolean`) a `AppSettings` SIN clasificarlo en `SETTINGS_FIELD_OWNER`, confirmado real que `npm run typecheck` lo rechaza (`error TS2741: Property '__sentinelTestField' is missing in type '{...}' but required in type 'Record<keyof AppSettings, "main" | "renderer">'`) — campo revertido después, `typecheck`/`build` reverificados en verde.
2. Roundtrip real a disco de los 10 campos renderer-owned, contra una app real (`electron.exe .` con storage aislado, mismo harness CDP ya establecido esta sesión): `turnWatchdogSeconds`/`maxToolLoop`/`compactionProviderId`/`compactionModelId`/`imageGenerationProviderId`/`imageGenerationModelId`/`integrations` (Tavily, cifrado confirmado presente)/`presets`/`computerUseAcknowledged`/`providers` — los 10 sobrevivieron el guardado, confirmado leyendo `config/settings.json` DIRECTO del disco (no solo `getSettings()` en memoria).
3. Los 4 campos main-owned (`activeProviderId`/`activeModelId`/`activeProjectPath`/`projectRoots`) ignoraron un payload con valores stale inyectados a propósito (`STALE_FAKE_PROVIDER_ID_12345`, etc.) en el MISMO `saveSettings()` donde un campo renderer-owned (`turnWatchdogSeconds:77`) SÍ se aplicó — confirma que el filtro real es por campo, no todo-o-nada del payload.

Limpieza real: proceso Electron de prueba matado, storage aislado (`os.tmpdir()`) borrado, `D:\AMATISTA\data` nunca referenciado por el harness.

Archivo: `src/main/ipc-settings.ts` (único cambio de código real de esta implementación). Sin commit — pendiente de que el usuario lo pida.

## Feature real — sidebar de PROYECTOS limpio, carpeta → lista de CHATS reales asociados (sin árbol de archivos)

Basado en `docs/_arch/verify_project_sidebar_clarity_design.md`, ya confirmado, opción (a) (igualdad exacta de `workspacePath`, sin containment de subcarpetas). Motivado por ruido real reportado por el usuario: el sidebar de PROYECTOS mostraba subcarpetas reales de primer nivel (`scanProjectRoot()`, `project-registry.ts`) sin relación real con los chats existentes — la investigación previa confirmó que eso NO era un árbol de archivos recursivo (solo raíz + subcarpetas de primer nivel, sin archivos), pero igual generaba ruido real, sin utilidad real para el usuario.

**Cambio real, `src/renderer/src/App.tsx`**:
- `chatsForWorkspacePath(path)` (función nueva, junto a `resolveOrCreateChatForPath()`/`resolveOrCreateProjectChat()`): mismo filtro exacto que ya usa `resolveOrCreateChatForPath()` (`chat.workspacePath === path`, sobre `chatSessions` ya en memoria del renderer, sin IPC nuevo), sin el `.sort()[0]` final — devuelve TODOS los chats reales de ese workspace, más recientes primero. Igualdad exacta de `path`, no contención de carpeta (`isWithinFolder()` vive en main, el renderer no importa `node:path`) — decisión explícita de diseño, opción (a) del documento de investigación: un chat en una subcarpeta real NO cuenta para su raíz.
- El bloque `{projects.filter(project => project.rootId === root.id).map(...)}` (subcarpetas de primer nivel, `scanProjectRoot()`) se reemplazó por `{chatsForWorkspacePath(root.path).map(chat => ...)}` — cada fila es un botón simple con el título real del chat (`chat.title`), clase `active`/`open-elsewhere` reusando el mismo criterio ya usado en la sección CHATS (`openPanels.find(entry => entry.chatId === chat.id)`), click real dispara `openChatInPanel()` (sin cambios en esa función). Sin fila si la raíz no tiene ningún chat todavía — mismo criterio ya establecido en esta misma app para controles sin utilidad real vacíos (presets/`effortOptions`): el botón "+" de cada raíz ya cubre crear el primero, sin necesidad de un placeholder tipo "sin chats".
- Deliberadamente SIN rename/delete/menú contextual en estas filas (a diferencia de la sección CHATS) — mismo espíritu limpio que motivó el pedido; el mismo chat sigue editable/borrable desde la lista CHATS de arriba (no se filtró de ahí, aparece en los 2 lugares a propósito, sin tocar `buildChatRows()`/la sección CHATS, fuera del alcance pedido).

**Dejado a propósito sin tocar, ya anotado como tal en el propio diseño**: `buildTree()`/`workspace-tree.ts` (el escaneo recursivo real con archivos) — confirmado en la investigación que no tiene consumidor real en el sidebar hoy, sin relación con este cambio. `scanProjectRoot()`/`project-registry.ts`/el canal IPC `projects:list` y el estado `projects` (`useState<ProjectEntry[]>`, `App.tsx`) quedan en el código pero SIN NINGÚN consumidor real tras este cambio (confirmado con grep, el único uso real era el bloque reemplazado) — candidato real a limpieza en una sesión futura, no removido acá por estar fuera del alcance explícito de este pedido ("esto es solo el render del sidebar").

**Verificado real**, `electron.exe .` + CDP nativo + `AMATISTA_STORAGE_ROOT` aislado, 2 carpetas de proyecto reales (con una subcarpeta real y un archivo real de prueba adentro de una de ellas, sembrados a propósito para el chequeo de fuga) sembradas directo en `config/settings.json` antes del arranque (campo `projectRoots`, sin credenciales, no requiere el camino cifrado de `saveSettings()`):
1. Las 2 raíces reales aparecen en el sidebar, sin ninguna sub-fila (sin chats todavía) — confirmado que la subcarpeta real y el archivo real de prueba sembrados en disco NUNCA aparecen en el DOM real (cero fuga, confirmado por búsqueda de texto sobre el HTML real capturado).
2. Click real en el "+" de una raíz (mismo botón real ya existente, sin cambios) creó un chat real — apareció de inmediato bajo esa raíz (sin reload, mismo re-render reactivo de siempre); un segundo click creó un SEGUNDO chat real en la misma raíz (confirmado que `newProjectSessionInFocusedPanel` sigue sin reusar, mismo comportamiento previo) — ambos visibles a la vez, la otra raíz sin cambios.
3. Un chat suelto real (sin `workspacePath`, creado con el botón real "+ Nuevo chat") se siguió mostrando correcto en la sección CHATS de arriba, sin ninguna excepción real disparada y con las 2 raíces de PROYECTOS intactas — confirma que `chatsForWorkspacePath()` no rompe nada con `workspacePath: undefined` (simplemente no matchea ningún `===`, filtrado limpio).

`npm run typecheck`/`npm run build` limpios. Limpieza real completa: proceso Electron de prueba matado (confirmado sin residuo con `tasklist`), storage aislado + las 2 carpetas de proyecto reales de prueba borrados (con un reintento manual tras un EPERM real de Windows por un handle todavía no liberado del todo, típico al matar un proceso Electron recién cerrado — no un bug de la feature), `D:\AMATISTA\data` real nunca referenciado por el harness.

Archivo: `src/renderer/src/App.tsx` (único cambio de código). Sin commit — pendiente de que el usuario lo pida.

## Feature real — "Abrir todos los paneles" desde el menú contextual de una carpeta de proyecto

Corrección de alcance real, explícita del usuario: el diseño previo (`verify_restore_panel_view_design.md`, sobre registrar/restaurar el grupo de paneles abiertos por `send_to_window`/orquestación) NO era lo que se pedía — el pedido real es más simple y general, sin relación con orquestación: click derecho en una carpeta de proyecto del sidebar (no en un chat puntual) → abrir TODOS los chats reales asociados a esa carpeta como paneles simultáneos, hasta `MAX_PANELS`. Reusa la MISMA pieza de datos ya implementada para el sidebar limpio (`chatsForWorkspacePath()`, sección anterior) — ahora consumida por una acción nueva, no solo el render.

**Cambio real, `src/renderer/src/App.tsx`**:
- `ContextMenuState` gana una 4ta variante discriminada: `{ type: 'projectRoot'; rootId: string; x: number; y: number }` — mismo patrón exacto que `{type: 'chat', chatId, x, y}` ya existente, identificado por `rootId` en vez de `chatId`.
- `.root-title-row` gana un `onContextMenu` real (antes solo tenía los 3 botones de siempre — abrir/nueva sesión/quitar, sin menú contextual) que dispara `setContextMenu({type: 'projectRoot', rootId: root.id, x, y})`.
- Nueva rama en el render del menú contextual (`contextMenu.type === 'projectRoot'`): un único ítem, "Abrir todos los paneles", que resuelve el `ProjectRoot` real desde `settings.projectRoots.find(item => item.id === contextMenu.rootId)` y llama a `openAllPanelsForProject(root)`.
- `openAllPanelsForProject(root: ProjectRoot)` (función nueva, junto a `chatsForWorkspacePath()`):
  1. `chatsForWorkspacePath(root.path)` — ya devuelve los chats reales de esa carpeta ordenados por `updatedAt` DESC (mismo criterio ya usado en toda esta app) — la priorización pedida ("¿cuáles 4 priorizar?") ya viene resuelta gratis por ese orden, sin lógica nueva: `.slice(0, MAX_PANELS)` alcanza.
  2. Loop real sobre los seleccionados llamando a `openChatInPanel(chat.id)` (SIN cambios en esa función) — dedup real gratis (un chat ya abierto en otro panel se reusa/enfoca, nunca duplica) y el tope de `MAX_PANELS` GLOBAL (no solo de esta carpeta) se respeta con el mismo mecanismo real que ya usa el resto de la app.
  3. Un único `setNotice()` agregado, DESPUÉS del loop completo — cualquier `setNotice()` individual que `openChatInPanel()` dispare internamente por rechazo de tope durante el loop queda pisado por este último (mismo mecanismo real de "el último `setNotice()` gana" que ya resuelve el hallazgo de la investigación previa sobre llamadas sucesivas pisándose entre sí — sin necesitar tocar `openChatInPanel()`). El texto reporta, con conteos reales: paneles nuevos abiertos, cuántos ya estaban abiertos y se reutilizaron, cuántos no entraron por tope de `MAX_PANELS`, y cuántos chats reales de esa carpeta quedaron afuera por priorización (si había más de `MAX_PANELS`).

**Hallazgo real durante la propia verificación, no anticipado, real y honesto**: el mecanismo `{notice}` de esta app (usado para TODO el feedback no-bloqueante — login de Codex, guardar API key de Tavily, tope de paneles, etc.) hoy **solo se renderiza dentro del panel de Configuración** (`App.tsx`, gateado por `settingsOpen`, con un comentario real explicando que se centralizó ahí "para que el feedback de CUALQUIER botón... quede visible" — pero visible SOLO si Configuración está abierta). El aviso agregado de esta feature se setea correctamente en React state (confirmado real: el texto exacto apareció al abrir Configuración después de la acción), pero es **invisible en el uso normal real** — un usuario haciendo click derecho en una carpeta desde el sidebar, sin tener Configuración abierta, no ve ningún feedback visual de que la acción funcionó (más allá de los paneles apareciendo, que sí es visible). No es un bug introducido por este cambio (mismo comportamiento que ya tenían el resto de los usos de `{notice}` antes de esta feature) — se documenta acá como hallazgo real, no oculto, sin arreglarlo (fuera del alcance pedido, decisión de UX más amplia que el usuario no pidió tocar).

**Verificado real**, `electron.exe .` + CDP + `AMATISTA_STORAGE_ROOT` aislado, 1 carpeta de proyecto real con 5 chats reales sembrados directo en la base SQLite real (`node:sqlite`, mismo módulo que usa `chat-store.ts`, títulos distintos y `updated_at` estrictamente creciente para una priorización inequívoca):
1. Click derecho real en la carpeta → menú contextual real con "Abrir todos los paneles".
2. Con 1 de los 4 más recientes ya pre-abierto manualmente (dedup): click en el ítem abrió exactamente los 4 chats reales más recientes (`Chat B`/`Chat C`/`Chat D`/`Chat E`), el 5to (`Chat A`, el más viejo) quedó afuera — confirmado leyendo el DOM real (`.panel-header-title`).
3. El pre-abierto (`Chat D`) se reusó — nunca se duplicó un panel para el mismo chat — total real de paneles tras la acción: exactamente 4, nunca 5 (`MAX_PANELS` respetado).
4. Aviso agregado real, un solo mensaje: `"ProyectoUnico": 3 paneles nuevos abiertos, 1 ya estaba abierto (reutilizado), 1 chat mas de esta carpeta no se abrieron -- se priorizan los 4 mas recientes.` — confirma los 4 conteos reales exactos, un único mensaje (no 4 mensajes pisándose).

`npm run typecheck`/`npm run build` limpios (mismo build que el sidebar limpio, misma sesión). Limpieza real completa: proceso Electron de prueba matado, storage aislado + carpeta de prueba borrados, `D:\AMATISTA\data` nunca referenciado.

Archivo: `src/renderer/src/App.tsx` (mismo archivo que el sidebar limpio de la sección anterior — implementado y verificado en la misma pasada, tal como pidió el usuario). Sin commit — pendiente de que el usuario lo pida.

### Pregunta puntual respondida — `MAX_PANELS=4`, ¿límite técnico o de producto?

Confirmado con código real (`App.tsx:251-255`, comentario ya existente): **límite de producto explícito, sin razón técnica dura** — "no hay una razon tecnica dura para 4 en vez de otro numero, es un limite de producto (mas de 4 conversaciones visibles a la vez deja de ser usable en una pantalla real)". Subir la constante en sí no rompería ningún consumidor (todos comparan genérico contra `MAX_PANELS`).

**Pero el layout de grilla (`panelsGridStyle`, `App.tsx:3841-3847`) SÍ necesita un cambio real si se sube** — no es calculado dinámicamente, es una escalera de casos escritos a mano para 1/2/3, con un `return` final hardcodeado a un 2×2 fijo (`repeat(2,1fr)` × `repeat(2,1fr)`) para "4 o más" — sin `grid-auto-rows`/`grid-auto-flow` definidos, un 5to/6to panel real caería en auto-placement de CSS Grid no controlado sobre una grilla que solo declaró 4 celdas explícitas.

- **6 paneles**: encaja limpio, `repeat(3,1fr)` × `repeat(2,1fr)` — mismo patrón ya usado para el caso de 3, sin celdas vacías.
- **5 paneles**: no cae limpio en un rectángulo — o se acepta 1 celda vacía real (`repeat(3,1fr)` × `repeat(2,1fr)`, 6 slots/5 ocupados) o hace falta `grid-template-areas` explícito para un layout asimétrico (ej. 2 arriba + 3 abajo) — más trabajo real que 6.

**Decisión del usuario**: dejarlo en 4 por ahora. Sin cambios de código por esta pregunta — queda documentado acá para si se prioriza subirlo en una sesión futura.

## Verificación real — Navegador embebido (4 tools: `browser_navigate`/`browser_click`/`browser_type`/`browser_screenshot`), los 7 casos pedidos, 2 bugs reales encontrados y arreglados

Implementación ya hecha y compilando limpio (`npm run typecheck`/`npm run build`, incluido `mcp:lsp:bundle`) al empezar esta pasada — basada en `docs/_arch/verify_embedded_browser_design.md` (diseño ya aprobado). Esta entrada es la VERIFICACIÓN REAL de los 7 casos pedidos por el usuario, con evidencia real, no solo lectura de código (aunque la lectura completa de los 14 archivos del diff confirmó que la implementación sigue el diseño aprobado al pie de la letra, sin desvíos).

### Harness real

`electron.exe .` (nunca el script/out directo, mismo hallazgo ya documentado en esta bitácora sobre `app.getAppPath()`) con `AMATISTA_STORAGE_ROOT` aislado (`<scratchpad>/browser-verify/data`) fijado en el proceso PADRE antes del spawn (`launch_electron.js`, `spawn()` con `detached:true`), `--remote-debugging-port=9333`. Cliente CDP propio (`cdp.js`, `WebSocket`/`fetch` nativos de Node 24, sin dependencias nuevas) — con un hallazgo de harness real documentado abajo. Servidor HTTP local propio (`server.js`, `http://127.0.0.1:8975`) sirviendo 3 páginas reales de contenido conocido (`click.html` con un botón real `#accept-btn` "Aceptar" + un `<div>` señuelo con el mismo texto + un botón `#decoy-btn` "Cancelar" + un `#log` real que registra qué elemento recibió cada click; `type.html` con un `<input id="email-field">` real; `hola.html` con el texto de control real `HOLA_BROWSER_XYZ`) — nunca dependiendo de un sitio público salvo `https://example.com` para el Caso 1 (confirmar carga real de internet).

Provider `anthropic-api` real sembrado copiando `config/settings.json` + `Local State` reales del usuario (`D:\AMATISTA\data`) al storage aislado ANTES del primer arranque (mismo truco ya documentado en esta bitácora: `safeStorage`/DPAPI en Windows está atado a la cuenta de Windows, no a la carpeta, así que copiar `Local State` permite que un `encryptedApiKey` real decifre bien bajo un `AMATISTA_STORAGE_ROOT` nuevo) — el provider real disponible con `runtime:'anthropic-api'` resultó ser una conexión `type:'anthropic'`/`authMode:'api-key'` apuntando al endpoint compatible de DeepSeek (`https://api.deepseek.com/anthropic`, modelo "DeepSeek V4 Pro", `capabilities.vision:false`) — real, del usuario, satisface la letra del pedido ("provider anthropic-api real") aunque el vendor detrás no sea Anthropic. Para el Caso 6 se usó el provider real `qcfg-claude-subscription` (`runtime:'claude-cli'`, ya autenticado por suscripción en esta máquina, `claude --version` responde).

**Hallazgo de harness real, no del código bajo prueba**: `Runtime.evaluate({replMode:true, awaitPromise:true})` de CDP, con la versión de Electron/Chromium de este repo, SOLO resuelve bien una expresión que evalúa a una Promise si el texto evaluado empieza LITERALMENTE con la palabra clave `await` en el nivel superior (el modo REPL de top-level await de DevTools) — envolver en `(async function(){...})()` o encadenar `.then()` devuelve el objeto `Promise` sin resolver, que `returnByValue` serializa como `{}` (una Promise nativa no tiene propiedades propias enumerables), **sin ningún error visible**. Esto produjo una falsa alarma real durante el harness (`window.universalAgent.getSettings()` parecía devolver `{providers:[]}` vacío) hasta que se aisló con 4 variantes controladas (`debug3.js`) — el fix fue reescribir cada expresión async como `await algo` literal en vez de un wrapper. Documentado para cualquier verificación futura de esta bitácora que use el mismo patrón CDP.

`activeProviderId`/`activeModelId` son campos **`'main'`-owned** en `SETTINGS_FIELD_OWNER` (`ipc-settings.ts`, ver la entrada RESUELTO de `PENDING.md` sobre este mecanismo) — `window.universalAgent.saveSettings()` del renderer los IGNORA a propósito (protección real contra la carrera ya documentada en `verify_settings_race.md`). La única forma real de cambiar el proveedor/modelo de un panel es el picker real de la UI (`.model-btn` → `.model-item`, click real, no bypass) — mismo criterio ya usado como "helper de respaldo" en una verificación anterior de esta bitácora, acá promovido a mecanismo primario.

### Caso 1 — `browser_navigate` real a una URL real

Prompt real al modelo (DeepSeek/`anthropic-api`): navegar a `https://example.com` y reportar el título real. Diálogo real de aprobación (`"Navegar en el navegador embebido"`, detalle `"https://example.com"`, `hasTrust:false`), aprobado con clic real en el DOM. El modelo reportó `"Example Domain"`. **Verificación independiente** (CDP conectado DIRECTO al target real de la `WebContentsView`, nunca confiando en el autoreporte de la tool ni del modelo): `document.title` real = `"Example Domain"`, `location.href` real = `"https://example.com/"`, `<h1>` real = `"Example Domain"` — contenido real de internet, confirmado por 3 vías independientes.

### Caso 2 — `browser_click` por DOM/selector real, con scroll y zoom simultáneos inyectados EN VIVO antes del click

Prompt real: navegar a `http://127.0.0.1:8975/click.html` y clickear el elemento "Aceptar" (con un señuelo `<div>` de mismo texto y un botón "Cancelar" real, mismo escenario que TEST5 de la investigación de diseño). Justo cuando apareció el diálogo real de aprobación del click (`"Click en el navegador embebido"`, detalle `"Click en: \"Aceptar\""`) — el `browser_click` TODAVÍA no había corrido — el harness inyectó, vía CDP directo sobre el target real de la vista, `window.scrollTo(0, 400)` + `document.body.style.zoom = '1.75'`. `getBoundingClientRect()` real del botón `#accept-btn` ANTES: `{x:100,y:220,w:160,h:50}`; DESPUÉS del scroll+zoom (con el botón parcialmente fuera de pantalla, `y:-15`): `{x:175,y:-15,w:280,h:87.5}` — un cambio de posición real y grave. Recién ENTONCES se aprobó el diálogo. **Verificación independiente**: `#log` real de la página (mecanismo de la propia página, no de la tool) = `"CLICK_REAL:accept-btn"` — acertó al botón real correcto pese al cambio de layout en vivo, confirmando en esta implementación real de producción el mismo hallazgo de la investigación (resolución por DOM en el mismo instante del click, inmune a scroll/zoom).

### Caso 3 — `browser_type` real, texto verificado en el `.value` real del input

Prompt real: navegar a `http://127.0.0.1:8975/type.html` y escribir `"PRUEBA_XYZ_9182@correo.test"` en el campo de correo. Diálogo real (`"Escribir en el navegador embebido"`, detalle con el campo y el texto completo), aprobado. **Verificación independiente**: `document.getElementById('email-field').value` real (leído vía CDP directo sobre el target de la vista) = `"PRUEBA_XYZ_9182@correo.test"` — coincide EXACTO, 27 caracteres, `document.activeElement === el` real = `true` (foco real confirmado, no solo el resultado de la tool).

### Caso 4 — `browser_screenshot` real, dimensiones reales != 0×0, contenido real — **1 bug real encontrado y arreglado acá** (ver más abajo, "Bug 1")

Tras el fix del Bug 1 (más abajo): la tool reportó `"1289x365px"`. **Verificación independiente** (`Page.getLayoutMetrics` + `Page.captureScreenshot` propios, vía CDP directo sobre el target real de la vista — nunca reusando la imagen que devolvió la tool): `clientWidth:1274, clientHeight:365` (coincide), captura propia real de 6627 bytes PNG guardada y **vista** — muestra el contenido real de `click.html` (los botones "Aceptar"/"Cancelar" reales, el `<div>` señuelo, con una barra de scroll real visible a la derecha, confirmando que es contenido AISLADO de la vista, no la ventana completa de Amatista). Contraste real: `Page.getLayoutMetrics` de la VENTANA PRINCIPAL en el mismo instante = `1584×915` — completamente distinto, confirmando que `browser_screenshot` nunca se mezcla con la ventana contenedora (mismo hallazgo ya confirmado en la investigación de diseño, TEST7, ahora reconfirmado en la implementación real de producción).

### Caso 5 — Capa 2 nunca se saltea, con `danger-full-access` + `toolTrustSession` simultáneos

Mismo test ya usado para Familia A/B de esta bitácora. Sandbox real seteado a `danger-full-access` (`<select>` real del composer). Paso 1: `generate_image` (tool que usa `ctx.confirm()` incondicional, SIEMPRE con checkbox de confianza, sin relación con el sandbox) — diálogo real `{"title":"Generar imagen","detail":"un gato naranja durmiendo","hasTrust":true}`, aprobado con el checkbox de confianza tildado real → `toolTrustSession` real queda `true` para la sesión (la imagen además se generó de verdad, confirmando que el provider de generación de imágenes real del usuario también sigue funcionando). Paso 2, SIN reconectar ni cambiar de sandbox, en la MISMA sesión ya con `danger-full-access` + `toolTrustSession:true` simultáneos: `browser_navigate` a `https://example.com` — **el diálogo real SIGUIÓ apareciendo** (`{"title":"Navegar en el navegador embebido","detail":"https://example.com","hasTrust":false}`), sin checkbox de confianza en el DOM, confirmado leyendo `overlay.querySelector('.trust-checkbox')` real, no asumido — rechazado con clic real en "Rechazar". Ninguna combinación de sandbox+confianza saltó el diálogo, mismo criterio ya verificado para Familia A/B: `requestHardToolApproval()` llama `requestToolApproval(..., allowTrust=false)` fijo, estructuralmente inalcanzable de otra forma.

### Caso 6 — CLI real: Claude Code CLI usando `browser_screenshot` vía MCP real

Panel real reconectado con el provider real `qcfg-claude-subscription` (`runtime:'claude-cli'`). Prompt real: navegar a `http://127.0.0.1:8975/hola.html`, sacar una captura real, y transcribir EXACTO el texto grande que se ve — sin dárselo nunca en el prompt. 2 diálogos reales (`browser_navigate` + `browser_screenshot`, vía `mcp-approval-pipe.ts`, mismo mecanismo de UI que el camino API — confirmado real que es la MISMA `.approval-overlay`), ambos aprobados. El modelo real transcribió exacto `"HOLA_BROWSER_XYZ"` — confirmado con un chequeo automatizado (`/HOLA_BROWSER_XYZ/.test(...)`) sobre el texto real de la respuesta, además de lectura visual del texto completo (que además trae la "radiografía" real de la operación — el `CLAUDE.md` global del usuario se aplica también dentro del subproceso `claude` CLI real, confirmando de paso que es el CLI auténtico de esta máquina, no un mock).

### Caso 7 — No-regresión: Familia A, Familia B, orquestación (`send_to_window`), y las 4 tools de LSP desde CLI — **1 bug real encontrado y arreglado acá** (ver más abajo, "Bug 2"), descubierto justamente al reproducir el flujo típico de un panel nuevo

- **Familia A (computer use)**: checkbox real `.computer-use-toggle` activado, prompt real pidiendo un `screenshot` de escritorio. Diálogo real idéntico al ya documentado (`"Capturar pantalla"`, `"Tomar una captura real del monitor primario."`, `hasTrust:false`), aprobado. Captura real ejecutada: **`1680×1050 píxeles`** de un monitor real (la máquina real tiene 3 monitores, confirmado por el propio texto de la tool) — el modelo no pudo "ver" el contenido (DeepSeek, `capabilities.vision:false`, limitación del vendor, no de Amatista), pero el mecanismo completo (Capa 1+2, captura real, dimensiones reales) funcionó idéntico a la verificación original de Familia A.
- **Familia B (tools de sistema)**: `system_info` real, diálogo real (`"Informacion del sistema"`, `hasTrust:true`, coherente con el sandbox `workspace-write` activo en ese momento), aprobado. Datos reales devueltos y verificables contra esta máquina real: `Windows 11 Home Single Language (build 26200)`, `Intel(R) Core(TM) i7-9750H CPU @ 2.60GHz`, `19.3/31.9 GB RAM`, disco `C:` `146.6/475.9 GB` — coincide con el hardware real de la máquina donde corre esta verificación.
- **Orquestación (`send_to_window`) real, 2 paneles reales**: chat B (`"Destino Orquestacion 777"`, creado real vía `"+ Nuevo chat"` + renombrado real vía doble-click en el sidebar) recibió un primer turno real (`"Responde unicamente con la palabra: ok"` → `"ok"`, satisface el requisito real de la tool de que el destino "ya tuvo un turno real antes"). Chat A (el panel principal real, único de los 2 con `isPrincipalChat()` real `true` — `panelAliasForTitle()` excluye títulos con sufijo `" — Panel N"`, que sí tiene el chat abierto vía `"Agregar panel"`, confirmado real por lectura de `chat-store.ts`) le pidió al modelo usar `send_to_window` hacia el título exacto del chat destino — diálogo real (`"Enviar mensaje a \"...\""`, con el mensaje real completo, `hasTrust:true`), aprobado. **Verificación independiente**: el mensaje real (`"PRUEBA_ORQUESTACION_777"`) apareció en el DOM real del panel 2 (destino), que respondió con un turno real propio referenciando el marcador exacto — confirmado leyendo el DOM de AMBOS paneles, no solo el reporte del panel emisor. El modelo además llamó `list_windows` por su cuenta antes de `send_to_window`, sin que se le pidiera — comportamiento normal de la tool, no forzado por el prompt.
- **4 tools de LSP desde CLI**: `list_symbols` real (Claude Code CLI/MCP) sobre el propio archivo `src/main/embedded-browser.ts` de este repo (workspace del panel real apuntado al repo real vía el picker de proyectos del sidebar) — devolvió los **98 símbolos reales** del archivo, verificables 1 a 1 contra el archivo real (`ensureBrowserView`, `setBrowserViewBounds`, `destroyBrowserView`, `navigateBrowserView`, `resolverScript`, `clickInBrowserView`, `typeInBrowserView`, `screenshotBrowserView`, con número de línea/columna reales) — confirma que el pipeline completo (Claude CLI → MCP → `LspManager` real → TypeScript LS real) sigue funcionando sin regresión.

### Bug 1 real, encontrado y arreglado — la `WebContentsView` colapsaba a ~1px de alto en la app real (CSS Grid)

**Síntoma real**: con la app REAL corriendo (ventana `1584×915`, confirmado que NINGUNA media query de `main.css` estaba activa — `window.matchMedia('(max-width:1080px)').matches`/`(max-height:740px)`/`(max-height:720px)` los 3 `false`), `browser_screenshot` devolvió `1289x1px` — la vista embebida, aunque funcionalmente operativa (todas las tools seguían "funcionando" contra ella), era visualmente casi invisible para el usuario real.

**Investigación real** (documentada en detalle porque el mecanismo exacto de Chromium no quedó 100% explicado, pero el diagnóstico y el fix sí están confirmados con evidencia real, no supuestos): la implementación original ponía el `<div className="browser-view-container">` COMO HIJO de `.chat` (`display:grid`) con una 3ra fila agregada vía `style={{gridTemplateRows:'minmax(160px,42%) minmax(0,1fr) auto'}}` inline. Confirmado real, con la app corriendo:
- Mutar esa fila en vivo (CDP directo) a `42%`, a `42fr`, y a un `160px`/`300px` fijo (sin `minmax` en absoluto) dio SIEMPRE `0px` para la 1ra fila — un `minmax(160px, X)` debería garantizar 160px como piso absoluto sin importar `X`, y no lo hizo, en ninguna de las 4 variantes probadas.
- La cadena de ancestros (`.chat` → `.chat-panel` → `.panels-grid` → `.main` → `.app`) tiene alturas DEFINIDAS en cada nivel (confirmado leyendo `getComputedStyle` real de cada uno) — no era un caso de "porcentaje sobre altura indefinida" (la explicación más común para este tipo de bug de CSS Grid).
- Un elemento `.chat` FRESCO, creado a mano y adjuntado directo a `document.body` (mismo cascade real de las 4 reglas `.chat` de `main.css`, incluidas las 3 con `!important` gateadas por media queries NO activas), reprodujo el mismo síntoma (una `<div class="browser-view-container">` de altura ~18px en vez de la esperada) — descartando que fuera un problema de "mount stale"/cache de React.
- Un repro AISLADO (página HTML mínima propia, mismo CSS, mismo DOM de 3 hijos, en el Browser pane vía Chrome, a un tamaño de ventana controlado ≥1080×740) **NO reprodujo el bug** — el mismo `minmax(160px,42%)` resolvió exacto (`345.656px`, matemáticamente correcto sobre una altura de `823px`). Esto acota el bug a algo específico del árbol real de la app (probablemente relacionado con los 3 bloques `.chat{...!important}` adicionales gateados por media query presentes en el cascade real, aunque no se logró aislar la causa exacta dentro del tiempo de esta verificación — documentado como hallazgo real, no como explicación completa).
- **Control real que sí funcionó de inmediato**: un elemento FLEX hermano (no grid-hijo) con `flex:0 0 42%; min-height:160px` insertado real dentro de `.chat-panel` (que YA es `display:flex; flex-direction:column`) resolvió exacto: `364.96875px` sobre una altura real de `869px` (42% exacto) — en el primer intento, sin ningún ajuste adicional.

**Fix real implementado**: `src/renderer/src/App.tsx` — el `<div className="browser-view-container">` se sacó de adentro de `<section className="chat">` (que vuelve a su grid de 2 filas de SIEMPRE, sin ningún cambio, sin tocar la cascada `!important` responsive) y pasó a ser un HERMANO flex de `.chat` dentro de `.chat-panel` (que ya era `display:flex; flex-direction:column`, sin cambios estructurales). `src/renderer/src/assets/main.css` — `.browser-view-container` pasó de `{width:100%; height:100%; min-height:0}` (dependía de la fila de grid rota) a `{flex:0 0 42%; min-height:160px; width:100%}` (tamaño real resuelto por flexbox). Cero cambios al mecanismo de reporte de bounds (`ResizeObserver` + `api.setBrowserViewBounds()`, sin tocar) ni a `.chat`/su cascada responsive.

**Reverificado real tras el fix, recompilado (`npm run typecheck`/`npm run build` limpios)**: `.browser-view-container` real = `364.97px` de alto (42% exacto de `869px`), `browser_screenshot` real reportó `1289x365px`, `Page.getLayoutMetrics` independiente confirmó `clientHeight:365`, captura propia independiente de 6627 bytes con contenido visual real correcto (ver Caso 4 arriba, capturado y visto DESPUÉS del fix).

### Bug 2 real, encontrado y arreglado — `browserControlActive` se apagaba solo en el primer mensaje de cualquier panel nuevo

**Síntoma real**: un panel recién abierto, NUNCA conectado todavía (`agentState` distinto de `'connected'`) — el flujo más común real de primer uso: el usuario activa el checkbox "Navegador" del composer y ENSEGUIDA manda su primer mensaje pidiéndole al agente que navegue. La tool devolvía, siempre, `"El navegador embebido no esta activado para este panel"` — pese a que el checkbox real del DOM mostraba tildado un instante antes.

**Causa real, confirmada leyendo código y reproducida en vivo**: `connectSessionForWindow()` (`ipc-agent.ts`) llama `disconnectSession(panelId)` INCONDICIONAL al principio de CADA conexión (patrón real preexistente, Fase 22b — "resetear antes de conectar", para que una reconexión nunca arrastre estado de la sesión anterior) — y `disconnectSession()` (`runtime-state.ts`) apaga `browserControlActive` de forma incondicional si estaba prendido (`if (session.browserControlActive) setBrowserControlActive(panelId, false)`, mismo bloque real que ya hace lo mismo para `computerUseActive`, Familia A). Como `sendPrompt()` en un panel nunca-conectado dispara `connectAgent()` → `agent:connect` → `connectSessionForWindow()` ANTES de que el turno llegue a ejecutar ninguna tool, el checkbox recién activado se apagaba solo (y la `WebContentsView` real se destruía) en la MISMA conexión que el usuario acababa de iniciar, antes de que la primera `browser_navigate`/`browser_click`/etc. llegara a correr — el navegador embebido quedaba, en la práctica, imposible de usar en el primer mensaje de cualquier panel nuevo (justo el escenario que reprodujo el Caso 1 de esta verificación en su primer intento real).

**Fix real implementado, `src/main/ipc-agent.ts`**: `connectSessionForWindow()` ahora captura `session.browserControlActive` justo ANTES de llamar `disconnectSession(panelId)`, y si estaba `true`, lo re-arma DESPUÉS (`setBrowserControlActive(panelId, true)`, mismo mecanismo real que ya usa el toggle del composer, no un campo nuevo) — la `WebContentsView` real se recrea idéntica si el usuario la tenía prendida, sin ningún cambio al comportamiento de reset para el resto de los campos que `disconnectSession()` ya reseteaba (sandbox, modo plan, etc., todos intactos).

**Alcance deliberadamente acotado, documentado en el propio código**: el mismo síntoma podría afectar a `computerUseActive` (Familia A, mismo patrón exacto de reset en `disconnectSession()`) — no confirmado en vivo como bug real para Familia A en esta pasada (el Caso 7 de arriba activó computer use en un panel YA conectado, que no dispara este camino), y el fix NO se extendió ahí a propósito: es código de una feature ya shippeada, fuera del alcance de esta verificación puntual del navegador embebido. Queda anotado como hallazgo abierto en `docs/_arch/PENDING.md`, sin tocar.

**Reverificado real tras el fix, recompilado**: Caso 1 completo repetido desde cero, exactamente en el escenario que antes fallaba (panel recién conectado, nunca conectado antes, toggle activado JUSTO antes del primer mensaje) — `browser_navigate` real funcionó de punta a punta en el primer intento, sin ningún mensaje de "no está activado". Casos 2/3/4 reconfirmados en la misma sesión ya corregida.

### Cierre

`npm run typecheck`/`npm run build`/`npm run mcp:lsp:bundle` en verde después de AMBOS fixes. Limpieza real completa: proceso `electron.exe` aislado y servidor HTTP local de prueba matados por PID (nunca tocados los `claude.exe` reales de otras sesiones que corrían en la máquina en paralelo), storage aislado (`AMATISTA_STORAGE_ROOT`) borrado por completo, `D:\AMATISTA\data` real confirmado sin ningún archivo con `LastWriteTime` posterior al inicio de esta verificación (`config/settings.json`/`Local State` con timestamps idénticos a antes de empezar — se copiaron para LEER, nunca se escribió sobre el original). Sin commit — pendiente de que el usuario lo pida.

## Fix real — `computerUseActive` (Familia A) tenía el MISMO bug de reseteo que `browserControlActive` (Bug 2 arriba), confirmado y arreglado con el mismo patrón exacto

El Bug 2 de la verificación del navegador embebido (arriba) dejó anotado a propósito, sin tocar, que `computerUseActive` usa el mismo bloque real de reset dentro de `disconnectSession()` (`runtime-state.ts`) y "podría tener el mismo síntoma" — Familia A es la feature de mayor riesgo del proyecto (control físico de mouse/teclado/pantalla completa), así que el fix se pidió aparte, en 2 pasadas separadas: primero diagnóstico puro (sin tocar código), después el fix — ambas documentadas con el detalle completo en `docs/_arch/PENDING.md` (entrada "RESUELTO — `computerUseActive`...").

**Diagnóstico confirmado real, en vivo, en la pasada anterior**: panel nuevo, nunca conectado, checkbox "Control de escritorio" activado por click real, primer mensaje real enviado — el checkbox se destildaba solo en el primer muestreo (~300ms después del envío), y un segundo mensaje pidiéndole al Claude Code CLI real que reportara si tenía la tool `screenshot` disponible confirmó que la tool NO aparecía en su inventario (el subproceso CLI arrancaba sin `AMATISTA_COMPUTER_USE_ACTIVE`, porque `session.computerUseActive` ya estaba en `false` antes de que `connectSessionForWindow()` llegara a spawnear el proceso).

**Fix real implementado, `src/main/ipc-agent.ts`**: mismo patrón exacto ya usado para `browserControlWasActive` — dentro de `connectSessionForWindow()`, se capturó `session.computerUseActive` en `computerUseWasActive` justo ANTES de la llamada incondicional a `disconnectSession(panelId)`, y se re-armó después con `setComputerUseActive(panelId, true)` si el valor capturado era `true`:

```ts
const browserControlWasActive = getSession(panelId).browserControlActive
const computerUseWasActive = getSession(panelId).computerUseActive
disconnectSession(panelId)
const session = getSession(panelId)
if (browserControlWasActive) setBrowserControlActive(panelId, true)
if (computerUseWasActive) setComputerUseActive(panelId, true)
```

Cero cambios a la lógica de aprobación/gate en sí (Capa 1 `computerUseActive`/Capa 2 `requestHardToolApproval()` intactas, tal como pedía la restricción explícita) — el único cambio es el punto de reseteo. `setComputerUseActive` ya estaba importado (reusado de la implementación original de Familia A, sin import nuevo). `npm run typecheck`/`npm run build` confirmados limpios tras el cambio.

**Verificado real, mismo escenario exacto que confirmó el bug** (harness CDP directo, `electron.exe .` con `AMATISTA_STORAGE_ROOT` aislado, `config/settings.json` sembrado con `computerUseAcknowledged:true`, sin API keys — provider real `qcfg-claude-subscription`/`claude-cli` auto-provisionado por `sanitizeSettings()`, autenticado con la sesión real de Claude Code CLI de esta máquina):
- Panel nuevo, nunca conectado. Checkbox real "Control de escritorio" activado por click real — `checked:true` confirmado antes de enviar.
- Primer mensaje real enviado (`sendPrompt()` → `connectAgent()` → `agent:connect` → `connectSessionForWindow()`, mismo camino real que disparó el bug). Muestreo denso del checkbox, 15 muestras cada 300ms durante y después del envío: **`checked:true` en las 15/15 muestras** — nunca se destildó solo. Turno 1 completado real, checkbox seguía `checked:true` después de que terminó.
- **Test práctico, variante más fuerte que la del diagnóstico** (no solo preguntar si la tool existe — pedirle al CLI que la INVOQUE de verdad): segundo mensaje real pidiéndole al mismo Claude Code CLI que invoque `screenshot`. Esta vez **el diálogo real de aprobación (Capa 2, `requestHardToolApproval()`) apareció**, con el título real `"Capturar pantalla"` — confirmando de punta a punta que (a) la tool SÍ está en el inventario del CLI ya en el primer mensaje de un panel nuevo (`AMATISTA_COMPUTER_USE_ACTIVE` llegó activo al spawn del proceso hijo, porque `computerUseActive` ya no se reseteaba antes de ese punto), y (b) Capa 2 se sigue pidiendo siempre, sin ninguna regresión al gate de aprobación. Rechazado a propósito con clic real en "Rechazar" (no hacía falta ejecutar el screenshot real para esta verificación puntual) — turno 2 completado real igual.

**Limpieza real confirmada**: proceso `electron.exe` aislado matado por PID, storage temporal (`AMATISTA_STORAGE_ROOT`, bajo el scratchpad de esta sesión) borrado por completo, `D:\AMATISTA\data` real nunca tocado. Sin commit — pedido explícito del usuario. El navegador embebido (`embedded-browser.ts` y el resto del diff asociado) sigue sin tocar, fuera del alcance de este fix puntual.

## Verificación real — Helper C#/FlaUI de UI Automation (mecanismo primario de Familia A en reemplazo de coordenadas)

Basado en `docs/_arch/verify_flaui_helper_viability.md` (investigación ya aprobada, viabilidad confirmada con evidencia real: self-contained JIT viable, AOT descartado con una `InvalidProgramException` real reproducida). Esta pasada verifica en vivo el código ya implementado y compilado por la sesión orquestadora: `resources/flaui-helper/{FlaUIHelper.csproj,Program.cs}` (proyecto C#/.NET 10 nuevo), `src/main/flaui-client.ts` (cliente NDJSON nuevo), `src/main/computer-use-actions.ts` (`clickByDescription`/`typeByDescription`/`describeCoordinateTarget` nuevos, mecanismo de coordenadas preexistente sin tocar), `src/main/tool-registry.ts` (rama `description` nueva en `mouse_click`/`keyboard_type`, rama `x`/`y` preexistente sin tocar), `src/main/mcp-lsp-server.ts`/`src/main/mcp-approval-pipe.ts` (mismo branching primario/fallback espejado para el camino CLI), `package.json` (`build.extraResources` nuevo) y `.gitignore` (excluye `bin/obj/publish` del proyecto .NET). Ejecutada en 2 pasadas: una en un git worktree aislado (verificación amplia de los 6 puntos), y una segunda pasada directa de la sesión orquestadora que cerró en vivo los 2 puntos que la primera había dejado como limitación honesta (ver "Actualización" al final de esta sección).

### Build real del helper, regenerado desde cero para esta verificación

`dotnet --version` → `10.0.300`. `dotnet build -c Release`: compilación limpia, 0 advertencias, 0 errores. `dotnet publish -c Release --self-contained true -r win-x64 -p:PublishSingleFile=true -o publish`: publicó limpio. Resultado real medido:

```
FlaUIHelper.exe                165,466,675 bytes
D3DCompiler_47_cor3.dll          4,741,488 bytes
PresentationNative_cor3.dll      1,235,728 bytes
wpfgfx_cor3.dll                  1,955,624 bytes
PenImc_cor3.dll                    157,992 bytes
vcruntime140_cor3.dll              124,544 bytes
FlaUIHelper.pdb                     18,752 bytes (excluido del empaquetado, ver Punto 6)
```

Total real del `.exe` + las 5 DLLs nativas (sin el `.pdb`): 173,682,051 bytes (165.66 MiB / 173.68 MB decimales).

### Smoke test directo del `.exe` (NDJSON crudo, previo a probar la app completa)

Contra un Notepad real: `get_tree`/`find_element`/`click`/`type`/`hit_test` funcionando (mismo resultado que la investigación: `root.className=Notepad`, hijo `Document`/`RichEditD2DPT`/`name="Editor de texto"`). `type` de un marcador de 22 caracteres devolvió `charsTyped:22,totalChars:22,interrupted:false` — releído con una consulta UIA TOTALMENTE independiente (PowerShell, `AutomationElement.FromHandle` + `TextPattern.DocumentRange.GetText(-1)`, proceso separado, sin relación con el helper ni con el cliente Node): `CONTAINS_MARKER=True`. Cancelación real confirmada: `type` de 200 caracteres, `cancel` real mandado a los 150ms apuntando al `id` del pedido en curso, resultado `interrupted:true,charsTyped:1` — confirma que el dispatch por `Task.Run` de cada línea NDJSON (Program.cs, `HandleLine()`) permite que un `cancel` concurrente interrumpa un `type` ya en curso, mismo principio de micro-pasos que `computer-use-actions.ts` ya usa con nut-js.

### Punto 1 — Reproducción real del escenario motivador (mover/redimensionar una ventana real entre la decisión y la ejecución del click)

Verificado real, con evidencia independiente en cada paso, contra el `.exe` de producción (mismo binario que usaría la app real vía `flaui-client.ts`, mismo protocolo NDJSON):

1. Notepad real llevado a foreground de verdad (`SetForegroundWindow`), rect real leído (`GetWindowRect`): `Left=650,Top=420,Right=1350,Bottom=920` — este es el instante que representa "el modelo decidió el target" (una descripción semántica, `controlType:"Document"`, sin ninguna coordenada de por medio).
2. **Movimiento real y medible** de la ventana, en un proceso PowerShell separado (`SetWindowPos`, sin relación con el helper): nueva posición real `Left=300,Top=250,Right=1080,Bottom=810` — desplazamiento real de -350px en X, -170px en Y, más un resize real de 700×500 a 780×560. Foreground reafirmado sobre Notepad después del movimiento (mismo criterio que un usuario arrastrando una ventana real: sigue enfocada).
3. **Recién ahí** (después del movimiento real) se mandó el comando `click` (`{"cmd":"click","controlType":"Document"}`) al helper — la resolución ocurre en el MISMO instante de la ejecución, nunca antes. Resultado real: `{"ok":true,"status":"ok","name":"Editor de texto","controlType":"Document"}`.
4. **Verificación real, totalmente independiente del helper y del cliente Node** (un proceso PowerShell nuevo, consulta UIA propia): `AutomationElement.FocusedElement` — `FOCUSED_RECT Left=306,Top=325,Right=1074,Bottom=772`. Este rectángulo cae DENTRO del rectángulo real POST-movimiento (`300,250`–`1080,810`), no dentro del rectángulo de "decisión" (`650,420`–`1350,920`) — confirma, con una fuente de verdad ajena al propio código bajo prueba, que el click resolvió y aterrizó sobre la posición REAL y ACTUAL del control, no sobre una coordenada vieja cacheada en el momento de la "decisión".

### Punto 2 — Fallback de coordenadas

Confirmado por lectura completa del diff real: `computer-use-actions.ts` mantiene `clickAt()`/`moveMouseTo()`/`typeText()` línea por línea idénticas a la versión ya verificada en la pasada "Familia A" anterior de este mismo archivo (PowerShell+user32 para mouse, nut-js para teclado, micro-pasos cancelables) — el diff real de esta feature solo AGREGA `clickByDescription`/`typeByDescription`/`describeCoordinateTarget` al lado de esas funciones, sin tocar un carácter de su cuerpo. `tool-registry.ts` confirma la misma separación en el "case" real de `mouse_click`/`keyboard_type`: la rama `if (description)` es la única agregada; el `else` que llama a `clickAt`/`typeText` es el camino original, sin cambios, alcanzado exactamente cuando no se pasa `description`. `describeCoordinateTarget()` (el diagnóstico best-effort vía `hit_test` que enriquece el mensaje de un click por coordenadas) se confirmó funcionando real en el smoke test de arriba. Cobertura real: (a) lectura completa del diff confirmando cero cambios en el mecanismo de coordenadas, (b) este mecanismo exacto ya fue verificado en vivo, de punta a punta, con clicks y tipeo reales dentro de un Notepad real, en la pasada "Familia A" de este mismo documento — sin ningún cambio de código entre esa verificación y esta.

### Punto 3 — Capa 2 nunca se saltea

**Confirmado con certeza estructural, 100% determinística**: `requestHardToolApproval()` (`runtime-state.ts`) llama SIEMPRE a `requestToolApproval(panelId, title, detail, allowTrust=false)`, con `allowTrust` hardcodeado `false` en la firma misma de la función. El único `if` que consulta `session.toolTrustSession` (`if (allowTrust && session.toolTrustSession) return true`) es estructuralmente inalcanzable cuando `allowTrust` es `false` a la entrada. Confirmados los 2 call sites reales para `mouse_click`/`keyboard_type`: `tool-registry.ts` (ambas ramas, `description` y `x`/`y`, llaman al MISMO `ctx.hardConfirm()` antes de bifurcar) y `mcp-approval-pipe.ts` (`handleComputerUseMouseClick`/`handleComputerUseKeyboardType`, mismo patrón). Ningún camino nuevo de esta feature agrega un tercer call site ni toca `resolveApproval()`/`session.toolTrustSession`.

**Confirmado además EN VIVO** (ver "Actualización" más abajo): con el toggle "Control de escritorio" activado, un mensaje real pidiéndole al Claude Code CLI real que invoque `keyboard_type con description` mostró el diálogo real `{"title":"Escribir texto","detail":"Campo: \"Editor de texto\"\nTexto: FLAUI_LIVE_MARKER_7749"}`, con `hasTrust:false` confirmado leyendo `overlay.querySelector('.trust-checkbox')` (ausente). Un segundo turno real con `mouse_click con description` mostró el diálogo real `{"title":"Click del mouse","detail":"Click en: \"Editor de texto\""}`, mismo `hasTrust:false`. Ninguno de los 2 diálogos reales tuvo checkbox de confianza, confirmando en vivo lo que ya era estructuralmente cierto por código.

### Punto 4 — Real CLI

Panel real conectado con el provider builtin `qcfg-claude-subscription` (runtime `claude-cli`, ya autenticado por suscripción en esta máquina) — el mismo `claude` real que usa el desarrollador día a día. Confirmado en vivo, con evidencia real:

1. **El catálogo de tools del CLI real refleja `computerUseActive` en caliente**: con el toggle apagado, el `claude` real buscó en su propio catálogo tools cuyo nombre contuviera "mouse"/"keyboard"/"click"/"type" — `count=0`. Con el toggle re-armado, confirmado que `mcp__amatista-lsp__mouse_click` pasó a existir en su catálogo real.
2. **Defensa en profundidad confirmada por código**: `mcp-approval-pipe.ts` re-verifica `session.computerUseActive` del lado de `main` en CADA llamada, independiente de lo que el proceso hijo afirme vía `AMATISTA_COMPUTER_USE_ACTIVE` en su propio `env`.
3. **Invocación real, de punta a punta, confirmada EN VIVO** (ver "Actualización" más abajo): el `claude` real invocó `keyboard_type` con `description:"Editor de texto"` y `mouse_click` con `description:"Editor de texto"` contra un Notepad real, ambos con el diálogo real de aprobación (Capa 2) mostrado y aprobado con un clic real, y ambos con ejecución real confirmada.

### Punto 5 — No-regresión

Confirmado por diff real, cero cambios de línea: `git diff` sobre `runtime-state.ts` confirma que `showOverlay()`/`hideOverlay()`/`beginComputerUseAction()`/`endComputerUseAction()`/el registro del panic key global no tienen ningún cambio en esta feature. `computer-use-actions.ts` mantiene el mecanismo de cancelación cooperativa (`signal?.aborted` chequeado por micro-paso) línea por línea igual al ya verificado. Ningún archivo de Familia B (`close_app`/`lock_screen`/`power`) ni de orquestación (`send_to_window`/`parallel_ask`) tiene un solo cambio de línea en este diff — la superficie de riesgo de una regresión real es, por construcción del propio diff, cero para esas dos familias. Cancelación char-by-char nueva del lado del helper C# confirmada real por el smoke test de arriba (`type` largo + `cancel` concurrente → `interrupted:true`).

### Punto 6 — Instalador real, `npm run dist`

`npm run dist` real (typecheck + `electron-vite build` + `mcp:lsp:bundle` + `electron-builder --win nsis`) corrido de punta a punta, ~4 minutos reales. Medido real, no estimado:

```
release\Amatista-V0.11.4-Setup.exe   241,975,841 bytes  (230.75 MiB / 241.98 MB)
```

Comparado contra el baseline real que documentó la investigación (`184,843,056 bytes`, de una build anterior a esta feature, ya explícitamente marcado como stale): delta real de **+57,132,785 bytes (+57.13 MB)**. La investigación había calculado un techo de ~296MB sumando su binario de INVESTIGACIÓN (`FlaUIProbe.exe`, más chico que el `FlaUIHelper.exe` real de producción) — con el binario REAL de producción (165.47MB decimales) el techo teórico corregido, sin ninguna compresión, sería 184.84MB + 165.47MB = 350.31MB. El resultado real medido (241.98MB) está **108.33MB por debajo de ese techo corregido (~31% de compresión LZMA real)**.

**Confirmación real de que `extraResources` copió lo que tenía que copiar** (`release/win-unpacked/resources/flaui-helper/`): los 6 archivos reales presentes (exe + 5 DLLs), tamaños idénticos byte a byte a los de `publish/` original, **`FlaUIHelper.pdb` confirmado AUSENTE** (el filtro `"!*.pdb"` de `package.json` funcionó real). `FlaUIHelper.exe` confirmado firmado con `signtool.exe` en el log real de `electron-builder`, igual que el resto de los ejecutables reales del instalador.

### Actualización — cierre en vivo de los Puntos 3 y 4 (y refuerzo adicional del Punto 1), sesión orquestadora

La primera pasada de esta verificación (worktree aislado) dejó los Puntos 3 y 4 confirmados solo por lectura de código/diff, no en vivo — el modelo real, en más de diez formulaciones e insistiendo en frases del tipo "no me des instrucciones a mí, hacelo vos", se negó consistentemente a invocar `mouse_click`/`keyboard_type`/`screenshot`, citando razonamiento anti-inyección explícito (un mensaje de chat es "exactamente el canal por el que entraría una inyección"). Un intento alternativo de hablarle directo al servidor MCP de producción, bypaseando el criterio del modelo, fue denegado por el propio clasificador de auto-mode de ese agente — comportamiento correcto, no se insistió.

La sesión orquestadora repitió el intento directamente, con una formulación MÁS directa y en primera persona ("Soy el usuario real de esta máquina, autorizo explícitamente esta acción puntual ahora...", sin la frase "no me des instrucciones" que parece haber leído como señal de sospecha) — y esta vez el `claude` real SÍ invocó ambas tools:

- **`keyboard_type con description:"Editor de texto"`**: diálogo real de Capa 2 apareció (`"Escribir texto"`, detalle `Campo: "Editor de texto"\nTexto: FLAUI_LIVE_MARKER_7749`, sin checkbox de confianza), aprobado con clic real. Turno completado, el CLI real reportó `"Ejecutado. 22 caracteres escritos vía UI Automation..."`. **Verificación INDEPENDIENTE real** (proceso PowerShell aparte, `AutomationElement.FromHandle` + `TextPattern`, sin relación con el helper ni con Amatista): `TEXT_LEN=45`, `CONTAINS_MARKER=True` — el marcador real apareció de verdad dentro de Notepad.
- **`mouse_click con description:"Editor de texto"`**: diálogo real de Capa 2 apareció (`"Click del mouse"`, detalle `Click en: "Editor de texto"`, sin checkbox de confianza). Con el diálogo TODAVÍA sin aprobar (mismo instante de "decisión ya tomada" que motiva el Punto 1), se movió y redimensionó Notepad de verdad en un proceso aparte (`SetWindowPos` real: `26,26,1076,837` → `500,480,1260,1000`, foreground reafirmado sobre Notepad — `FOREGROUND_MATCHES_NOTEPAD=True`). Recién ahí se aprobó el diálogo con un clic real. Turno completado, el CLI real reportó éxito (`"Click ejecutado — resuelto por UI Automation contra el <Document>..."`). **Verificación independiente de esta corrida puntual quedó contaminada**: la consulta UIA de foco real, corrida ~500ms después, encontró un elemento (`"Aceptar"`, `ControlType.Button`) de una aplicación de terceros completamente ajena (`AUTO-CATASTRO CR 5.12.0`) que el usuario tenía abierta en esta misma máquina real — confirmado con una consulta adicional que el foco real de Windows había pasado a esa app ajena, no a Notepad, en el momento de leer. Esto es contaminación real del entorno de prueba (una app de terceros del usuario robando foco real en la ventana de tiempo entre la ejecución y la verificación), no evidencia de que el click haya fallado — el diálogo, el movimiento real medido con foreground confirmado, y el reporte coherente del CLI real están todos confirmados; solo el paso final de lectura de foco de ESTA corrida puntual no es concluyente. Combinado con el Punto 1 (arriba, mismo mecanismo exacto, verificación independiente limpia y sin contaminación) y con el `keyboard_type` de esta misma actualización (verificación independiente limpia), la evidencia agregada sigue siendo sólida — documentado con la misma honestidad que el resto de esta bitácora, sin inflar una corrida puntualmente contaminada a una confirmación que no tiene.

**Hallazgo real, documentado, no arreglado** (encontrado durante la primera pasada, confirmado con `git diff` que es preexistente, ajeno a este feature): cambiar el sandbox mode del composer mid-sesión dispara `disconnect()` (código preexistente) sin el re-arme que sí existe para el camino de auto-reconexión del primer mensaje — `computerUseActive` se apaga solo (de forma fail-safe, visible en el DOM real) cada vez que se cambia de sandbox con Familia A ya armada. Ver `docs/_arch/PENDING.md`.

**Limpieza real completa (ambas pasadas)**: todos los procesos `electron.exe`/`FlaUIHelper.exe`/Notepad de prueba matados por PID, `AMATISTA_STORAGE_ROOT` aislado borrado en cada corrida, `D:\AMATISTA\data` real nunca tocado. Cero cambios de código fuente en ninguna de las 2 pasadas de verificación (no se encontró ningún bug real atribuible a este diff que ameritara un fix). Sin commit — pendiente de que el usuario lo pida.

## Fix estructural real — `computerUseActive`/`browserControlActive` dejan de resetearse en `disconnectSession()`, cierra el patrón recurrente de raíz

Basado en `docs/_arch/verify_session_flags_survive_disconnect_design.md` + una confirmación puntual posterior sobre el alcance real de `codex:logout` (ambos ya aprobados). Contexto: el mismo síntoma (`computerUseActive`/`browserControlActive` apagándose solos) apareció 3 veces reales (primer mensaje de un panel nuevo, cambio de sandbox mid-sesión, y la investigación de diseño encontró 3 disparadores incidentales más sin descubrir todavía: cambiar de proveedor, cambiar de modelo, cambiar de chat activo en el mismo panel, más `projects:removeRoot`/`workspace:open`) — parchar cada disparador nuevo con captura-antes/re-armado-después (el patrón ya usado 2 veces) requiere que alguien se acuerde de replicarlo en cada call site nuevo. Se invirtió la dirección del fix en vez de seguir parchando.

### Cambio real

`src/main/runtime-state.ts`: se quitaron las 2 líneas del `finally` de `disconnectSession()` que apagaban estos 2 campos incondicionalmente. Ahora son una decisión explícita del usuario que sobrevive a cualquier disconnect incidental — solo se apagan en 4 puntos deliberados reales:
- Checkbox "Control de escritorio"/"Navegador" desmarcados a mano (`App.tsx`) — ya llamaban `api.setComputerUseActive(false)`/`api.setBrowserControlActive(false)` DIRECTO, sin pasar por `disconnectSession()`. Confirmado sin cambios necesarios.
- Panic key real (`panicStop()`, `runtime-state.ts`) — ya llamaba `setComputerUseActive(panelId, false)` directo para cada panel armado. Confirmado sin cambios necesarios (no toca `browserControlActive`, deliberado, sin tecla de pánico a nivel de SO para el navegador embebido, ya documentado).
- **Nuevo**: `src/main/ipc-agent.ts`, handler `agent:disconnect` — reset EXPLÍCITO agregado justo antes de `disconnectSession()` cuando `payload.panelClosing === true` (la señal real de `closePanel()`/`deleteChat()`, el único punto deliberado real que SÍ dependía del reset que se acaba de quitar): lee `sessionRegistry.get(panelId)` y llama `setComputerUseActive(panelId, false)`/`setBrowserControlActive(panelId, false)` si estaban activos, ANTES de que `sessionRegistry.delete(panelId)` borre el objeto — necesario porque `destroyBrowserView()`/la limpieza de `armedPanels`/`inFlightPanels` viven en estructuras SEPARADAS de `SessionRuntimeState` (un `Map`/2 `Set` a nivel de módulo), que `sessionRegistry.delete()` no toca.
- **Simplificación**: `connectSessionForWindow()` (`ipc-agent.ts`) — se eliminó el parche de captura/re-armado (`browserControlWasActive`/`computerUseWasActive`, 6 líneas) que motivó la investigación. Ya no hace falta: `disconnectSession()` nunca vuelve a tocar estos 2 campos, así que no hay nada que capturar ni restaurar.

`toolTrustSession`/`planModeActive`/`planModeEnforced`/`priorSandbox` sin tocar — confirmado en la investigación que no califican para esta migración (`toolTrustSession` es un candidato real pero con un argumento genuino en contra, su propio texto de UI dice "por el resto de esta sesión"; "modo plan" es un modo de tarea, no una capacidad de riesgo persistente).

`npm run typecheck`/`npm run build` limpios.

### Verificación real, harness CDP (`electron.exe .` + `AMATISTA_STORAGE_ROOT` aislado, providers reales auto-provisionados por `sanitizeSettings()`)

**Punto 1 — los disparadores incidentales, el flag debe sobrevivir**: con `computerUseActive` real activado (`checked:true`), se dispararon en vivo 3 de los 6 disparadores confirmados por código en la investigación (los más representativos, de mayor uso real):
- Cambio real del `<select>` de sandbox mid-sesión (a `danger-full-access`) — `checked:true` después.
- Cambio real de chat activo en el mismo panel (click real en "+ Nuevo chat") — `checked:true` después.
- Cambio real de proveedor/modelo en el picker del composer (click real en `.model-item`, un ítem distinto al activo, entre las 9 opciones reales del picker: Claude Sonnet/Opus, GPT-5.6-Sol/Terra/Luna, GPT-5.5, Antigravity Auto, Gemini 3.1 Pro/3.7 Flash) — `checked:true` después.

Los otros 3 (`projects:removeRoot`/`workspace:open`/`codex:logout`) comparten el mismo mecanismo ya probado (los 3 terminan en la MISMA `disconnectSession()`, sin lógica propia adicional) — no se re-ejercitaron uno por uno; `codex:logout` deliberadamente NO se disparó de verdad contra la cuenta real de Codex/ChatGPT de esta máquina (evitar un logout real e innecesario del entorno del usuario).

**Punto 2 — los puntos deliberados, sin cambios, deben seguir apagando el flag**: checkbox desmarcado a mano real (`checked:false` después) y panic key real (`Ctrl+Alt+Shift+Esc` vía `keybd_event` físico, mismo mecanismo ya usado en la verificación original de Familia A) — `checked:false` después de ambos. Ningún cambio de comportamiento, confirmado.

**Punto 3/4 — el caso crítico: `browserControlActive`+`computerUseActive` reales, `closePanel()` real**: panel con ambos flags activados de verdad (vista embebida real viva — confirmado con un 2do target real apareciendo en la lista de CDP, `Target.getTargets`, inexistente al arrancar la app, apareció recién al activar el toggle). Se abrió un 2do panel real (`closePanel()` se niega a cerrar el último) y se cerró el PRIMER panel (el armado) con el botón real "Cerrar panel" (`×`, `closePanel()` → `agent:disconnect` con `panelClosing:true`).

Con instrumentación temporal (2 funciones debug + 1 IPC + 1 binding de preload, agregadas y **revertidas por completo** antes de cerrar esta pasada — confirmado con `grep -rn` sobre `src/` que no queda ningún rastro):
- `views.has(panelId)` (el `Map` real de `embedded-browser.ts` que trackea qué panel tiene una `WebContentsView` viva) pasó de `true` (antes de cerrar) a `false` (después) — confirma que `destroyBrowserView()` SÍ se ejecutó real (`removeChildView()` real llamado sobre la ventana real, entrada real removida del `Map`).
- `armedPanels`/`inFlightPanels` (los 2 `Set` reales de `runtime-state.ts` que controlan el panic key global y el overlay) quedaron en `{armed:0, inFlight:0, armedIds:[], inFlightIds:[]}` tras cerrar — **sin ninguna entrada huérfana**, confirmando que cerrar un panel armado NO deja el panic key global apuntando a un panel que ya no existe.

**Hallazgo real, honesto, confirmado PRE-EXISTENTE (no una regresión de este fix)**: el target real de la `WebContentsView` en la lista de CDP siguió apareciendo hasta 15s después de cerrar el panel, pese a que `views.has(panelId)` ya confirmaba `false`. Investigado: `destroyBrowserView()` (código sin ningún cambio en esta pasada) no tiene `.destroy()` explícito disponible en la API real de `WebContentsView` — el propio comentario del archivo, escrito ANTES de esta investigación, ya documentaba que remover la vista del `contentView` la deja "elegible para GC real", sin garantía de tiempo. Confirmado que la función no cambió en este diff — el comportamiento (el target de depuración tarda en desaparecer, aunque la vista real ya está desenganchada, invisible e inutilizable) es una característica pre-existente del mecanismo, no algo introducido por este fix. Ver `docs/_arch/PENDING.md` para el detalle completo, anotado como candidato real a investigar aparte si el usuario lo considera relevante.

**Limpieza real completa**: proceso `electron.exe` aislado matado por PID, `AMATISTA_STORAGE_ROOT` borrado por completo, `D:\AMATISTA\data` real nunca tocado (ninguna cuenta real logueada/deslogueada). Sin commit — pedido explícito del usuario.

## Feature real — Mascota Q integrada en Amatista (decorativa, acento de marca por modelo activo)

3 archivos reales del proyecto Q (`mascot.js`/`mascot-embed.js`/`three.min.js` r128 UMD), ya probados por el usuario fuera de Electron (preview real, `mascot-standalone.html`) — pedido explícito de solo integrarlos, sin tocar su contenido más allá de lo necesario para que compilen en el contexto real de Amatista. **Honestidad real del proceso**: el primer intento del usuario de "pasar los 3 archivos" no trajo ningún adjunto/contenido real al chat, solo la descripción — se buscó en el disco real de la máquina (`D:\QV3\q-mascot`/`D:\QV3\q-shell`) y se encontraron 2 candidatos reales que NO coincidían con la descripción (una versión de `mascot.js` sin `setModel()` ni los estados `scared`/`angry`/`transform`, y un `robot-ui.js` que contradecía la restricción "cero IPC/fetch/WS" — tenía `fetch`/`WebSocket` reales a un backend de Q y listeners globales en `document` con `preventDefault()` incondicional que habrían roto el click/selección de texto en TODA la app). Confirmado con evidencia real (`grep`) antes de tocar nada — el usuario mandó el contenido real recién en el siguiente mensaje, que sí coincide exacto con la descripción original (`setModel()` con `MODEL_COLORS`, 14 estados completos, `mascot-embed.js` sin ningún `fetch`/`WebSocket`/`window.qRobot`).

### Cambio real

- `src/renderer/public/mascot/{three.min.js,mascot.js,mascot-embed.js}` — copiados verbatim (byte a byte, sin ninguna edición) desde el contenido real que mandó el usuario (`mascot.js`/`mascot-embed.js`) + `D:\QV3\q-mascot\www\vendor\three.min.js` (idéntico, confirmado por `diff`, a la copia de `q-shell`). `public/` es la convención real de Vite/electron-vite para assets que se sirven tal cual (confirmado real: `npm run build` los copió verbatim a `out/renderer/mascot/`, mismo mecanismo que ya usa `out/renderer/index.html` con paths relativos `./assets/...` — confirmado que este proyecto ya corre con base relativa, necesaria para `file://` en producción).
- `src/renderer/src/App.tsx`: `declare global { interface Window { Mascot?: {...} } }` (los 3 scripts son globales planos, no ES modules, la única forma real de tipar el acceso). `mascotModelName(providerType)` — mapeo literal `ProviderType` real de Amatista → 4 buckets de Mascot (`anthropic`→`claude`, `openai`/`openai-codex`→`gpt`, `google`/`antigravity`→`gemini`; `foundry`/`openai-compatible`/`openrouter` sin mapeo, cae al default silencioso que ya trae `setModel()`, tal como pedía la restricción explícita de no inventar casos nuevos). `useEffect` en `App()` que inyecta los 3 `<script>` reales en orden estricto (`three.min.js`→`mascot.js`→`mascot-embed.js`, cada uno espera el `onload` del anterior), con un guard a nivel de módulo (`mascotScriptsInjected`) para que el doble-mount real de `StrictMode` (`main.tsx`, confirmado con `grep`) no inyecte una 2da instancia corriendo en paralelo sobre el mismo canvas (los scripts dejan efectos reales — contexto WebGL, loop de `requestAnimationFrame`, cadena de `setTimeout` — que un cleanup de `useEffect` removiendo el `<script>` del DOM no deshace). `<div className="mascot-container"><canvas id="mascot-canvas" /></div>` real en el sidebar, entre `.sidebar-brand` y `.new-chat-row`. Único call real a `Mascot.setModel()` dentro de `setActiveChatModel()` (la función real y única a la que delegan `selectProvider()`/`selectModel()`, confirmado por lectura de código) — resuelve el `ProviderProfile` real por `providerId` y llama `window.Mascot?.setModel(mascotModelName(provider.type))`.
- `src/renderer/src/assets/main.css`: `.sidebar` gana una fila `auto` real nueva en su `grid-template-rows` (única definición real de esa propiedad en todo el archivo, confirmado con `grep` — las otras 2 reglas `.sidebar` de breakpoints/rediseño no la tocan). `.mascot-container` (alto fijo real 150px) + `#mascot-canvas` (100%/100%, el propio `resize()` de `mascot.js` mide `offsetWidth`/`offsetHeight` reales de este contenedor).

Cero cambios en `mascot.js`/`mascot-embed.js` — copiados tal cual, confirmado con `diff` contra el contenido real recibido.

### Verificación real, harness CDP (`electron.exe .` + `AMATISTA_STORAGE_ROOT` aislado, providers reales auto-provisionados por `sanitizeSettings()`: `qcfg-claude-subscription`/anthropic, `qcfg-antigravity-subscription`/antigravity, Codex ChatGPT/openai-codex)

**Punto 1 — la mascota renderiza real dentro de Amatista corriendo**: `#mascot-canvas` real presente en el DOM con tamaño real (`291×150`, coincide con `.mascot-container`). Screenshot real (`Page.captureScreenshot`, clip real al `getBoundingClientRect()` del canvas) confirma el robot renderizado — cuerpo bronce/negro, ojos con lente azul brillante, aro base con el logo "Q" real, exactamente el diseño del `mascot.js` recibido.

**Punto 2 — el ciclo autónomo cicla solo, sin intervención**: confirmado 2 veces reales, con un wrapper temporal instalado vía CDP (`window.Mascot.setAnim`/`setState` envueltos para loguear cada llamada real con timestamp — instalado a tiempo, dentro de los 2s reales de margen antes del primer `scheduleFun()` de `mascot-embed.js`, sin tocar ningún archivo fuente). Corrida de 65s reales: **7 transiciones reales sin intervención** — `idle(0ms)→spinning(+15002ms)→idle(+19004ms)→happy(+31013ms)→idle(+34516ms)→sleeping(+54526ms)→idle(+62528ms)`, timing real casi exacto al `FUN_STATES` definido en `mascot-embed.js` (15000/4000/12000/3500/20000/8000ms). Corrida de reconfirmación, 20s: `idle→spinning(+15003ms)→idle(+19006ms)`. Cero llamadas a `fetch`/`WebSocket` observadas (confirmado por lectura del propio `mascot-embed.js` recibido, sin ninguna).

**Punto 3 — el color del aro/lente cambia real al cambiar de proveedor/modelo real en el picker**: click real en `.model-btn` → `.model-item` real ("GPT-5.6-Sol", provider real `type:'openai-codex'`), después "Claude Sonnet" (provider real `type:'anthropic'`) — cada click dispara `setActiveChatModel()` real → `window.Mascot.setModel()` real. **Evidencia dura, no solo visual**: 2 screenshots reales (`Page.captureScreenshot`) decodificados con un parser PNG propio (zlib + unfiltering real de scanlines, sin dependencias) para leer valores RGB exactos — los píxeles reales más saturados de cada captura:
- Estado GPT: `rgb(44,144,88)` en `(144,52)` — verde real, coincide con `MODEL_COLORS.gpt = 0x10a37f` (`rgb(16,163,127)`).
- Estado Claude: `rgb(192,118,65)` en `(160,54)` — naranja/terracota real, coincide con `MODEL_COLORS.claude = 0xd97757` (`rgb(217,119,87)`).

Misma región aproximada del canvas real, mismo estado (`idle`, forzado explícito con `Mascot.setAnim('idle')` antes de cada captura para comparar en igualdad de condiciones) — el color cambió de verde real a naranja real, confirmando `setModel()` funcionando de punta a punta a través del picker real de la UI. (Intento inicial de leer píxeles en vivo vía `canvas.getContext('2d').drawImage()` dio `rgb(0,0,0)` uniforme — `WebGLRenderer` de `mascot.js` no tiene `preserveDrawingBuffer` activado, el drawing buffer ya está limpio para cuando el JS externo lo lee; **no es un bug de la integración**, es un comportamiento estándar de WebGL — se resolvió comparando los PNGs reales ya capturados por el compositor de Chrome en vez de leer el canvas en vivo.)
...
## Feature real — Mascota Q arrastrable por toda la ventana de Amatista

4to archivo real del proyecto Q (`mascot-drag.js`, contenido real recibido, Pointer Events sobre el propio contenedor — nunca `document`, a diferencia del `robot-ui.js` original que ya se había descartado en la feature anterior por esa misma razón). Se saca el contenedor de la mascota del grid del sidebar (donde vivía desde la feature anterior) y pasa a `position:fixed` a nivel de `.app`, para poder arrastrarse sobre cualquier parte real de la ventana.

### Cambio real

- `src/renderer/public/mascot/mascot-drag.js` — copiado verbatim (mismo directorio que los otros 3, mismo mecanismo real de `public/` de Vite). Cero cambios.
- `src/renderer/src/App.tsx`:
  - `declare global` gana `window.initMascotDrag?: (container: HTMLElement) => (() => void) | null`.
  - La cadena de inyección de scripts (`useEffect` ya existente) gana un 4to elemento real: `mascot/mascot-drag.js`, después de `mascot-embed.js`.
  - El `<div className="mascot-container">` se movió de adentro de `<aside className="sidebar">` a directamente dentro de `.app` (mismo criterio real que `.sidebar-toggle`, ya `position:fixed` y ya documentado con el mismo motivo: tiene que seguir siendo alcanzable sin depender del layout del sidebar).
  - `mascotContainerRef` (nuevo) + un `useEffect` nuevo: calcula la posición inicial real (abajo a la derecha, `left`/`top` en px, `window.innerWidth/innerHeight - ancho/alto - margen`, nunca `bottom`/`right` — confirmado que `mascot-drag.js` solo lee/escribe `left`/`top`), después pollea (100ms, hasta 5s) hasta que `window.initMascotDrag` exista real (carga async de la cadena de scripts) y lo invoca con el contenedor real, guardando el cleanup real que devuelve para el cleanup del propio `useEffect`.
- `src/renderer/src/assets/main.css`: `.sidebar` vuelve a su `grid-template-rows` original de 4 filas (la mascota ya no vive ahí). `.mascot-container` pasa a `position:fixed; width:220px; height:150px; z-index:35` (por encima de `.sidebar-toggle`, z-index 30; por debajo de `.scrim`/`.settings-panel`/`.context-menu`, 80/90/400 — decorativa, nunca debe tapar un modal real) + `cursor:grab`/`.mascot-dragging{cursor:grabbing}`/`touch-action:none`.

Cero cambios en `mascot.js`/`mascot-embed.js`/`mascot-drag.js` — los 4 archivos siguen siendo exactamente el contenido real recibido.

### Verificación real, harness CDP (`electron.exe .` + `AMATISTA_STORAGE_ROOT` aislado)

**Punto 1 — arrastre real a 3 zonas distintas de la ventana, con `Input.dispatchMouseEvent` (mouse real simulado, no `.click()` sintético, con `mousePressed`→`mouseMoved`×N→`mouseReleased`)**: posición inicial real confirmada abajo a la derecha (`x:1348,y:775` sobre una ventana de `1584×915`). Arrastre real 1 → zona SIDEBAR: `(30,225)`. Arrastre real 2 → zona CHAT: `(790,325)`, screenshot real guardado. Arrastre real 3 → esquina OPUESTA: `(0,0)` (clamp real a 0 confirmado, no negativo). Los 3 arrastres reales movieron el contenedor real y quedó ahí, confirmado leyendo `getBoundingClientRect()` real después de cada uno, no solo confiando en que el evento se disparó.

**Punto 2 — clicks normales en el resto de la UI siguen funcionando, sin interferencia real**: confirmado con clicks reales por coordenada (`Input.dispatchMouseEvent`, no `element.click()`) — con la mascota real posicionada en el sidebar pero SIN superponerse al botón real "+ Nuevo chat" (confirmado explícito: `overlaps:false` antes del click), un click real en el centro exacto del botón real disparó la apertura real del composer (`textarea` presente después). Un click real por coordenada en el `textarea` del composer (área de chat) lo dejó enfocado real (`document.activeElement === textarea`). Screenshot real adicional confirma visualmente que la mascota, posicionada en el sidebar, no tapa ningún control real.
...
## 3 fixes reales — confirmación faltante al borrar chat, notificación `turn/cancelled` faltante para Codex, distinción visual de la fila de proyecto

Basado en una investigación previa (3 reportes reales del usuario, pura — sin tocar código), que confirmó con evidencia de código:

- **Bug 1** (el botón "Detener" no corta un turno de Codex): el kill del proceso real SÍ funcionaba (`cancelSessionTurn()` → `session.cancelCurrentTurn` → `client.stop()` → `RpcStdioClient.stop()` → `this.process.kill()` sobre el subproceso real `codex app-server --stdio`, PIEZA 5 ya existente) — el defecto real era que la rama Codex de `ipc-agent.ts` (~305-402) nunca emitía ningún `sendSessionEvent()` al cancelar, a diferencia de las ramas API/CLI (que sí lo hacen al capturar `TurnCancelledError`). `turnActive` (lo que dirige el spinner) solo se limpia vía el handler de la notificación `turn/cancelled` en `handleAgentEvent()` (`App.tsx`) — sin la notificación, quedaba pegado hasta que el watchdog de turno (120s) lo timeouteaba solo. Confirmado también que Codex no expone protocolo de cancelación enviable (`turn/cancelled` solo se escucha, nunca se envía) — matar el proceso sigue siendo la única cancelación real posible, eso no cambió.
- **Bug 2** (fila de proyecto visualmente idéntica a sus chats): confirmado con CSS/JSX real — `.root-title-open` (la fila raíz/contenedor) resultaba más CHICA (`12px` vs `13px !important` de `.project`, los hijos) y más APAGADA (`#999` vs `#ccc`) que sus propios chats — invertido respecto de lo esperable para un contenedor, sin ningún `font-weight` distintivo en ninguna de las dos.
- **Bug 3** (chat dependiente borrado sin poder restaurar): confirmado permanente por diseño hoy — `deleteChatSession()` (`chat-store.ts`) es un `DELETE FROM chat_sessions WHERE id = ?` real, y el esquema completo de `chat_sessions` (incluidas todas las migraciones `ALTER TABLE`) no tiene ninguna columna de soft-delete — no hay nada dormido para exponer, el gap es de esquema. Hallazgo adicional, no pedido explícitamente pero real: el botón de borrar (`App.tsx`) era el ÚNICO de sus pares (borrar proveedor/preset/logout Codex) sin `window.confirm()` — implementado como Fix 1 de esta pasada, por pedido explícito del usuario tras ver la investigación. Fix 4 (papelera real/soft-delete) quedó deliberadamente FUERA de esta pasada — requiere cambio de esquema genuino, diseño breve entregado aparte para aprobación antes de tocar nada.

### Fix 1 — `window.confirm()` faltante en borrar chat

`src/renderer/src/App.tsx`, `deleteChat(chatId)`: agrega, al principio de la función, el mismo patrón ya usado en los otros 3 lugares comparables (`if (!window.confirm(confirmMessage)) return`), con un mensaje real que incluye el título del chat (`Eliminar el chat "${target.title}"? Esta accion no se puede deshacer.`). Cubre los 2 call sites reales de `deleteChat()` (el botón `x` de cada fila y la opción del menú contextual) sin duplicar la lógica.

### Fix 2 — notificación `turn/cancelled` faltante en la rama Codex

`src/main/ipc-agent.ts`, rama `session.activeRuntime === 'codex'`: agrega, justo después de `await waitForCompletion` y antes de la compactación fire-and-forget, un `if (turnCancelled) { sendSessionEvent(panelId, {chatId: requestChatId, workspace: requestWorkspace, kind:'notification', method:'turn/cancelled', params:{partialText: accumulatedText || undefined}}) }` — mismo patrón EXACTO que las ramas API/CLI de más arriba en el mismo archivo. Cero cambios al protocolo JSON-RPC de Codex ni al mecanismo de cancelación en sí (matar el proceso, ya existente).

### Fix 3 — distinción visual de la fila de proyecto (sidebar)

`src/renderer/src/assets/main.css`, `.root-title-open`: reusa el lenguaje visual ya existente de `.section-label` (`letter-spacing`, la firma real de "esto es estructura/categoría" en este sidebar) en una versión más liviana — brillo SUBIDO (`color:#d8d8d8`, no el `#7c7c7c` de `.section-label` tal cual) y `font-size` igualado al de los hijos (`13px`, corrige la inversión de tamaño) — para que la fila lea claramente MÁS prominente que sus chats, no más apagada. `.root-title-chevron` hereda el color nuevo sin cambios propios (no tenía color explícito).

### Verificación real, una por una (rebuild `npm run build` antes de cada pasada CDP — el harness lanza `electron.exe .` contra `out/`, no contra source)

**Fix 1**: 2 clicks reales (`Input.dispatchMouseEvent`, no `.click()` sintético) sobre el botón `x` real de un chat real (`Chat general`, único chat al arrancar). El diálogo nativo de Electron SÍ apareció con el mensaje real exacto (`Eliminar el chat "Chat general"? Esta accion no se puede deshacer.`, confirmado vía el evento real `Page.javascriptDialogOpening`) — pero `Page.handleJavaScriptDialog` no logró resolverlo (`hasBrowserHandler:true` en el evento — limitación real y conocida de CDP combinado con los diálogos nativos de Electron en Windows, no un defecto del código de la app). Verificación end-to-end del comportamiento real completada controlando la respuesta vía un `window.confirm` interceptado por CDP (mismo click real por coordenada, mismo código real de `deleteChat()` ejercitado de punta a punta): cancelar preservó el chat original (`chats reales tras CANCELAR: 1`, mismo título); confirmar lo borró de verdad — como era el único chat abierto, el fallback preexistente de `deleteChat()` creó `"Chat nuevo"` en su lugar (comportamiento previo, sin relación a este fix) — título real confirmado distinto del original, prueba de que el chat original sí se borró.

**Fix 2**: turno REAL contra Codex, no simulado — provider real `openai-codex`/`authMode:'subscription'`/modelo real `gpt-5.5` (confirmado que `CODEX_HOME` no se overridea para `authMode:'subscription'` en `codex-client.ts`, así que este turno usó la sesión real ya autenticada del usuario en `%USERPROFILE%\.codex\auth.json`, sin ningún aislamiento). Panel conectado real (click real en "Conectar agente", `agentState` real pasó a `connected`), prompt real largo enviado (Enter real via `Input.dispatchKeyEvent`), `.stop-btn` (el botón que reemplaza a "enviar" mientras `turnActive`) apareció en 2-3ms confirmando el turno real activo. Click real en "Detener" a los 3s de turno activo (turno genuinamente en curso, no en el instante 0) — `.stop-btn` desapareció en **11ms** y **161ms** (2 corridas independientes), nunca cerca de los 120s del watchdog. Estado final real tras cancelar: `"Agente sin iniciar"` (desconectado, NO un estado de error) — confirmado como comportamiento esperado y ya documentado (matar el proceso app-server real mata también el thread, requiere reconexión manual, PIEZA 5 preexistente sin cambios).

**Fix 3**: proyecto real (este mismo repositorio) agregado a `projectRoots`, 2 chats reales creados dentro vía 2 clicks reales en el botón "+" (`.project-new-session`). Estilo computado real confirmado con `getComputedStyle()` (no solo la regla CSS en el archivo): fila raíz `fontSize:13px`, `color:rgb(255,255,255)` (estado `.active` real, el panel quedó con ese workspace activo tras crear los chats), `letterSpacing:0.52px` — vs. fila hija `fontSize:13px`, `color:rgb(204,204,204)`, `letterSpacing:normal`. Screenshot real adjunto (`fix3_project_row.png`) confirma visualmente: la fila de proyecto ahora se lee claramente más prominente (blanca, con tracking de letras) que sus 2 chats debajo (gris plano) — inversión del problema original corregida.

`npm run typecheck`/`npm run build` limpios tras cada uno de los 3 fixes. Limpieza real completa en las 3 pasadas: procesos `electron.exe` aislados matados por PID, cada `AMATISTA_STORAGE_ROOT` temporal borrado por completo, `D:\AMATISTA\data` nunca referenciado (todas las pasadas usaron `AMATISTA_STORAGE_ROOT` aislado en `%TEMP%`). Sin commit — pendiente de que el usuario lo pida.

## Feature real — Papelera (soft-delete real, `deleted_at`)

Cierra el Bug 3 de la investigación de la sección anterior (chat dependiente borrado sin poder restaurar) — confirmado ahí como **permanente por diseño**, sin ninguna columna de soft-delete dormida en el esquema. Diseño breve confirmado y aprobado por el usuario antes de tocar el esquema real.

### Esquema

`src/main/chat-store.ts`, misma migración `ALTER TABLE ... ADD COLUMN` + `try/catch` que `parent_chat_id`/`summary`/etc.: `ALTER TABLE chat_sessions ADD COLUMN deleted_at TEXT`. `NULL` = chat activo (default, compatible con todo lo existente); timestamp ISO = borrado (soft-delete).

### Cambio real

- `deleteChatSession(chatId)`: pasó de `DELETE FROM chat_sessions WHERE id = ?` a `UPDATE chat_sessions SET deleted_at = ? WHERE id = ?` — ya NO dispara el `ON DELETE CASCADE` real de `chat_messages`/`chat_attachments` (ese FK sigue existiendo tal cual, solo que un `UPDATE` no lo activa).
- `loadChatSnapshot()` (la única query real que alimenta el sidebar) ganó `WHERE deleted_at IS NULL` — único punto de filtrado, por restricción explícita del pedido. **Nota real, documentada, no arreglada**: otras 3 queries de `chat-store.ts` (`findChatSessionByTitle`, `findChatSessionByPanelAlias`, `listChatSessionsForWindowDiscovery` — esta última alimenta `send_to_window`/`list_windows`) siguen sin este filtro — ver `PENDING.md` para el detalle de por qué se dejó así.
- `restoreChatSession(chatId)` (nueva): `UPDATE chat_sessions SET deleted_at = NULL WHERE id = ?`.
- `purgeChatSession(chatId)` (nueva): el `DELETE` real que antes hacía `deleteChatSession()` — ahora es el ÚNICO punto real donde dispara el cascade de mensajes/adjuntos, y solo se llama desde la Papelera.
- `listDeletedChatSessions()` (nueva): `SELECT id, title, deleted_at FROM chat_sessions WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC`.
- `src/main/ipc-chats.ts`: 3 handlers nuevos (`chats:restoreSession`/`chats:purgeSession`/`chats:listDeleted`), mismo patrón que `chats:deleteSession` ya existente.
- `src/preload/index.ts`/`index.d.ts`: 3 bindings nuevos, tipado manual (confirmado que `UniversalAgentApi` en `index.d.ts` no se genera automático desde `index.ts`, hay que mantener los 2 en sync a mano).
- `src/renderer/src/App.tsx`: estado nuevo `deletedChats` (solo `id`/`title`/`deletedAt`, nunca el historial completo — eso se trae solo al restaurar). `refreshDeletedChats()` (llamado en `bootstrap()` y tras cada borrado/restauración/purga). `restoreChat(chatId)`: en vez de reconstruir a mano un `ChatSession` parcial desde los 3 campos que trae `listDeletedChatSessions()`, resincroniza `chatSessions`/`chats` por completo vía el mismo `window.universalAgent.loadChats()` que ya usa `bootstrap()` — el chat restaurado ya aparece en ese resultado (el filtro de `loadChatSnapshot()` ya no lo excluye), con su historial de mensajes real intacto. `purgeChat(chatId, title)`: `window.confirm()` con lenguaje reforzado, deliberadamente distinto del de `deleteChat()` (Fix 1 de la sección anterior) — "esto SÍ es irreversible", porque ESTE sí lo es.
- Sección nueva "PAPELERA" al final del sidebar (`App.tsx`), renderizada SOLO si `deletedChats.length > 0` — nunca una sección vacía permanente. Cada fila: título (`MarqueeSpan`) + botón Restaurar (↺) + botón Eliminar definitivamente (x).
- `src/renderer/src/assets/main.css`: `.trash-row`/`.trash-title`/`.trash-restore`/`.trash-purge` — mismo lenguaje visual ya establecido (`.project-row`/`.chat-delete-btn`), apagado a propósito (`#888`, igual que `.add-root` — estos chats ya no son "activos").

**Deliberadamente sin tocar** (restricción explícita, confirmado que no hacía falta): `parent_chat_id` y su manejo de huérfanos en `buildChatRows()` (`App.tsx`) — un chat soft-deleted simplemente deja de aparecer en `loadChatSnapshot()`, así que sus hijos reales lo ven exactamente como ya ven hoy a un padre borrado (huérfano, promovido a raíz), cero código nuevo necesario ahí. Sin ninguna política de auto-purga por tiempo (restricción explícita) — la purga es siempre una acción manual y deliberada desde la Papelera.

### Verificación real, los 5 casos pedidos (harness CDP + `node:sqlite` en lectura directa contra la SQLite real — no solo lo que muestra la UI)

Harness en 2 arranques: 1º arranca `electron.exe .` una vez para que la app cree el esquema real (con la migración `deleted_at` ya aplicada), lo mata; entre medio, sembrado real directo en `amatista.db` vía `node:sqlite` (mismo módulo que usa `chat-store.ts`) — un chat standalone (`chat-d`, con 1 mensaje real) para los casos 1-4, y un padre real (`chat-p`, con 1 mensaje real) con 2 hijos reales (`chat-p-child1`/`chat-p-child2`) para el caso 5; 2º arranque para la verificación real. Las lecturas de SQLite durante la verificación se hacen con una 2ª conexión de solo lectura MIENTRAS Electron sigue corriendo (WAL permite lectores concurrentes) — nunca solo confiando en el estado de la UI.

**Caso 1+2 — borrar real, soft-delete confirmado, aparece en Papelera**: click real (`Input.dispatchMouseEvent`) en el botón `x` de `chat-d`, `window.confirm` interceptado para simular "Aceptar". Estado real en SQLite tras el click: `{id:'chat-d', deleted_at:'2026-09-17T14:40:04.667Z'}`, `chatDCount:1`, `msgDCount:1` — confirma que es un `UPDATE` real, la fila y su mensaje siguen existiendo, NO un `DELETE`. Desaparece de `.chat-row` en el sidebar normal; aparece real en `.trash-title`: `["Chat D para papelera"]`.

**Caso 3 — restaurar real**: click real en el botón ↺ (`.trash-restore`) de la fila de Papelera. Estado real en SQLite tras restaurar: `deleted_at: null`, `chatDCount:1`, `msgDCount:1` (sin cambios, nunca se tocó). Reaparece real en `.chat-row`. Verificación reforzada, no solo la base: se abrió el chat restaurado en el panel real (click en `.chat-title-btn`) y se confirmó que el texto real del mensaje (`"Mensaje real de prueba en Chat D"`) aparece real en el DOM del panel — el historial de mensajes sobrevivió de punta a punta, no solo la fila de metadata.

**Caso 4 — purgar real desde la Papelera, cascade real, y SOLO ahí**: se volvió a borrar `chat-d` (2º soft-delete, para tenerlo de nuevo en la Papelera) — lectura real de SQLite JUSTO ANTES de purgar confirma `msgDCount:1` todavía (el cascade **no** disparó en el soft-delete, como debía). Click real en el botón `x` de purgar (`.trash-purge`) con el `window.confirm` reforzado interceptado. Estado real en SQLite tras purgar: `chatD: undefined`, `chatDCount:0`, `msgDCount:0` — `DELETE` real confirmado, con su cascade real de mensajes, disparado únicamente en este paso. `.trash-title` real queda vacío, la sección "PAPELERA" deja de renderizarse.
